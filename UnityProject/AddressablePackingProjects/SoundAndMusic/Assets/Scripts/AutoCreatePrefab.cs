...
public class AutoCreatePrefab : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        if (audioClipsToReplace.Count == 0 && prefabToDuplicate == null)
        {
            EditorGUILayout.HelpBox(
                "Please select the target prefab to duplicate, then select audio clips to create prefabs for before using this tool.",
                MessageType.Info
            );
        }

        prefabToDuplicate = EditorGUILayout.ObjectField(
            "Prefab to Duplicate",
            prefabToDuplicate,
            typeof(GameObject),
            false
    ...
    private void DisplayAudioClipsList()
    {
        ...
        {
            audioClipsToReplace[i] = EditorGUILayout.ObjectField(
                "Audio Clip " + i,
                audioClipsToReplace[i],
                typeof(AudioClip),
                false
            ) as AudioClip;
        }
    ...
    private void DuplicateAndReplace()
    {
        ...
        foreach (AudioClip audioClip in audioClipsToReplace)
        {
            GameObject duplicatedPrefab =
                PrefabUtility.InstantiatePrefab(prefabToDuplicate) as GameObject;
    ...
    private void CreatePrefabForAudioClip(AudioClip audioClip, string baseSavePath)
    {
        ...
        string directoryPath =
            Path.GetDirectoryName(relativePath)?.Replace("Audio Files", baseSavePath);
    ...
    private bool PrefabAlreadyExistsForAudioFile(string audioClip)
    {
        ...
        foreach (string prefabGuid in prefabGUIDs)
        {
            ...
            if (Path.GetFileNameWithoutExtension(prefabPath) != audioClip)
                continue;
