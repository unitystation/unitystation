...
public class GUI_PreRoundWindow : MonoBehaviour {
  ...
  [SerializeField]
  private Scrollbar loadingBar = null;

  [SerializeField]
  private GameObject normalWindows = null;

  [SerializeField]
  private GameObject warnText = null;
  ...
  [SerializeField]
  private GameObject rejoiningRoundPanel = null;
  ...
  private void UpdateCountdownUI() {
    ...
    timer.text = TimeSpan.FromSeconds(countdownEndTime - NetworkTime.time)
                     .ToString(@"mm\:ss");

    if (GameManager.Instance.QuickLoad && mapLoadingPanel.activeSelf == false) {
      if (startedAlready == true || this.isActiveAndEnabled == false)
        return;
      startedAlready = true;
  ...
  public void SyncCountdown(bool started, double endTime) {
    Logger.LogFormat(
        "SyncCountdown called with: started={0}, endTime={1}, current NetworkTime={2}",
        Category.Round, started, endTime, NetworkTime.time);
  ...
  public void OnReadyButton() {
    _ = SoundManager.Play(CommonSounds.Instance.Click01);
    CharacterCreator.CharacterCustomization customizerScript =
        characterCustomization
            .GetComponentInChildren<CharacterCreator.CharacterCustomization>();
  ...
  /// <summary>
  /// Sets the new ready status. Will tell the server about the new ready state
  /// if it has changed.
  /// </summary>
  ...
  /// <summary>
  /// Warns the player when they have no job selected and default their job
  /// preference
  /// </summary>
  ...
  private void SetInfoScreenOn() {
    ...
    if (string.IsNullOrEmpty(ServerInfoUI.serverDesc))
      return;
