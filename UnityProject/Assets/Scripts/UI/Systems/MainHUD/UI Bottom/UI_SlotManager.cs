...
public class UI_SlotManager : MonoBehaviour {
  ...
  public List<Tuple<IDynamicItemSlotS, BodyPartUISlots.StorageCharacteristics>>
      ContainSlots = new List<
          Tuple<IDynamicItemSlotS, BodyPartUISlots.StorageCharacteristics>>();
  public void Start() {
  ...
  public void UpdateUI() {
    if (PlayerManager.LocalPlayerScript.OrNull()?.DynamicItemStorage != null) {
      var DynamicItemStorage =
          PlayerManager.LocalPlayerScript.DynamicItemStorage;
      var Newstored = DynamicItemStorage.ClientSlotCharacteristic;
      List<Tuple<IDynamicItemSlotS, BodyPartUISlots.StorageCharacteristics>>
          Inadd = new List<Tuple<IDynamicItemSlotS,
                                 BodyPartUISlots.StorageCharacteristics>>();
      List<Tuple<IDynamicItemSlotS, BodyPartUISlots.StorageCharacteristics>>
          Inremove = new List<Tuple<IDynamicItemSlotS,
                                    BodyPartUISlots.StorageCharacteristics>>();

      foreach (var slot in Newstored) {
        if (slot.Value.NotPresentOnUI)
          continue;

        bool NotPresent = true;
        foreach (var Oldslot in ContainSlots) {
          if (Oldslot.Item1 == slot.Value.RelatedIDynamicItemSlotS &&
              Oldslot.Item2 == slot.Value) {
            NotPresent = false;
          }
        }

        if (NotPresent) {
          Inadd.Add(new Tuple<IDynamicItemSlotS,
                              BodyPartUISlots.StorageCharacteristics>(
              slot.Value.RelatedIDynamicItemSlotS, slot.Value));
        }
      }
      ...
      foreach (var OLDslot in ContainSlots) {
        ...
        foreach (var Newslot in Newstored) {
          if (OLDslot.Item1 == Newslot.Value.RelatedIDynamicItemSlotS &&
              OLDslot.Item2 == Newslot.Value) {
            NotPresent = false;
          }
        }
  ...
  public void AddContainer(IDynamicItemSlotS bodyPartUISlots) {
    foreach (var storageCharacteristicse in bodyPartUISlots.Storage) {
      if (storageCharacteristicse.SlotArea == SlotArea.Hands) {
        ...
        // if (ClientContents.ContainsKey(storageCharacteristicse.SlotArea) ==
        // false) ClientContents[storageCharacteristicse.SlotArea] = new
        // List<UI_DynamicItemSlot>();
  ...
  public void RemoveContainer(BodyPartUISlots bodyPartUISlots) {
    ...
    foreach (var uiDynamicItemSlot in BodyPartToSlot[bodyPartUISlots]) {
      OpenSlots.Remove(
          uiDynamicItemSlot.GetComponentInChildren<UI_DynamicItemSlot>());
      uiDynamicItemSlot.GetComponentInChildren<UI_DynamicItemSlot>()
          .ReSetSlot();
  ...
  public void RemoveSpecifyedUISlot(
      IDynamicItemSlotS bodyPartUISlots,
      BodyPartUISlots.StorageCharacteristics StorageCharacteristics) {
    if (BodyPartToSlot.ContainsKey(bodyPartUISlots) == false)
      BodyPartToSlot[bodyPartUISlots] = new List<GameObject>();
    var namedItemSlot = bodyPartUISlots.RelatedStorage.GetNamedItemSlot(
        StorageCharacteristics.namedSlot);
    for (int i = 0; i < BodyPartToSlot[bodyPartUISlots].Count; i++) {
      var slot = BodyPartToSlot[bodyPartUISlots][i]
                     .OrNull()
                     ?.GetComponentInChildren<UI_DynamicItemSlot>();
      ...
      if (slot.ItemSlot == namedItemSlot) {
        OpenSlots.Remove(BodyPartToSlot[bodyPartUISlots][i]
                             .GetComponentInChildren<UI_DynamicItemSlot>());
        BodyPartToSlot[bodyPartUISlots][i]
            .GetComponentInChildren<UI_DynamicItemSlot>()
            .ReSetSlot();
