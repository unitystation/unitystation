...
//		private readonly float targetAspect = 1.777f; // 16 : 9 aspect
		...
		private IEnumerator ForceGameWindowAspect()
		{
			...
			if (width % 2 != 0)
			{
//				TADB_Debug.Log( $"Odd width {width}->{width-1}" );
				width--;
			}
			int height = Screen.height;
			if (height % 2 != 0)
			{
//				TADB_Debug.Log( $"Odd height {height}->{height-1}" );
				height--;
			}
			
//			TADB_Debug.Log("Screen height before resizing: " + Camera.main.pixelHeight + " Aspect Y: " + height/(float)Screen.height);
//			TADB_Debug.Log("Screen height before resizing: " + Camera.main.pixelWidth + " Aspect X: " + width/(float)Screen.width);
...
//			TADB_Debug.Log("Screen height after resizing: " + Camera.main.pixelHeight);
