...
public class PanelHudBottomController : MonoBehaviour
{
	...
	public void TryInteractWithPocket(int slot)
	{
		...
		if (PlayerManager.LocalPlayerScript.ItemStorage.GetActiveHandSlot().IsEmpty && pocket.ItemSlot.IsEmpty)
		{
			Chat.AddExamineMsgToClient(emptyHandNPocketMessage);
			return;
		}
		// if hand and pocket are full
		if (PlayerManager.LocalPlayerScript.ItemStorage.GetActiveHandSlot().IsOccupied && pocket.ItemSlot.IsOccupied)
		{
