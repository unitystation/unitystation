...
public class CameraZoomHandler : MonoBehaviour {
  ...
  private void UpdatePixelPerfectCamera() {
    ...
    PixelPerfectCamera current =
        Camera.main.OrNull()?.GetComponent<PixelPerfectCamera>();
  ...
  private void DisplaySettings_SettingsChanged(
      object sender, DisplaySettings.DisplaySettingsChangedEventArgs e) {
    if (e.ZoomLevelChanged) {
      Refresh();
    }
  }

  void Awake() { displaySettings = FindObjectOfType<DisplaySettings>(); }
  ...
  void Update() {
    ...
    if (displaySettings.ScrollWheelZoom &&
