...
public class GeigerCounter : MonoBehaviour, IInteractable<HandActivate>, IServerInventoryMove
{
	...
	public void CycleUpdate()
	{
		...
		{
			// JESTER SoundManager.PlayNetworkedAtPos(Noise[Level.Extreme][RNG.Next(0,3)], registerItem.WorldPositionServer, sourceObj: gameObject);
		}
		else if (node.RadiationNode.RadiationLevel > 500)
		{
			// JESTER SoundManager.PlayNetworkedAtPos(Noise[Level.High][RNG.Next(0,3)], registerItem.WorldPositionServer, sourceObj: gameObject);
		}
		else if (node.RadiationNode.RadiationLevel > 100)
		{
			// JESTER SoundManager.PlayNetworkedAtPos(Noise[Level.Mid][RNG.Next(0,3)], registerItem.WorldPositionServer, sourceObj: gameObject);
		}
		else if (node.RadiationNode.RadiationLevel > 20)
		{
			// JESTER SoundManager.PlayNetworkedAtPos(Noise[Level.Low][RNG.Next(0,3)], registerItem.WorldPositionServer, sourceObj: gameObject);
		}
	}
