...
namespace US13.Items.Implants.Organs
{
	public class Lungs : BodyPartFunctionality
	{
			...
			foreach (var modifier in RelatedPart.AppliedModifiers)
			{
				...
				if (modifier == RelatedPart.DamageModifier)
				{
					toMultiply = Mathf.Max(0f,
						Mathf.Max((float)(RelatedPart.MaxHealth - RelatedPart.TotalDamageWithoutOxyCloneRadStam), 0) /
		...
		public bool TryBreathing(IGasMixContainer node, float efficiency, bool OverrideCooldown =false)
		{
			...
			{
				var inNode = RelatedPart.HealthMaster.RegisterTile.Matrix.MetaDataLayer.Get(SharedConverterExtensions.RoundToInt((Vector3)RelatedPart.HealthMaster.transform.localPosition));
