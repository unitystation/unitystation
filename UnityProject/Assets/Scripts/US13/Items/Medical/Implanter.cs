				...
				if (bodyPart.BodyPartType == interaction.TargetBodyPart && hasFoundPart == false)
				{
					...
					if (toSlot != null)
					{
						...
						ToolUtils.ServerUseToolWithActionMessages(interaction, timeToImplant,
							$"You begin injecting {interaction.TargetObject.ExpensiveName()}'s {SweetExtensions.ExpensiveName(bodyPart.gameObject)} with the implanter...",
							$"{interaction.Performer.ExpensiveName()} begins injecting {interaction.TargetObject.ExpensiveName()}'s {SweetExtensions.ExpensiveName(bodyPart.gameObject)} with the implanter...",
							$"You inject {interaction.TargetObject.ExpensiveName()}'s {SweetExtensions.ExpensiveName(bodyPart.gameObject)} with the implanter.",
							$"{interaction.Performer.ExpensiveName()} injects {interaction.TargetObject.ExpensiveName()}'s {SweetExtensions.ExpensiveName(bodyPart.gameObject)} with the implanter.",
							() =>
							{
