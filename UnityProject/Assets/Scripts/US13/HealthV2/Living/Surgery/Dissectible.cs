...
namespace US13.HealthV2.Living.Surgery
{
		...
		ICheckedInteractable<HandApply>, IExaminable, IHoverTooltip
	{
		...
		{
		    foreach (var organBodyPart in Enumerable.Where<BodyPart>(BodyPartIsOn.ContainBodyParts, organBodyPart => organBodyPart == onBodyPart))
		    {
		        foreach (var procedure in Enumerable
		                     .Where<SurgeryProcedureBase>(organBodyPart.SurgeryProcedureBase, procedure => procedure is not (CloseProcedure or ImplantProcedure))
		...
		{
		    foreach (var Procedure in Enumerable
		                 .Where<SurgeryProcedureBase>(bodyPart.SurgeryProcedureBase, procedure => procedure is CloseProcedure or ImplantProcedure)
		                 .Where(procedure => surgeryProcedureBase == procedure))
		    {
		...
		private void CheckAvailableBodyParts(SurgeryProcedureBase surgeryProcedureBase, BodyPart onBodyPart)
		{
		    foreach (var bodyPart in LivingHealthMasterBase.BodyPartList)
		    {
			    ...
			    foreach (var Procedure in Enumerable
				             .Where<SurgeryProcedureBase>(bodyPart.SurgeryProcedureBase, procedure => procedure is not (CloseProcedure or ImplantProcedure))
				             .Where(procedure => surgeryProcedureBase == procedure))
			    {
