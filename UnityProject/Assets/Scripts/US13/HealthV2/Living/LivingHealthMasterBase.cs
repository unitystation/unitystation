...
namespace US13.HealthV2.Living
{
		...
		IAreaReactionBase, IRightClickable, IServerSpawn, IHoverTooltip, IChargeable
	{
		...
		public ReagentPoolSystem reagentPoolSystem => SweetExtensions.OfType<ReagentPoolSystem>(ActiveSystems).FirstOrDefault();
		...
		{
			return SweetExtensions.OfType<T>(ActiveSystems).FirstOrDefault();
		}

		public bool TryGetSystem<T>(out T system)
		{
			system = SweetExtensions.OfType<T>(ActiveSystems).FirstOrDefault();
		...
		private IEnumerator ApplyNegativeMutation()
		{
			...
			while (Stability < 0)
			{
				...
				var MutationToApply = EnumerableExt.PickRandom(AvailableMutations);
			...
			double traumaChance = 50)
		{
			...
			var bodyPartToTakeDamage = EnumerableExt.PickRandom(SurfaceBodyParts);
		...
		public string GetExamineText(PlayerScript script = null)
		{
			...
			foreach (BodyPart part in BodyPartList)
			{
				...
				{
					healthString.Append(
						$"<color=red>\n {theyPronoun} {SweetExtensions.ExpensiveName(part.gameObject)} is bleeding!</color>");
				}
			}
