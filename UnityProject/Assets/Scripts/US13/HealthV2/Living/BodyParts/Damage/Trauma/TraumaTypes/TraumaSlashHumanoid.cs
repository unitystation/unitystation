...
namespace US13.HealthV2.Living.Damage.Trauma.TraumaTypes
{
	public class TraumaSlashHumanoid : TraumaLogic
	{
			...
			switch (currentStage)
			{
				...
				case 1:
					...
					Chat.AddActionMsgToChat(bodyPart.HealthMaster.gameObject,
						...
						$"{SweetExtensions.ExpensiveName(bodyPart.gameObject)} starts dripping blood lightly as it gets slightly cut open.</color>");
				...
				case 2:
					...
					Chat.AddActionMsgToChat(bodyPart.HealthMaster.gameObject,
						...
						$"{SweetExtensions.ExpensiveName(bodyPart.gameObject)} as a huge gash can be seen open.</color>");
				...
				case 3:
					...
					Chat.AddActionMsgToChat(bodyPart.HealthMaster.gameObject,
						...
						$"{SweetExtensions.ExpensiveName(bodyPart.gameObject)} becomes heavily mangled and torn!</color></size>");
					StopCoroutine(nameof(NaturalHealing));
					break;
				case 4:
		...
		public override string StageDescriptor()
		{
			...
			{
				0 => null,
				1 => $"{SweetExtensions.ExpensiveName(bodyPart.gameObject)} - Rough Abrasion.",
				2 => $"{SweetExtensions.ExpensiveName(bodyPart.gameObject)} - Open Laceration.",
				3 => $"{SweetExtensions.ExpensiveName(bodyPart.gameObject)} - Weeping Avulsion.",
				_ => null
			};
		}
	}
