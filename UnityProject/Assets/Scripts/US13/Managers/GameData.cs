...
namespace US13.Managers
{
	public class GameData : MonoBehaviour, IInitialise
	{
		...
		private async Task Init()
		{
			...
			{		_ = LobbyManager.LobbyManager.Instance.TryAutoLogin();
		...
		public async Task TryLoginThenServerConnectFromJoinArgs()
		{
			...
			LobbyManager.LobbyManager.Instance.JoinServer(JoinArgs.ServerIP, ushort.Parse(JoinArgs.Port));
		...
		private async Task<bool> HubToServerConnect(string ip, ushort port, string uid, string token)
		{
			...
			if (string.IsNullOrEmpty(token) == false)
			{
				...
				if (await LobbyManager.LobbyManager.Instance.TryTokenLogin(token)) // TODO uid not needed anymore?
				{
					LobbyManager.LobbyManager.Instance.JoinServer(ip, port);
					return true;
				}
				Loggy.Warning("Logging in via hub account (via command line args) failed.");
			}

			if (await LobbyManager.LobbyManager.Instance.TryAutoLogin())
			{
				LobbyManager.LobbyManager.Instance.JoinServer(ip, port);
				return true;
			}
