...
namespace US13.Managers
{
	...
	public class AdminCommandsManager : NetworkBehaviour
	{
		...
		public void CmdStartRound(NetworkConnectionToClient sender = null)
		{
			...
			if (GameManager.Instance.CurrentRoundState == RoundState.PreRound && GameManager.Instance.waitForStart)
			{
				if (SubsystemMatrixQueueInit.InitializedAll == false ||
				    SubSceneManager.SubSceneManager.Instance.ServerInitialLoadingComplete == false)
				{
		...
		public void CmdChangeNextMap(string nextMap, NetworkConnectionToClient sender = null)
		{
			...
			if (SubSceneManager.SubSceneManager.AdminForcedMainStation == nextMap) return;

			LogAdminAction(
				$"{player.Username}: Changed the next round map from {SubSceneManager.SubSceneManager.AdminForcedMainStation} to {nextMap}.");

			SubSceneManager.SubSceneManager.AdminForcedMainStation = nextMap;
		}
		...
		public void CmdAdminChangeAwaySite(string nextAwaySite, NetworkConnectionToClient sender = null)
		{
			...
			if (SubSceneManager.SubSceneManager.AdminForcedAwaySite == nextAwaySite) return;

			LogAdminAction(
				$"{player.Username}: Changed the next round away site from {SubSceneManager.SubSceneManager.AdminForcedAwaySite} to {nextAwaySite}.");

			SubSceneManager.SubSceneManager.AdminForcedAwaySite = nextAwaySite;
		}
		...
		public void CmdPlaceTile(GUI_DevTileChanger.PlaceStruct data, NetworkConnectionToClient sender = null)
		{
			...
			var matrixInfo = MatrixManager.MatrixManager.Get(data.matrixId);
			...
			var startLocalPos = MatrixManager.MatrixManager.WorldToLocalInt(data.startWorldPosition, matrixInfo);
			var endLocalPos = MatrixManager.MatrixManager.WorldToLocalInt(data.endWorldPosition, matrixInfo);
		...
		{
			if (MatrixManager.MatrixManager.MainStationMatrix?.Objects == null) yield break;

			var currentIndex = 0;
			var maximumIndexes = 20;
			foreach (var stationObject in
			         MatrixManager.MatrixManager.MainStationMatrix.Objects.GetComponentsInChildren<LightSource>())
			{
		...
		private static IEnumerator SelfPowerEverything()
		{
			...
			var currentIndex = 0;
			var maximumIndexes = 20;
			foreach (var stationObject in MatrixManager.MatrixManager.MainStationMatrix.Objects
		...
		public void DestroyAllLights(NetworkConnectionToClient conn = null)
		{
			...
			Chat.AddSystemMsgToChat(
				...
				MatrixManager.MatrixManager.MainStationMatrix);
		}
