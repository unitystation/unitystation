...
namespace US13.Managers.LobbyManager
{
	...
	public class LobbyManager : SingletonManager<LobbyManager>
	{
		...
		public async Task<bool> TryLogin(string emailAddress, string password, bool autoConnect = false)
		{
			...
			UnityEngine.PlayerPrefs.SetString(PlayerPrefKeys.AccountEmail, emailAddress);
		...
		public async Task<bool> TryTokenLogin(string token)
		{
			...
			var username = UnityEngine.PlayerPrefs.GetString(PlayerPrefKeys.AccountName);
		...
		public async Task<bool> TryAutoLogin()
		{
			...
			if (UnityEngine.PlayerPrefs.GetInt(PlayerPrefKeys.AccountAutoLogin) == 1 && UnityEngine.PlayerPrefs.HasKey(PlayerPrefKeys.AccountToken))
			{
				return await TryTokenLogin(UnityEngine.PlayerPrefs.GetString(PlayerPrefKeys.AccountToken));
			}
		...
		{
			UnityEngine.PlayerPrefs.SetInt(PlayerPrefKeys.AccountAutoLogin, shouldAutoLogin ? 1 : 0);
		}
		...
		public async Task<bool> CreateAccount(string username, string email, string accountIdentifier, string password) // TODO accountIdentifier, order
		{
			...
			if (isSuccess)
			{
				UnityEngine.PlayerPrefs.SetString(PlayerPrefKeys.AccountEmail, email);
		...
		{
			var email = UnityEngine.PlayerPrefs.GetString(PlayerPrefKeys.AccountEmail);
		...
		public void Logout()
		{
			...
			UnityEngine.PlayerPrefs.DeleteKey(PlayerPrefKeys.AccountAutoLogin);
