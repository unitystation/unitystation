...
namespace US13.Player
{
	public class PlayerScript : NetworkBehaviour, IAdminInfo, IPlayerPossessable, IHoverTooltip
	{
		...
		public void OnInteract(TargetedInteraction interaction, Component interactable)
		{
			...
			{
				PlayEffect.SendToAll(interactable.gameObject, "TelekinesisEffect", false, interaction.Performer, null,0);
			}
			...
			if (RNG.Next(0, 100) > ClueHandsImprintInverseChance)
			{
				...
				var slotGlove = EnumerableExt.PickRandom(DynamicItemStorage.GetNamedItemSlots(NamedSlot.hands));
			...
			{
				var slot = EnumerableExt.PickRandom(DynamicItemStorage.GetNamedItemSlots(NamedSlot.uniform));
