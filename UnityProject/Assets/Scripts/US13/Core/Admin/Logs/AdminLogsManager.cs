...
namespace US13.Core.Admin.Logs
{
	public class AdminLogsManager : SingletonManager<AdminLogsManager>
	{
		...
		private static void AddNewLogInternal(List<LogInfo> Info,  LogCategory category, Severity severity = Severity.MISC, bool BubbleUpToChatAdmin = false)
		{
			...
			{
				global::US13.Core.Chat.Chat.AddChatMsgToChatServer( entry.ToStringChat(), ChatChannel.Admin, null);
			}
		...
		{
			if (perp == null) return "[]";
			if (perp.NetId() == global::Util.NetId.Invalid) return perp.ExpensiveName();
		...
		public static uint GetPerpIdFromString(string input)
		{
			...
			if (match.Success)
			{
				...
				{
					return global::Util.NetId.Invalid;
				}
			}
			else
			{
