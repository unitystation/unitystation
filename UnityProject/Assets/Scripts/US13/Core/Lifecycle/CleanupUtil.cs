...
namespace US13.Core.Lifecycle
{
	public static class CleanupUtil
	{
			...
			for (int i = 0, max = listInQuestion.Count; i < max; i++)
			{
				...
				else
				{
					if (verbose)
					{
						try
						{
							Loggy.Info("Name of leaked object : " + target.name, Category.MemoryCleanup);
						}
						catch (Exception ee)
						{
							//do nothing
						...
						try
						{
							Loggy.Info("Typename of leaked object : " + target.GetType().Name, Category.MemoryCleanup);
						}
						catch (Exception ee)
						{
							//do nothing
			...
			for (int i = 0, max = listInQuestion.Count; i < max; i++)
			{
				...
				else
				{
					if (verbose)
					{
						try
						{
							Loggy.Info("Name of leaked object : " + (listInQuestion[i].Target as MonoBehaviour).name, Category.MemoryCleanup);
						}
						catch (Exception ee)
						{
							//do nothing
						...
						try
						{
							Loggy.Info("Typename of leaked object : " + listInQuestion[i].Target.GetType().Name, Category.MemoryCleanup);
						}
						catch (Exception ee)
						{
							//do nothing
			...
			for (int i = 0, max = listInQuestion.Count; i < max; i++)
			{
				...
				else
				{
					if (verbose)
					{
						try
						{
							Loggy.Info("Name of leaked object : " + listInQuestion[i].name, Category.MemoryCleanup);
						}
						catch (Exception ee)
						{
							//do nothing
						...
						try
						{
							Loggy.Info("Typename of leaked object : " + listInQuestion[i].GetType().Name, Category.MemoryCleanup);
						}
						catch (Exception ee)
						{
							//do nothing
			...
			foreach (var keyValuePair in dictInQuestion)
			{
				...
				else
				{
					if (verbose)
					{
						try
						{
							Loggy.Info("Typename of (possibly) leaked object : " + keyValuePair.Key.GetType().Name, Category.MemoryCleanup);
						}
						catch (Exception ee)
						{
							//do nothing
						...
						try
						{
							Loggy.Info("Typename of (possibly) leaked object : " + keyValuePair.Value.GetType().Name, Category.MemoryCleanup);
						}
						catch (Exception ee)
						{
							//do nothing
		...
		public static void EndRoundCleanup()
		{
			...
			foreach (var a in GameObject.FindObjectsOfType<AdminPlayerEntry>(true))
			{
		...
		public static void CleanupInbetweenScenes()
		{
			...
			GameManager.Instance.ResetStaticsOnNewRound();
			CargoManager.Instance.OnRoundRestart();
			LavaLandManager.Instance?.Clean();
		...
		public static void RoundStartCleanup()
		{
			...
			{
				if ((a.iAction is global::US13.Actions.ItemActionButton) && (a.iAction as global::US13.Actions.ItemActionButton == null || (a.iAction as global::US13.Actions.ItemActionButton).CurrentlyOn == null))
				{
					a.iAction = null;
			...
			AdminOverlay.Instance?.Clear();
			TileManager.Instance.DeepCleanupTiles();
			CleanupUtil.RidListOfDeadElements(GameManager.Instance.SpaceBodies);
			global::US13.Actions.UIActionManager.Instance.Clear();//maybe it'l work second time?
			SpriteHandlerManager.Instance?.Clean();
