	...
	public class ParallaxController : MonoBehaviour
	{
		...
		void RealignTiles(bool? UpdateX = null)
		{
			...
			for (int i = 0; i < backgroundTiles.Count; i++)
			{
				for (int j = 0; j < backgroundTiles[0].rows.Count; j++)
				{
					Vector3 newPos = UnityEngine.Camera.main.transform.position;
					newPos.z = zOffset;
					if (UpdateX is true or null)
					{
						newPos.x = UnityEngine.Camera.main.transform.position.x;
					...
					{
						newPos.y = UnityEngine.Camera.main.transform.position.y;
		...
		void MonitorTiles()
		{
			transform.position = UnityEngine.Camera.main.transform.position * moveMultiplier;

			if ((UnityEngine.Camera.main.transform.position.x - backgroundTiles[centerColumn].rows[centerRow].transform.position.x) > tileBounds.x)
			{
				MoveTiles(Vector2.right);
			}

			if ((UnityEngine.Camera.main.transform.position.x
			     - backgroundTiles[centerColumn].rows[centerRow].transform.position.x) < -tileBounds.x)
			{
				MoveTiles(Vector2.left);
			}

			if ((UnityEngine.Camera.main.transform.position.y
