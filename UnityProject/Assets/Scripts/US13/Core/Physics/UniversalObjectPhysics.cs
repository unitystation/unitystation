...
namespace US13.Core.Physics
{
	public partial class UniversalObjectPhysics : NetworkBehaviour, IRegisterTileInitialised
	{
		...
		[HideInInspector] public CheckedComponent<Items.Attributes> attributes = new CheckedComponent<Items.Attributes>();
		...
		private void ProcessThingsToHit()
		{
			...
			foreach (var hit in Hits)
			{
				...
				if (hit.TryGetComponent<LivingHealthMasterBase>(out var livingHealthMasterBase) && isServer)
				{
					...
					global::US13.Core.Chat.Chat.AddThrowHitMsgToChat(gameObject, livingHealthMasterBase.gameObject,
						currentAim);
				}
