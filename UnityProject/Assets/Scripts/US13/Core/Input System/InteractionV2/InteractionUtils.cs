...
namespace US13.Core.Input_System.InteractionV2
{
	/// <summary>
	...
	public static class InteractionUtils
	{
		...
		public static void RequestInteract<T>(T interaction, IBaseInteractable<T> interactableComponent = null)
			where T : Interaction
		{
			if (!Cooldowns.Cooldowns.TryStartClient(interaction, CommonCooldowns.Instance.Interaction)) return;
		...
		public static IBaseInteractable<T> ClientCheckAndTrigger<T>(IEnumerable<IBaseInteractable<T>> interactables, T interaction)
			where T : Interaction
		{
			if (Cooldowns.Cooldowns.IsOnClient(interaction, CommonCooldowns.Instance.Interaction)) return null;
