...
namespace US13.Core.Input_System
{
	...
	public class MouseInputController : MonoBehaviour
	{
		...
		public virtual void Start()
		{
			...
			lightingSystem = UnityEngine.Camera.main.GetComponent<LightingSystem>();
		}

		public void OnEnable()
		{
			if (CustomNetworkManager.IsHeadless) return;
			UpdateManager.Add(CallbackType.LATE_UPDATE,MeLateUpdate);
		}
		...
		{
			if (CustomNetworkManager.IsHeadless) return;
			UpdateManager.Remove(CallbackType.LATE_UPDATE,MeLateUpdate);
		}
		...
		public static void Point()
		{
			...
			if (PlayerManager.LocalPlayerScript.IsGhost ||
			    PlayerManager.LocalPlayerScript.playerHealth.ConsciousState != ConsciousState.CONSCIOUS)
				return;
			if (Cooldowns.Cooldowns.TryStartClient(PlayerManager.LocalPlayerScript, CommonCooldowns.Instance.Interaction) == false)
				return;
