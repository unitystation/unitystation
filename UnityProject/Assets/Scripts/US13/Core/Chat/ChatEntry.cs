...
namespace US13.Core.Chat
{
	...
	public class ChatEntry : MonoBehaviour
	{
		...
		public void SetText(string message, string languageSprite, TMP_FontAsset font)
		{
			...
			message =
				$"<size=+{UnityEngine.PlayerPrefs.GetInt(ChatOptions.FONTSCALE_KEY, ChatOptions.FONTSCALE_KEY_DEFAULT)}>{message}</size>";

			messageText.text = message;
			messageTextDark.text = "<color=#000000>" + ReplaceColour(message);
			var alphaSetting = UnityEngine.PlayerPrefs.GetInt(PlayerPrefKeys.CHAT_BACKGROUND_ALLWAYS_ENABLED, 0) == 1;
		...
		private IEnumerator FadeCooldown(bool Quick = false)
		{
			...
			AnimateFade(ChatUI.ChatContentMinimumAlpha, 3f);
			...
			{
				if (ChatUI.ChatContentMinimumAlpha < 0.01f)
				{
		...
		private void AnimateFade(float toAlpha, float time, bool showBackground = true)
		{
			...
			if (showBackground && (UnityEngine.PlayerPrefs.GetInt(PlayerPrefKeys.CHAT_BACKGROUND_ALLWAYS_ENABLED, 0) == 1 || toAlpha <= 0.01f))
			{
