			...
			Loudness loudness, ChatModifier modifiers, ushort languageId = 0, bool isWhispering = false, string Voice = "", uint originatorNetId = UInt32.MinValue)
		{
			...
			switch (channels)
			{
				case ChatChannel.Syndicate:
					audioSourceParameters.Volume = UnityEngine.PlayerPrefs.GetFloat(PlayerPrefKeys.RadioVolumeKey);
				...
				case ChatChannel.Common:
					if (UnityEngine.PlayerPrefs.GetInt(PlayerPrefKeys.CommonRadioToggleKey) == 1)
					{
						audioSourceParameters.Volume = UnityEngine.PlayerPrefs.GetFloat(PlayerPrefKeys.RadioVolumeKey);
