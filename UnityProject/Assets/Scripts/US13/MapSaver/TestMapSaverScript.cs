...
namespace US13.MapSaver
{
	public class TestMapSaverScript : MonoBehaviour
	{
		...
		public bool SAVETONormal = false;
		...
		public void SaveMatrix()
		{
			...
			//, UseInstance: true
			var map = MapSaver.SaveMatrix(Compact, MetaTileMap, true);
		...
		public void SaveMap()
		{
			...
			var Map = MapSaver.SaveMap(MapMatrices, Compact, "COOL MAP");
			//TODO Add a category for maps and blueprints
			if (SAVETONormal)
			{
		...
		{
			MapMatrices =  FindObjectsOfType<MetaTileMap>().ToList();
			MapMatrices.Reverse();
			SaveMap();
		}
		...
		public void LoadMap()
		{
			...
			MapSaver.MapData mapData = JsonConvert.DeserializeObject<MapSaver.MapData>(json);
