	...
	public class CharacterCustomization : MonoBehaviour
	{
		...
		public void SetUpBodyPart(BodyPart bodyPart, bool instantiateCustomisations = true)
		{
			...
			if (instantiateCustomisations)
			{
				...
				{
					var newSprite = Instantiate<BodyPartCustomisationBase>(bodyPart.LobbyCustomisation, ScrollListBody.transform);
				...
				if (bodyPart.OptionalOrgans.Count > 0)
				{
					...
					option.SetUp(this, (IBodyPartDropDownOrgans)bodyPart, "");
				...
				//Setup sprite//
				//OpenBodySprites
				if (GameObjectExtensions.OrNull<ItemStorage>(bodyPart.OrNull()?.OrganStorage)?.Populater?.DeprecatedContents != null)
				{
				...
				if (GameObjectExtensions.OrNull<ItemStorage>(bodyPart.OrNull()?.OrganStorage)?.Populater?.SlotContents != null)
				{
