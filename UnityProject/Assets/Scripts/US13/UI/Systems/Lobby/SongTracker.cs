...
namespace US13.UI.Systems.Lobby
{
	...
	public class SongTracker : MonoBehaviour
	{
		...
		{
			var toggle = UnityEngine.PlayerPrefs.GetInt(PlayerPrefKeys.MuteMusic);
			if (toggle == 0)
			{
				toggle = 1;
			}
			...
			UnityEngine.PlayerPrefs.SetInt(PlayerPrefKeys.MuteMusic, toggle);
			UnityEngine.PlayerPrefs.Save();
			DetermineMuteState();
		...
		public void OnVolumeSliderChange()
		{
			...
			{
				if (UnityEngine.PlayerPrefs.GetInt(PlayerPrefKeys.MuteMusic) == 0)
                {
					ToggleMusicMute();
				}
