...
{
	public class AdminPlayerChatOld : MonoBehaviour
	{
		...
		{
			var filePath = Path.Combine(ChatLogsFolder, $"{RoundID}_{playerId}.txt");
		...
		protected void ClientGetUnreadAdminPlayerMessages(string playerId)
		{
			...
			AdminCheckMessages.Send(playerId, clientAdminPlayerChatLogs[playerId].Count, GameManager.RoundID);
		}
