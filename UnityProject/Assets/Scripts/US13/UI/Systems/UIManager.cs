...
using Logs;
using Mirror;
using Standard_Assets.HSVPicker.UI;
using UnityEngine;
...
using US13.UI.Systems.MainHUD.UI_Bottom.Construction;
using US13.UI.Systems.MainHUD.UI_Bottom.Ghost;
using US13.UI.Systems.ServerInfoPanel;
using US13.UI.Systems.Tooltips.HoverTooltips;
using US13.Variable_Viewer.BookViewer;
using US13.Variable_Viewer.VariableUI;
using US13.Variable_Viewer.VariableUI.Hierarchy;
using Util;
...
namespace US13.UI.Systems
{
	...
	public static bool UseGamePad = true;
		...
		void IInitialise.Initialise()
		{
			...
			if (UnityEngine.PlayerPrefs.HasKey(PlayerPrefKeys.TTSToggleKey) == false)
			{
				UnityEngine.PlayerPrefs.SetInt(PlayerPrefKeys.TTSToggleKey, 1);
				ttsToggle = true;
				UnityEngine.PlayerPrefs.Save();
			}
			else
			{
				ttsToggle = UnityEngine.PlayerPrefs.GetInt(PlayerPrefKeys.TTSToggleKey) == 1;
			}
		...
		void DetermineInitialTargetFrameRate()
		{
			...
			{
				UnityEngine.PlayerPrefs.SetInt(PlayerPrefKeys.TargetFrameRate, 99);
				UnityEngine.PlayerPrefs.Save();
			}

			var targetFrameRate = UnityEngine.PlayerPrefs.GetInt(PlayerPrefKeys.TargetFrameRate);
			targetFrameRate = Mathf.Clamp(targetFrameRate, 30, 144);
			UnityEngine.PlayerPrefs.SetInt(PlayerPrefKeys.TargetFrameRate, targetFrameRate);
			UnityEngine.PlayerPrefs.Save();
		...
		public static void ToggleTTS(bool activeState)
		{
			...
			UnityEngine.PlayerPrefs.SetInt(PlayerPrefKeys.TTSToggleKey, activeState ? 1 : 0);
			UnityEngine.PlayerPrefs.Save();
		}
