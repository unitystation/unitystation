...
namespace US13.UI.Systems.MainHUD.UI_Bottom
{
	public class ControlAction : MonoBehaviour
	{
		...
		{
			return 1 == UnityEngine.PlayerPrefs.GetInt(PlayerPrefKeys.ThrowHoldPreference, 0);
		}
		...
		public static void SetPreferenceThrowHoldPreference(bool preference)
		{
			if (preference)
			{
				UnityEngine.PlayerPrefs.SetInt(PlayerPrefKeys.ThrowHoldPreference, 1);
				ThrowHold = true;
			}
			else
			{
				UnityEngine.PlayerPrefs.SetInt(PlayerPrefKeys.ThrowHoldPreference, 0);
				ThrowHold = false;
			}
