	...
	public class ChatBubble : MonoBehaviour, IDisposable
	{
		...
		private IEnumerator ShowDialogue(CancellationToken cancelToken)
		{
			...
			while (msgQueue.Count > 0 || ActiveBubbles.Count > 0)
			{
				...
				var NumberMaxShow = UnityEngine.PlayerPrefs.GetInt(PlayerPrefKeys.ChatBubbleNumber, 2);
		...
		{
			UnityEngine.PlayerPrefs.SetInt(PlayerPrefKeys.ChatBubbleNumber, NummberBubbles);
		}

		public static int GetPreferenceNummberBubbles()
		{
			return UnityEngine.PlayerPrefs.GetInt(PlayerPrefKeys.ChatBubbleNumber, 3);
		}
	}
}
