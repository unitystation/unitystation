...
namespace US13.UI.Core.ChatBubble
{
	public class SubChatBubble : MonoBehaviour
	{
		...
		IEnumerator ShowDialogue(CancellationToken cancelToken, ChatBubble.BubbleMsg msg)
		{
			...
			msg.instantText = UnityEngine.PlayerPrefs.GetInt(PlayerPrefKeys.ChatBubbleInstant) == 1;

			bubbleSize = UnityEngine.PlayerPrefs.GetFloat(PlayerPrefKeys.ChatBubbleSize);
			...
			else
			{
				...
				msg.characterPopInSpeed = UnityEngine.PlayerPrefs.GetFloat(PlayerPrefKeys.ChatBubblePopInSpeed);
		...
		private void CheckShowTime(ChatBubble.BubbleMsg msg)
		{
			...
			var additionalTime = UnityEngine.PlayerPrefs.GetFloat(PlayerPrefKeys.ChatBubbleAdditionalTime);
		...
		private void ShowCharacter(ChatBubble.BubbleMsg msg)
		{
			while (msg.elapsedTime > 0f)
			{
				...
				if ((msg.modifier & ChatModifier.Clown) == ChatModifier.Clown &&
				    UnityEngine.PlayerPrefs.GetInt(PlayerPrefKeys.ChatBubbleClownColour) == 1)
				{
