...
using US13.UI.Systems.ItemListPanel;
using US13.UI.Systems.MainHUD.TabDropdown;
...
namespace US13.UI.Core
{
	public class ControlTabs : MonoBehaviour
	{
		...
		public static void ShowTab(NetTabType type, GameObject tabProvider, ElementValue[] elementValues)
		{
			...
			if (Instance.OpenedNetTabs.ContainsKey(tabDescriptor) == false)
			{
		...
		public static void UpdateTab(NetTabType type, GameObject tabProvider, ElementValue[] values, bool touched = false)
		{
			...
			if (openedNetTabs.ContainsKey(lookupTab) == false) return;
		...
		private void ShowFinger(GameObject tab, GameObject element)
		{
			...
			fingerObject.transform.position = element.transform.position + new Vector3(Random.Range(-5f, 5f), Random.Range(-5f, 5f), 0);
		...
		IEnumerator AnimTabRoll(bool wait, bool? shouldOpen)
		{
			...
			else if (shouldOpen == false && rolledOut == false)
			{
				yield break;
			}
			if (preventRoll == false)
			{
				preventRoll = true;
			}
			else
