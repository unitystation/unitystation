...
namespace US13.UI.Core.OptionsMenu.ThemeOptions
{
    ...
    public class ThemeManager : MonoBehaviour, IInitialise
    {
        ...
        void IInitialise.Initialise()
        {
	        ...
	        if (UnityEngine.PlayerPrefs.HasKey(PlayerPrefKeys.HighlightChat) == false)
	        {
		        UnityEngine.PlayerPrefs.SetInt(PlayerPrefKeys.HighlightChat, 1);
	        }

	        if (UnityEngine.PlayerPrefs.HasKey(PlayerPrefKeys.MentionSound) == false)
	        {
		        UnityEngine.PlayerPrefs.SetInt(PlayerPrefKeys.MentionSound, 1);
	        }

	        if (UnityEngine.PlayerPrefs.HasKey(PlayerPrefKeys.MentionSoundIndex) == false)
	        {
		        UnityEngine.PlayerPrefs.SetInt(PlayerPrefKeys.MentionSoundIndex, 0);
	        }

	        ChatHighlight = UnityEngine.PlayerPrefs.GetInt(PlayerPrefKeys.HighlightChat) == 1;
	        MentionSound = UnityEngine.PlayerPrefs.GetInt(PlayerPrefKeys.MentionSound) == 1;
        ...
        public void ChatHighlightToggle(bool toggle)
        {
	        ...
	        UnityEngine.PlayerPrefs.SetInt(PlayerPrefKeys.HighlightChat, toggle ? 1 : 0);
        ...
        {
	        MentionSound = toggle;
	        UnityEngine.PlayerPrefs.SetInt(PlayerPrefKeys.MentionSound, toggle ? 1 : 0);
	        UnityEngine.PlayerPrefs.Save();
        }
...
", userPersistent: true);
        ...
        void LoadUserPreferences()
        {
            ...
            if (!UnityEngine.PlayerPrefs.HasKey(PlayerPrefKeys.ChatBubbleThemeKey))
            {
                UnityEngine.PlayerPrefs.SetString(PlayerPrefKeys.ChatBubbleThemeKey, "Default");
                UnityEngine.PlayerPrefs.Save();
            }

            //Set the chosen theme:
            //FIXME put this is some kind of loop later on when we have more themetypes
            SetPreferredTheme(ThemeType.ChatBubbles, UnityEngine.PlayerPrefs.GetString(PlayerPrefKeys.ChatBubbleThemeKey));
        }
        ...
        void SavePlayerPrefs(ThemeConfig config)
        {
            ...
            UnityEngine.PlayerPrefs.SetString(prefKey, config.themeName);
            UnityEngine.PlayerPrefs.Save();
