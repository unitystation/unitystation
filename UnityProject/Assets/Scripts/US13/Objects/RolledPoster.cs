...
namespace US13.Objects
{
	public class RolledPoster : NetworkBehaviour, ICheckedInteractable<PositionalHandApply>
	{
	    ...
	    [FormerlySerializedAs("posterVariant")]	public Posters.Posters InitialPoster;
		[SyncVar(hook = nameof(SyncPosterType))]
		[PlayModeOnly, NonSerialized] public Posters.Posters posterVariant;
		...
		public void SyncPosterType(Posters.Posters oldP, Posters.Posters p)
		{
			posterVariant = p;
			...
			if (posterVariant == Posters.Posters.RandomContraband)
			{
			...
			else if (posterVariant == Posters.Posters.RandomOfficial)
			{
