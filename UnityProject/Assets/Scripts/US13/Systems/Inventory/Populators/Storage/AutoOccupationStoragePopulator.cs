...
namespace US13.Systems.Inventory.Populators.Storage
{
	...
	public class AutoOccupationStoragePopulator : ItemStoragePopulator
	{
		...
		public override void PopulateItemStorage(IStoreThings toPopulate,MonoBehaviour component, PopulationContext context, SpawnInfo info)
		{
			...
			occupation.InventoryPopulator.PopulateItemStorage(toPopulate,component , context, info);
			if (StandardPopulator != null)
			{
				Loggy.Trace().Format("Populating item storage using standard populator",
					Category.EntitySpawn);
				StandardPopulator.PopulateItemStorage(toPopulate,component, context, info);
			}
		}
	}
}
