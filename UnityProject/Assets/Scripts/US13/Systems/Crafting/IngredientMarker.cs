...
namespace US13.Systems.Crafting
{
	...
	public class IngredientMarker : MonoBehaviour, ICheckedInteractable<InventoryApply>
	{
		...
		public void ServerPerformInteraction(InventoryApply interaction)
		{
			...
			if (cut)
			{
				_ = Inventory.Inventory.ServerDespawn(interaction.TargetObject);
				_ = Inventory.Inventory.ServerDespawn(interaction.UsedObject);
				...
				if (spwn.Successful)
				{
					Inventory.Inventory.ServerAdd(spwn.GameObject, interaction.TargetSlot);
				}
			}
			else if (cut2)
			{
