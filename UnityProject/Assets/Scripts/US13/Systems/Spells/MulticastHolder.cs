...
public class MulticastHolder : Spell
{
	...
	public override bool CastSpellServer(PlayerInfo caster, Vector3 clickPosition, BodyPartType targetZone)
	{
		...
		ProjectileManager.InstantiateAndShoot(projectilePrefab, castVector, PlayerManager.LocalPlayerScript.GameObject,
			null, targetZone);
		Used++;
	...
	public void UpdateMe()
	{
		...
		{
			PlayerManager.LocalPlayerScript.PlayerNetworkActions.CmdRequestSpell(SpellData.Index, TransformState.HiddenPos,UIManager.DamageZone);
			Active = false;
			Used = 0;
			return;
		}
		...
		{
			PlayerManager.LocalPlayerScript.PlayerNetworkActions.CmdRequestSpell(SpellData.Index, CommonInput.CashedMouseWorldPosition, UIManager.DamageZone);
