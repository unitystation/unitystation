	...
	public override bool CastSpellServer(PlayerInfo caster)
	{
		...
		if (caster.GameObject.TryGetComponent<Rotatable>(out var directional))
		{
			if (directional.CurrentDirection == OrientationEnum.Down_By180 || directional.CurrentDirection == OrientationEnum.Up_By0)
			{
				...
				var Local = (directional.WorldDirection.To3() + caster.Script.WorldPos + Vector3.left).ToLocal(Matrix).RoundToInt();
				var Local2 = (directional.WorldDirection.To3() + caster.Script.WorldPos + Vector3.right).ToLocal(Matrix).RoundToInt();
				var Local3 = (directional.WorldDirection.To3() + caster.Script.WorldPos).ToLocal(Matrix).RoundToInt();
			...
			{
				var Local = (directional.WorldDirection.To3() + caster.Script.WorldPos + Vector3.up).ToLocal(Matrix).RoundToInt();
				var Local2 = (directional.WorldDirection.To3() + caster.Script.WorldPos + Vector3.down).ToLocal(Matrix).RoundToInt();
				var Local3 = (directional.WorldDirection.To3() + caster.Script.WorldPos).ToLocal(Matrix).RoundToInt();
				if (Matrix.MetaTileMap.HasTile(Local, LayerType.Walls) == false)
				{
					obstructions[0] = Local;
