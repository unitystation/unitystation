	...
	public class ItemPlinth : NetworkBehaviour, ICheckedInteractable<PositionalHandApply>, IOnHitDetect
	{
		...
		public void ServerPerformInteraction(PositionalHandApply interaction)
		{
			if (HasItem)
			{
				...
				Inventory.Inventory.ServerTransfer(itemStorage.GetIndexedItemSlot(0), interaction.HandSlot);
				DisplayedItem = null;
				OnItemChange?.Invoke();
			}
			else
			{
				...
				else
				{
					...
					Inventory.Inventory.ServerTransfer(interaction.HandSlot, itemStorage.GetNextEmptySlot());
					OnItemChange?.Invoke();
				}
			}
