...
using US13.Items.Storage.VirtualStorage;
using US13.Managers;
...
namespace US13.Systems.Research.Objects
{
	public class ResearchServer : NetworkBehaviour, IMultitoolMasterable, IServerSpawn, IServerDespawn
	{
		...
		private void RemoveHardDisk()
		{
			...
			{
				Inventory.Inventory.ServerDrop(disk.gameObject.PickupableOrNull().ItemSlot);
				Techweb = null;
				Techweb.TechWebDesignUpdateEvent?.Invoke(0, null);
			}
		}
		...
		private void AddHardDisk(ItemSlot disk)
		{
			...
			if (Inventory.Inventory.ServerTransfer(disk, diskStorage.GetNextFreeIndexedSlot()))
			{
