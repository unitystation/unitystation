	...
	public class ChangelingAbility : NetworkBehaviour, IActionGUI
	{
		...
		public void CallActionServerToggle(PlayerInfo sentByPlayer, bool toggle)
		{
			...
			if (validateAbility && AbilityData is ChangelingToggleAbility toggleAbility)
			{
				...
				{
					Loggy.Error($"[ChangelingAbility/CallActionServerToggle] Failed to use ability {AbilityData.Name}. Toggle to was {toggle}." +
		...
		private bool CastAbilityServerWithParam(PlayerInfo sentByPlayer, List<string> param)
		{
			...
			if (AbilityData is ChangelingParamAbility abilityParam)
			{
				...
				catch (Exception ex)
				{
					...
					Loggy.Error($"[ChangelingAbility/CastAbilityServerWithParam] Failed to use ability {AbilityData.Name}. Params was: {errorStringBuilder}." +
					$"Sent by player '{sentByPlayer.Username}' {ex}");
					return false;
				}
				return true;
		...
		private bool CastAbilityServer(PlayerInfo sentByPlayer, Vector3 clickPosition)
		{
			...
			{
				Loggy.Error($"[ChangelingAbility/CastAbilityServer] Failed to use ability {AbilityData.Name}. Click position was {clickPosition}." +
				$"Sent by player '{sentByPlayer.Username}' {ex}");
				return false;
			}
			return true;
		...
		private bool ValidateAbilityClient()
		{
			...
			bool isRecharging = Cooldowns.IsOnClient((PlayerScript)changelingMain.ChangelingMind.Body, AbilityData) ||
			Cooldowns.IsOnServer((PlayerScript)changelingMain.ChangelingMind.Body, AbilityData);
			if (isRecharging)
			{
				Chat.AddExamineMsg(changelingMain.gameObject, $"Ability {AbilityData.Name} is recharging!");
				return false;
