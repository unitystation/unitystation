...
public abstract class InputTrigger : NetworkBehaviour
{
    ...
    public bool canUse(GameObject originator, string hand, Vector3 position, bool allowSoftCrit){
        var playerScript = originator.GetComponent<PlayerScript>();

        if ((playerScript.canNotInteract() && (allowSoftCrit && !playerScript.playerHealth.IsSoftCrit)))
        {
            return false;
        }
