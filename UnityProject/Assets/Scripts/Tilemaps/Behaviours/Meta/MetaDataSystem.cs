...
/// <summary>
/// Subsystem behavior which manages updating the MetaDataNodes and simulation
/// that affects them for a given matrix.
/// </summary>
public class MetaDataSystem : SubsystemBehaviour {
  ...
  /// These are regularly checked for tiles from other matrices, i.e. they build
  /// the bridge to other matrices
  /// </summary>
  ...
  public override void Initialize() {
    ...
    if (MatrixManager.IsInitialized) {
      ...
      Dsw.Start();
      matrix.MetaTileMap.InitialiseUnderFloorUtilities(
          CustomNetworkManager.IsServer);
      Dsw.Stop();
      Logger.Log(
          $"Initialise {gameObject.name} Utilities (Power cables, Atmos pipes): " +
              Dsw.ElapsedMilliseconds + " ms",
          Category.Matrix);
    }

    sw.Stop();

    Logger.Log($"{gameObject.name} MetaData init: " + sw.ElapsedMilliseconds +
                   " ms",
               Category.Matrix);
  }
  ...
  public override void UpdateAt(Vector3Int localPosition) {
    ...
    // If the node is atmos passable (i.e. space or room), we need to setup its
    // neighbors again, otherwise it's occupied and does need a neighbor check
    if (metaTileMap.IsAtmosPassableAt(localPosition, true)) {
      node.ClearNeighbors();
      node.Type = metaTileMap.IsSpaceAt(localPosition, true) ? NodeType.Space
                                                             : NodeType.Room;
      ...
      if (matrix.GetFirst<RegisterDoor>(localPosition, true)) {
        ...
        // TODO hard coded these values, might be better to put them in register
        // door?
    ...
    if (node.IsIsolatedNode == false && node.Type != NodeType.Space &&
        matrix.MetaTileMap.GetTile(localPosition, true) is BasicTile tile &&
        tile != null) {
  ...
  private void LocateRooms() {
    ...
    Logger.LogFormat("Created rooms in {0}ms", Category.TileMaps,
  ...
  private void FindRoomAt(Vector3Int position) {
    if (!metaTileMap.IsAtmosPassableAt(position, true)) {
      ...
      if (matrix.GetFirst<RegisterDoor>(position, true)) {
        ...
        node.HeatCapacity = 10000f;
      } else if (matrix.MetaTileMap.GetTile(position, true) is BasicTile tile &&
                 tile != null) {
        node.HeatCapacity = tile.HeatCapacity;
        node.ThermalConductivity = tile.ThermalConductivity;
      }
      ...
      SetupNeighbors(node);
    } else if (!metaTileMap.IsSpaceAt(position, true) &&
               !metaDataLayer.IsRoomAt(position) &&
               !metaDataLayer.IsSpaceAt(position)) {
      CreateRoom(position);
    }
  }
      ...
      if (freePositions.TryDequeue(out Vector3Int position)) {
        ...
        for (var i = 0; i < neighbors.Length; i++) {
          ...
          if (metaTileMap.IsSpaceAt(neighbor, true)) {
            Vector3Int worldPosition = MatrixManager.LocalToWorldInt(
                neighbor, MatrixManager.Get(matrix.Id));

            // If matrix manager says, the neighboring positions is space, the
            // whole room is connected to space. Otherwise there is another
            // matrix, blocking off the connection to space.
            if (MatrixManager.IsSpaceAt(worldPosition, true,
                                        matrix.MatrixInfo)) {
              isSpace = true;
            }
          } else if (metaTileMap.IsAtmosPassableAt(position, neighbor, true)) {
            // if neighbor position is not yet a room in the meta data layer and
            // not in the room positions list, add it to the positions that need
            // be checked
            if (!roomPositions.Contains(neighbor) &&
                !metaDataLayer.IsRoomAt(neighbor)) {
  ...
  private void AssignType(IEnumerable<Vector3Int> positions,
                          NodeType nodeType) {
  ...
  private void SetupNeighbors(MetaDataNode node) {
    ...
    foreach (Vector3Int dir in MetaUtils.Directions) {
      ...
      if (metaTileMap.IsSpaceAt(neighbor, true)) {
        // // if current node is a room, but the neighboring is a space tile,
        // this node needs to be checked regularly for changes by other matrices
        // if (node.IsRoom && !externalNodes.ContainsKey(node) &&
        // metaTileMap.IsSpaceAt(node.Position, true) == false)
        // {
        // 	externalNodes[node] = node;
        // }

        // If the node is not space, check other matrices if it has a tile next
        // to this node.
        if (!node.IsSpace) {
          Vector3 neighborWorldPosition = MatrixManager.LocalToWorldInt(
              neighbor, MatrixManager.Get(matrix.Id));

          // if matrixManager says, it's not space at the neighboring position,
          // there must be a matrix with a non-space tile
          if (!MatrixManager.IsSpaceAt(neighborWorldPosition.RoundToInt(), true,
                                       matrix.MatrixInfo)) {
            MatrixInfo matrixInfo =
                MatrixManager.AtPoint(neighborWorldPosition.RoundToInt(), true);
            ...
            if (matrixInfo != null && matrixInfo.MetaTileMap != metaTileMap) {
              ...
              Vector3Int neighborlocalPosition = MatrixManager.WorldToLocalInt(
                  neighborWorldPosition, matrixInfo);

              // add node of other matrix to the neighbors of the current node
              node.AddNeighbor(
                  matrixInfo.MetaDataLayer.Get(neighborlocalPosition), dir);

              // skip other checks for neighboring tile on local tilemap, to
              // prevent the space tile to be added as a neighbor
              continue;
            }
          }
        }
