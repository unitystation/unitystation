...
public class MetaDataNode : IGasMixContainer
{
	...
	/// </summary>
	public List<ElectricalMetaData> ElectricalData = new();

	/// <summary>
	///     This contains all the pipe data needed On the tile
	/// </summary>
	public List<PipeNode> PipeData = new();

	/// <summary>
	///     This contains all the disposal pipe data needed On the tile
	/// </summary>
	public List<DisposalPipeNode> DisposalPipeData = new();
	...
	private readonly Dictionary<LayerType, float> damageInfo = new();

	//Which overlays this node has on
	public HashSet<GasSO> GasOverlayData { get; } = new();

	public AppliedDetails AppliedDetails = new();
	...
	private void TryCreateDamageInfo(LayerType layerType)
	{
	...
	private readonly List<MetaDataNode> neighborList;

	public ReactionManager ReactionManager { get; }
	...
	public MetaDataNode(Vector3Int localPosition, ReactionManager reactionManager, Matrix matrix,
		MetaDataSystem InMetaDataSystem)
	{
		...
		for (var i = 0; i < neighborList.Capacity; i++) neighborList.Add(null);
		...
		lock (neighborList)
		{
			foreach (var neighbor in neighborList)
			{
				...
				var equalise = neighbor.IsOccupied == false && neighbor.IsIsolatedNode == false
				                                            && IsOccupiedBlocked(neighbor) == false &&
				                                            neighbor.IsOccupiedBlocked(this) == false;
	...
	public void ClearNeighbors()
	{
		lock (neighborList)
		{
			for (var i = 0; i < 4; i++) neighborList[i] = null;
		}
	}
	...
	public void AddNeighbor(MetaDataNode neighbor, Vector3Int direction)
	{
		...
		lock (neighborList)
		{
			...
			{
				if (MetaUtils.Directions[i] != direction) continue;
			...
			Logger.LogErrorFormat("Failed adding neighbor {0} to node {1} at direction {2}", Category.Matrix,
	...
	private bool IsOccupiedBlocked(MetaDataNode neighbourNode)
	{
