...
namespace Systems.Atmospherics {
  public class AtmosSystem : SubsystemBehaviour {
    ...
    private Dictionary<int, RoomGasSetter> toSet =
        new Dictionary<int, RoomGasSetter>();
    ...
    public override void Initialize() {
      ...
      foreach (Vector3Int position in bounds.allPositionsWithin()) {
        ...
        var topTile =
            metaTileMap.GetTile(position, LayerTypeSelection.Effects |
                                              LayerTypeSelection.Underfloor);
...
#region Room Gas Setter
    ...
    public void AddToList(int room, RoomGasSetter toAdd) {
      if (toSet.ContainsKey(room)) {
        Logger.LogError(
            $"Room number: {room} was already added, cant add {toAdd.gameObject.ExpensiveName()}");
        return;
      }
    ...
    /// <summary>
    /// Use to set a rooms gas after the round has started, otherwise do it in
    /// the initialisation
    /// </summary>
