...
/// Component which allows all the tiles of a matrix to be interacted with using
/// their configured TileInteractions.
///
/// Also provides various utility methods for working with tiles.
/// </summary>
public class InteractableTiles : MonoBehaviour,
                                 IClientInteractable<PositionalHandApply>,
                                 IExaminable,
  ...
  [Tooltip(
      "Action message to performer when they begin cable cutting interaction.")]
  [SerializeField]
  ...
  private void Start() {
    ...
    // Register message handler for CableCuttingMessage here because
    // CableCuttingWindow prefab won't be loaded on server so registration
    // cannot be inside Start or Awake method inside CableCuttingWindow.
    // ReplaceHandler does the same thing as RegisterHandler, except
    // RegisterHandler warns about conflicting ID types. See Mirror's
    // documentation or Mirror's implementation of these methods in
    // NetworkServer.cs.
    // TODO: This is somehow called multiple times. Not sure why. Figure out if
    // it's an issue and document why this
    //       happens.
    NetworkServer.ReplaceHandler<CableCuttingWindow.CableCuttingMessage>(
        ServerPerformCableCuttingInteraction);
  }

  /// <summary>
  /// Gets the interactable tiles for the matrix at the indicated world
  /// position. Never returns null - always returns the interactable tiles for
  ...
  /// <summary>
  /// Gets the interactable tiles for the matrix at the indicated world
  /// position. Unless there's only space! in that case tries to fetch an
  /// adjacent matrix, in the case of none, it returns the space matrix
  /// </summary>
  public static Matrix TryGetNonSpaceMatrix(Vector3Int worldPos,
                                            bool isServer) {
  ...
  /// <summary>
  /// Gets the LayerTile of the tile at the indicated position, null if no tile
  /// there (open space).
  /// </summary>
  /// <param name="worldPos"></param>
  /// <returns></returns>
  public LayerTile LayerTileAt(Vector2 worldPos,
    ...
    Vector3Int pos =
        objectLayer.transform.InverseTransformPoint(worldPos).RoundToInt();
  ...
  public bool Interact(PositionalHandApply interaction) {
    ...
    if (tile is BasicTile basicTile) {
      ...
      if (basicTile.LayerType == LayerType.Underfloor) {
        ...
        foreach (BasicTile underFloorTile in matrix.MetaTileMap
                     .GetAllTilesByType<BasicTile>(localPosition,
          ...
          if (underFloorTile is ElectricalCableTile &&
              Validations.HasItemTrait(
                  PlayerManager.LocalPlayerScript.DynamicItemStorage
                      .GetActiveHandSlot()
                      .ItemObject,
          ...
          // at the target.
            ...
            var underFloorApply = new TileApply(
                interaction.Performer, interaction.UsedObject,
                interaction.Intent, (Vector2Int)localPosition, this,
                underFloorTile, interaction.HandSlot, interaction.TargetVector);
        ...
        var tileApply = new TileApply(
            interaction.Performer, interaction.UsedObject, interaction.Intent,
            (Vector2Int)localPosition, this, basicTile, interaction.HandSlot,
  ...
  private void EnableCableCuttingWindow() {
    ...
    // else add component, init and enable window
      ...
      LoadCableCuttingWindow window =
          (LoadCableCuttingWindow)gameObject.AddComponent(
              typeof(LoadCableCuttingWindow));
  ...
  /// </summary>
  private void ServerPerformCableCuttingInteraction(
      NetworkConnection conn, CableCuttingWindow.CableCuttingMessage message) {
    // get object at target position
    GameObject hit =
        MouseUtils.GetOrderedObjectsAtPoint(message.targetWorldPosition)
            .FirstOrDefault();
    // get matrix
    ...
    // convert world position to cell position and set Z value to Z value from
    // message
    Vector3Int targetCellPosition =
        matrix.MetaTileMap.WorldToCell(message.targetWorldPosition);

    // get electical tile from targetCellPosition
    ElectricalCableTile electricalCable = TileManager.GetTile(
        message.TileType, message.Name) as ElectricalCableTile;
    ...
    string othersMessage =
        Chat.ReplacePerformer(othersStartActionMessage, message.performer);
    Chat.AddActionMsgToChat(message.performer, performerStartActionMessage,
                            othersMessage);
    ...
    foreach (var ElectricalData in metaDataNode.ElectricalData) {
      ...
      // Electrocute the performer. If shock is painful enough, cancel the
      // interaction.
      ElectricityFunctions.WorkOutActualNumbers(ElectricalData.InData);
      float voltage = ElectricalData.InData.Data.ActualVoltage;
      var electrocution =
          new Electrocution(voltage, message.targetWorldPosition, "cable");
      var performerLHB =
          message.performer.GetComponent<LivingHealthMasterBase>();
      var severity = performerLHB.Electrocute(electrocution);
      if (severity > LivingShockResponse.Mild)
        return;

      ElectricalData.InData.DestroyThisPlease();
      Spawn.ServerPrefab(electricalCable.SpawnOnDeconstruct,
                         message.targetWorldPosition,
  ...
  private bool TryInteractWithTile(TileApply interaction) {
    // Iterate over the interactions for the given tile until a valid one is
    // found.
    foreach (var tileInteraction in interaction.BasicTile.TileInteractions) {
      ...
      if (tileInteraction.WillInteract(interaction, NetworkSide.Client) &&
          Cooldowns.TryStartClient(interaction,
                                   CommonCooldowns.Instance.Interaction)) {
        // request the tile interaction with this index
        RequestInteractMessage.SendTileApply(interaction, this,
                                             tileInteraction);
        return true;
      }
    }
  ...
  // for internal IF2 usages only, does server side logic for processing
  // tileapply
  public void ServerProcessInteraction(GameObject performer,
                                       Vector2 targetVector,
                                       GameObject processorObj,
                                       ItemSlot usedSlot, GameObject usedObject,
                                       Intent intent,
                                       TileApply.ApplyType applyType) {
    ...
    if (tile is BasicTile basicTile) {
      ...
      if (basicTile.LayerType == LayerType.Underfloor) {
        foreach (var underFloorTile in matrix.MetaTileMap
                     .GetAllTilesByType<BasicTile>(localPosition,
                                                   LayerType.Underfloor)) {
          var underFloorApply = new TileApply(
              performer, usedObject, intent, (Vector2Int)localPosition, this,
              underFloorTile, usedSlot, targetVector, applyType);

          foreach (var tileInteraction in underFloorTile.TileInteractions) {
            if (tileInteraction == null)
              continue;
            if (tileInteraction.WillInteract(underFloorApply,
                                             NetworkSide.Server)) {
        ...
        var tileApply = new TileApply(
            performer, usedObject, intent, (Vector2Int)localPosition, this,
            basicTile, usedSlot, targetVector, applyType);
  ...
  private void PerformTileInteract(TileApply interaction) {
    foreach (var tileInteraction in interaction.BasicTile.TileInteractions) {
      ...
      if (tileInteraction.WillInteract(interaction, NetworkSide.Server)) {
        ...
        if (Cooldowns.TryStartServer(interaction,
                                     CommonCooldowns.Instance.Interaction)) {
        ...
        // interaction should've triggered and did or we hit a cooldown, so
  ...
  public bool Interact(MouseDrop interaction) {
    ...
    if (tile is BasicTile basicTile) {
      var tileApply = new TileApply(
          interaction.Performer, interaction.UsedObject, interaction.Intent,
          (Vector2Int)WorldToCell(interaction.ShadowWorldLocation), this,
          basicTile, null,
          -((Vector2)interaction.Performer.transform.position -
            interaction.ShadowWorldLocation),
          TileApply.ApplyType.MouseDrop);
      var tileMouseDrop = new TileMouseDrop(
          interaction.Performer, interaction.UsedObject, interaction.Intent,
          (Vector2Int)WorldToCell(interaction.ShadowWorldLocation), this,
          basicTile,
          -((Vector2)interaction.Performer.transform.position -
  ...
  public void OnHoverStart() { OnHover(); }
  ...
  public void OnHover() {
    ...
    if (wallMount) {
      ...
      Vector3Int PlaceDirection =
          PlayerManager.LocalPlayerScript.WorldPos - tilePos;
      bool isWallBlocked = false;
      if (PlaceDirection.x != 0 &&
          !MatrixManager.IsWallAt(
              tilePos + new Vector3Int(PlaceDirection.x > 0 ? 1 : -1, 0, 0),
              true)) {
        if (PlaceDirection.x > 0) {
        ...
        if (PlaceDirection.y != 0 &&
            !MatrixManager.IsWallAt(
                tilePos + new Vector3Int(0, PlaceDirection.y > 0 ? 1 : -1, 0),
                true)) {
          if (PlaceDirection.y > 0) {
      ...
      if (wallMount.IsWallProtrusion) // for light bulbs, tubes, cameras, etc.
                                      // move the sprite towards the floor
      {
        if (orientation == OrientationEnum.Right) {
          spritePos.x += 0.5f;
          Highlight.instance.spriteRenderer.transform.rotation =
  ...
  WallMountHandApplySpawn CheckWallMountOverlay() {
    ...
    var itemSlot = PlayerManager.LocalPlayerScript?.DynamicItemStorage
                       ?.GetActiveHandSlot();
  ...
  public bool TryMine(Vector3 worldPosition) {
    ...
    tileChangeManager.MetaTileMap.RemoveOverlaysOfType(
        cellPos, LayerType.Effects, OverlayType.Mining);
  ...
  public void CreateAnimatedTile(Vector3 worldPosition,
                                 AnimatedOverlayTile animatedTile,
                                 float animationTime) {
    Vector3Int cellPos = metaTileMap.WorldToCell(worldPosition);

    StartCoroutine(
        EnableAnimationCoroutine(cellPos, animatedTile, animationTime));
  }

  private IEnumerator EnableAnimationCoroutine(Vector3Int cellPos,
                                               AnimatedOverlayTile animatedTile,
                                               float animationTime) {
    ...
    tileChangeManager.MetaTileMap.RemoveOverlaysOfType(
        cellPos, LayerType.Effects, animatedTile.OverlayType);
  }
}
