...
public class MetaTileMap : MonoBehaviour {
  ...
  private readonly Dictionary<Layer, Dictionary<Vector3Int, TileLocation>>
      PresentTiles =
          new Dictionary<Layer, Dictionary<Vector3Int, TileLocation>>();

  public Dictionary<Layer, Dictionary<Vector3Int, TileLocation>>
      PresentTilesNeedsLock => PresentTiles;

  private readonly Dictionary<Layer, Dictionary<Vector3Int, List<TileLocation>>>
      MultilayerPresentTiles =
          new Dictionary<Layer, Dictionary<Vector3Int, List<TileLocation>>>();

  public Dictionary<Layer, Dictionary<Vector3Int, List<TileLocation>>>
      MultilayerPresentTilesNeedsLock => MultilayerPresentTiles;

  private Dictionary<Layer, BetterBoundsInt> BoundLocations =
      new Dictionary<Layer, BetterBoundsInt>();

  /// <summary>
  /// Use this dictionary only if performance isn't critical, otherwise try
  /// using arrays below
  /// </summary>This
  public Dictionary<LayerType, Layer> Layers { get; private set; }

  private static readonly Stack<TileLocation> PooledTileLocation =
      new Stack<TileLocation>();

  private static TileLocation GetPooledTile() {
    lock (PooledTileLocation) {
      return PooledTileLocation.Count > 0 ? PooledTileLocation.Pop()
                                          : new TileLocation();
    }
  }
  ...
  private void OnEnable() {
    ...
    foreach (Layer layer in GetComponentsInChildren<Layer>(true)) {
      ...
      if (type != LayerType.Effects && type != LayerType.None) {
      ...
      var InBoundLocations =
          new BetterBoundsInt() { Maximum = Vector3Int.one,
      ...
      if (layer.LayerType != LayerType.Underfloor) {
        ...
        for (int n = bounds.xMin; n < bounds.xMax; n++) {
          for (int p = bounds.yMin; p < bounds.yMax; p++) {
            ...
            if (getTile != null) {
              ...
              Tile.transformMatrix =
                  layer.Tilemap.GetTransformMatrix(localPlace);
        ...
        lock (PresentTiles) { PresentTiles[layer] = ToInsertDictionary; }
      }
    ...
    LayersKeys = layersKeys.ToArray(); layersValues.Sort(
        (layerOne, layerTwo) => layerOne.LayerType.GetOrder().CompareTo(
            layerTwo.LayerType.GetOrder()));
    ...
    LayersValues = layersValues.ToArray();
        ...
        damageableLayersValues.Sort((layerOne, layerTwo) =>
                                        layerOne.LayerType.GetOrder().CompareTo(
                                            layerTwo.LayerType.GetOrder()));

    DamageableLayers = damageableLayersValues.ToArray();
        matrix = GetComponent<Matrix>(); mainThread = Thread.CurrentThread;
  }
  ...
  private void MainThreadRemoveTile(TileLocation tileLocation) {
    if (tileLocation.layer.LayerType == LayerType.Floors ||
        tileLocation.layer.LayerType == LayerType.Base) {
      tileLocation.layer.TilemapDamage.SwitchObjectsMatrixAt(
          tileLocation.position);
    }

    // Remove before setting
    if (tileLocation.layer.LayerType ==
        LayerType.Underfloor) // TODO Tile map upgrade
    {
      lock (MultilayerPresentTiles) {
        var tileLocations = GetTileLocationsNeedLockSurrounding(
            tileLocation.position, tileLocation.layer);
    ...
    // TODO note Boundaries only recap later when tiles are added outside of it,
    // so therefore it can only increase in size
    //  remember update transforms and position and colour when removing On tile
    //  map I'm assuming It doesn't clear it? Maybe it sets it to the correct
    //  ones when you set a tile idk
    tileLocation.layer.subsystemManager.UpdateAt(tileLocation.position);
    lock (PooledTileLocation) { PooledTileLocation.Push(tileLocation); }

    if (CustomNetworkManager.IsServer) {
      if (tileLocation.layer.LayerType ==
          LayerType.Underfloor) // TODO Tilemap upgrade
      {
        matrix.TileChangeManager.AddToChangeList(
            tileLocation.position, tileLocation.layer.LayerType,
      ...
      RemoveTileMessage.Send(matrix.NetworkedMatrix.MatrixSync.netId,
                             tileLocation.position,
  ...
  private void MainThreadSetTile(TileLocation tileLocation) {
    ...
    if (CustomNetworkManager.IsServer) {
      if (tileLocation.layerTile.LayerType ==
          LayerType.Underfloor) // TODO Tilemap upgrade
      {
        matrix.TileChangeManager.AddToChangeList(
            tileLocation.position, tileLocation.layerTile.LayerType,
            tileLocation.layer, tileLocation, true, false);
      } else {
        matrix.TileChangeManager.AddToChangeList(
            tileLocation.position, tileLocation.layerTile.LayerType,
            tileLocation.layer, tileLocation, false, false);
      }

      UpdateTileMessage.Send(
          matrix.NetworkedMatrix.MatrixSync.netId, tileLocation.position,
          tileLocation.layerTile.TileType, tileLocation.layerTile.name,
          tileLocation.transformMatrix, tileLocation.Colour,
          tileLocation.layerTile.LayerType);
    }
  }
  ...
  /// </summary>
  public float ApplyDamage(Vector3Int cellPos, float damage,
                           Vector3Int worldPos,
    ...
    foreach (var damageableLayer in DamageableLayers) {
      ...
      RemainingDamage -= damageableLayer.TilemapDamage.ApplyDamage(
          damage, attackType, worldPos);
    }
  ...
  public bool
  IsPassableAtOneTileMap(Vector3Int origin, Vector3Int to, bool isServer,
                         CollisionType collisionType = CollisionType.Player,
                         bool inclPlayers = true, GameObject context = null,
                         List<LayerType> excludeLayers = null,
                         List<TileType> excludeTiles = null,
                         bool ignoreObjects = false, bool isReach = false,
                         bool onlyExcludeLayerOnDestination = false) {
    // Simple case: orthogonal travel
    if (origin.x == to.x || origin.y == to.y) {
      return IsPassableAtOrthogonal(
          origin, to, isServer, collisionType, inclPlayers, context,
          excludeLayers, excludeTiles, ignoreObjects, isReach: isReach);
    } else // diagonal travel
    {
      ...
      List<LayerType> diagonalExcludes =
          onlyExcludeLayerOnDestination ? null : excludeLayers;

      bool isPassableIfHorizontalFirst =
          IsPassableAtOrthogonal(origin, toX, isServer, collisionType,
          ...
          IsPassableAtOrthogonal(toX, to, isServer, collisionType, inclPlayers,
                                 context, excludeLayers, excludeTiles,
                                 ignoreObjects, isReach: isReach);

      if (isPassableIfHorizontalFirst)
        return true;

      bool isPassableIfVerticalFirst =
          IsPassableAtOrthogonal(origin, toY, isServer, collisionType,
                                 inclPlayers, context, diagonalExcludes,
                                 excludeTiles, ignoreObjects,
                                 isReach: isReach) &&
  ...
  private bool
  IsPassableAtOrthogonal(Vector3Int origin, Vector3Int to, bool isServer,
                         CollisionType collisionType = CollisionType.Player,
                         bool inclPlayers = true, GameObject context = null,
                         List<LayerType> excludeLayers = null,
                         List<TileType> excludeTiles = null,
    ...
    if (ignoreObjects == false &&
        ObjectLayer.IsPassableAtOnThisLayer(origin, to, isServer, collisionType,
                                            inclPlayers, context, excludeTiles,
                                            isReach: isReach) == false) {
      return false;
    }
    ...
    for (var i = 0; i < SolidLayersValues.Length; i++) {
      ...
      if (collisionType != CollisionType.Shuttle) {
        if ((solidLayer.LayerType == LayerType.Grills ||
             solidLayer.LayerType == LayerType.Tables ||
             solidLayer.LayerType == LayerType.Walls ||
             solidLayer.LayerType == LayerType.Windows) == false) {
          continue;
        }
      }
      ...
      if (excludeLayers != null &&
          excludeLayers.Contains(solidLayer.LayerType)) {
        continue;
      }
  ...
  public bool IsAtmosPassableAt(Vector3Int origin, Vector3Int to,
    ...
    return _IsAtmosPassableAt(origin, toX, isServer) &&
               _IsAtmosPassableAt(toX, to, isServer) ||
           _IsAtmosPassableAt(origin, toY, isServer) &&
               _IsAtmosPassableAt(toY, to, isServer);
  }

  private bool _IsAtmosPassableAt(Vector3Int origin, Vector3Int to,
                                  bool isServer) // TODO needs Object Passable
  {
    ...
    foreach (var layer in LayersValues) {
      if (layer.LayerType == LayerType.Walls ||
          layer.LayerType == LayerType.Windows) {
        lock (PresentTiles) {
  ...
  public TileLocation
  GetCorrectTileLocationForLayer(Vector3Int position, Layer layer,
                                 bool UseExactForMultilayer = false) {
    TileLocation tileLocation = null;
  ...
  public Vector3Int SetTile(Vector3Int position, TileType TileType,
                            string tileName, Matrix4x4? matrixTransform = null,
                            Color? color = null, bool isPlaying = true) {
    return SetTile(position, TileManager.GetTile(TileType, tileName),
                   matrixTransform, color, isPlaying);
  }

  public Vector3Int SetTile(Vector3Int position, LayerTile tile,
    ...
    if (Layers.TryGetValue(tile.LayerType, out var layer)) {
      ...
      if (tile.LayerType == LayerType.Underfloor) // TODO Tile map upgrade
      {
        lock (MultilayerPresentTiles) {
          var TileLocations =
              GetTileLocationsNeedLockSurrounding(position, layer);
        ...
        if (tileLocation == null) {
          ...
          lock (PresentTiles) { PresentTiles[layer][position] = tileLocation; }
        }
      }
  ...
  private void LogMissingLayer(Vector3Int position, LayerType layerType) {
    Logger.LogErrorFormat(
        "Modifying tile at cellPos {0} for layer type {1} failed because matrix {2} " +
            "has no layer of that type. Please add this layer to this matrix in" +
            " the scene.",
        Category.TileMaps, position, layerType, name);
  }

  /// <summary>
  /// Gets the tile with the specified layer type at the specified world
  /// position
  /// </summary>
  ...
  /// <param name="layerType"></param>
  /// <returns></returns>
  public LayerTile GetTileAtWorldPos(Vector3Int worldPosition,
                                     LayerType layerType,
                                     bool UseExactForMultilayer = false) {
  ...
  /// </summary>
  /// <param name="cellPosition">cell position within the tilemap to get the
  /// tile of. NOT the same as world position.</param> <param
  /// name="layerType"></param> <returns></returns>
  public LayerTile GetTile(Vector3Int cellPosition, LayerType layerType,
    ...
    if (Layers.TryGetValue(layerType, out var layer)) {
      TileLocation tileLocation = null;
      tileLocation = GetCorrectTileLocationForLayer(cellPosition, layer,
                                                    UseExactForMultilayer);
  ...
  private TileLocation GetTileExactLocationMultilayer(Vector3Int cellPosition,
                                                      Layer layer) {
    // TODO Tile map upgrade , z Is used as a depth but that needs to be moved
    // to vector4int where it would turn into w This you would just cast to
    // vector3int And use the w for depth Instead of z

    lock (MultilayerPresentTiles) {
      var tileLocations =
          GetTileLocationsNeedLockSurrounding(cellPosition, layer);
      if (tileLocations != null) {
  ...
  private List<TileLocation>
  GetTileLocationsNeedLockSurrounding(Vector3Int cellPosition, Layer layer) {
    // TODO Tile map upgrade , z Is used as a depth but that needs to be moved
    // to vector4int where it would turn into w
  ...
  private TileLocation GetTileLocationMultilayer(Vector3Int cellPosition,
                                                 Layer layer) {
    // TODO Tile map upgrade , z Is used as a depth but that needs to be moved
    // to vector4int where it would turn into w This you would just cast to
    // vector3int And use the w for depth Instead of z
  ...
  /// <summary>
  /// Gets the colour of the tile with the specified layer type at the specified
  /// cell position
  /// </summary>
  /// <param name="cellPosition">cell position within the tilemap to get the
  /// tile of. NOT the same as world position.</param> <param
  ...
  /// <returns></returns>
  public bool IsDifferent(Vector3Int cellPosition, LayerTile layerTile,
                          LayerType layerType,
    ...
    if (Layers.TryGetValue(layerType, out var layer)) {
      ...
      if (tileLocation?.layerTile != layerTile)
        return true;

      if (color != null) {
        if (tileLocation.Colour != color.GetValueOrDefault(Color.white))
          return true;
      }

      if (transformMatrix != null) {
        if (tileLocation.transformMatrix !=
            transformMatrix.GetValueOrDefault(Matrix4x4.identity))
          return true;
      }

      return false;
      // return layer.IsDifferent(cellPosition, layerTile, transformMatrix,
      // color);
    }
  ...
  /// </summary>
  /// <param name="cellPosition">cell position within the tilemap to get the
  /// tile of. NOT the same as world position.</param> <returns></returns>
  public LayerTile GetTile(Vector3Int cellPosition,
                           bool ignoreEffectsLayer = false,
                           bool UseExactForMultilayer = false) {
    TileLocation tileLocation = null;
    foreach (var layer in LayersValues) {
      if (layer.LayerType == LayerType.Objects)
      ...
      if (ignoreEffectsLayer && layer.LayerType == LayerType.Effects)
        continue;
  ...
  /// <summary>
  /// Gets the topmost tile at the specified cell position , Whilst ignoring the
  /// specified tiles in the ExcludedLayers
  /// </summary>
  /// <param name="cellPosition">cell position within the tilemap to get the
  /// tile of. NOT the same as world position.</param> <returns></returns>
  public LayerTile GetTile(Vector3Int cellPosition,
                           LayerTypeSelection ExcludedLayers,
                           bool UseExactForMultilayer = false) {
    TileLocation tileLocation = null;
    foreach (var layer in LayersValues) {
      if (layer.LayerType == LayerType.Objects)
    ...
    for (var index = 0; index < LayersValues.Length; index++) {
      ...
      if (layer.LayerType == LayerType.Objects) {
        foreach (RegisterTile o in isServer
                     ? ((ObjectLayer)LayersValues[index])
                           .ServerObjects.Get(position)
                     : ((ObjectLayer)LayersValues[index])
                           .ClientObjects.Get(position)) {
  ...
  /// <returns></returns>
  public Vector3Int? GetFreeOverlayPos(Vector3Int position,
                                       LayerType layerType) {
    if (layerType == LayerType.Objects) {
      Logger.LogError("Please use get objects instead of get tile");
      return null;
    ...
    if (Layers.TryGetValue(layerType, out var layer)) {
      // Go through overlays under the overlay limit. The first overlay checked
      // will be at z = 1.
      var count = 0;
  ...
  /// <param name="overlayName"></param>
  /// <returns></returns>
  public List<Vector3Int> GetOverlayPosByType(Vector3Int position,
                                              LayerType layerType,
  ...
  /// <param name="overlayName"></param>
  /// <returns></returns>
  public List<Vector3Int> GetAllOverlayPos(Vector3Int position,
                                           LayerType layerType) {
    if (layerType == LayerType.Objects) {
      Logger.LogError("Please use get objects instead of get tile");
      return null;
  ...
  /// <returns></returns>
  public List<OverlayTile> GetOverlayTilesByType(Vector3Int position,
                                                 LayerType layerType,
                                                 OverlayType overlayType) {
    if (layerType == LayerType.Objects) {
      Logger.LogError("Please use get objects instead of get tile");
  ...
  /// </summary>
  public bool HasOverlay(Vector3Int position, LayerType layerType,
                         OverlayTile overlayTileWanted) {
    if (layerType == LayerType.Objects) {
      Logger.LogError("Please use get objects instead of get tile");
      return false;
    }
  ...
  /// Whether a tile has this overlay already
  /// </summary>
  public bool HasOverlayOfType(Vector3Int position, LayerType layerType,
                               OverlayType overlayTypeWanted) {
    if (layerType == LayerType.Objects) {
      Logger.LogError("Please use get objects instead of get tile");
      return false;
    }
    ...
    if (Layers.TryGetValue(layerType, out var layer)) {
      ...
      while (count < OVERLAY_LIMIT) {
        ...
        if (tileLocation != null) {
          ...
          if (overlayTile != null &&
              overlayTile.OverlayType == overlayTypeWanted) {
            return true;
          }
        }
  ...
  public void RemoveTile(Vector3Int position) {
    ...
    foreach (var layer in LayersValues) {
      ...
      if (Application.isPlaying == false) {
        if (layer.LayerType == LayerType.Underfloor) {
          // TODO Tile map upgrade , xyz z = is the z The level so We need one
          // more xyzw w = what w Coordinate on the z Coordinate on the layer
          // the tile is so, Upgrade messages and the entire system to use
          // vector4int but For now since the z is left hanging is ok If it was
          // vector4int then Use that directly
      ...
      if (tileLocation != null) {
        ...
        if (refLayer != LayerType.Effects) {
          RemoveOverlaysOfType(tileLocation.position, LayerType.Effects,
                               OverlayType.Damage);
        }
  ...
  public Vector3
  LocalToWorld(Vector3 localPos) => LayersValues[0].LocalToWorld(localPos);
  public Vector3
  CellToWorld(Vector3Int cellPos) => LayersValues[0].CellToWorld(cellPos);
  public Vector3
  WorldToLocal(Vector3 worldPos) => LayersValues[0].WorldToLocal(worldPos);
  ...
  public void CacheLocalBound() {
    ...
    LocalCachedBounds =
        new BetterBoundsInt() { Maximum = maxPosition, Minimum = minPosition };
  }
  ...
  public BetterBounds CacheGlobalBound() {
    ...
    var bottomRight = localToWorldMatrix.MultiplyPoint(
        new Vector3(localBound.xMax, localBound.yMin, 0) + offset);
    var topLeft = localToWorldMatrix.MultiplyPoint(
        new Vector3(localBound.xMin, localBound.yMax, 0) + offset);
    var topRight = localToWorldMatrix.MultiplyPoint(localBound.max + offset);

    var globalPoints =
        new Vector3[4] { bottomLeft, bottomRight, topLeft, topRight };
    ...
    var newGlobalBounds =
        new BetterBounds() { Maximum = maxPosition, Minimum = minPosition };

    if (matrix.MatrixMove == null ||
        (CustomNetworkManager.IsServer &&
         matrix.MatrixMove.IsMovingServer == false &&
         matrix.MatrixMove.IsRotatingServer == false) ||
        (CustomNetworkManager.IsServer == false &&
         matrix.MatrixMove.IsMovingClient == false &&
         matrix.MatrixMove.IsRotatingServer == false)) {
  ...
  public void SetPreviewTile(Vector3Int position, LayerTile tile,
    ...
    for (var i = 0; i < LayersValues.Length; i++) {
      ...
      if (layer.LayerType < tile.LayerType) {
        if (layer.LayerType == LayerType.Objects)
          continue;
        Layers[layer.LayerType].SetPreviewTile(position, LayerTile.EmptyTile,
                                               Matrix4x4.identity);
      }
    }

    if (Layers.ContainsKey(tile.LayerType) == false) {
      Logger.LogErrorFormat($"LAYER TYPE: {0} not found!", Category.TileMaps,
                            tile.LayerType);
      return;
    }
...
#region Physics
  ...
  public MatrixManager.CustomPhysicsHit? Raycast(
    ...
    if (direction.x < 0) {
      gridOffsetx =
          -(-0.5d + Offsetuntouchx); // 0.5f  //this is So when you multiply it
                                     // gives you 0.5 that some tile borders
    ...
    var vexinvX =
        (1d /
         (direction.x)); // Editions need to be done here for Working offset
    ...
    while (Math.Abs((xSteps + gridOffsetx + stepX) * vexinvX) < distance ||
      ...
      if ((xSteps + gridOffsetx + stepX) * vexinvX <
          (ySteps + gridOffsety + stepY) *
              vexinvY) // which one has a lesser multiplication factor since
                       // that will give a less Magnitude
      {
        xSteps += stepX;
        ...
        if (LTSUtil.IsLayerIn(layerMask, LayersValues[i].LayerType)) {
          ...
          if (tileLocation != null) {
            if (tileNamesToIgnore != null &&
                tileNamesToIgnore.Any(
                    c => c.name == tileLocation?.layerTile.name)) continue;

            Vector2 normal;

                if (LeftFaceHit) { normal = Vector2.left * stepX; } else {
                  normal = Vector2.down * stepY;
                }

                Vector3 AdjustedNormal = ((Vector3)normal).ToWorld(matrix);
                AdjustedNormal =
                    AdjustedNormal - (Vector3.zero.ToWorld(matrix));

                return new MatrixManager.CustomPhysicsHit(
                    ((Vector3)vec).ToWorld(matrix), (vecHit).ToWorld(matrix),
                    AdjustedNormal,
                    new Vector2((float)RelativeX, (float)RelativeY).magnitude,
                    tileLocation);
          }
        }
      }
      ...
      for (int n = bounds.xMin; n < bounds.xMax; n++) {
        for (int p = bounds.yMin; p < bounds.yMax; p++) {
            ...
            if (getTile != null) {
              ...
              if (isServer) {
                ...
                if (electricalCableTile != null) {
                  layer.matrix.AddElectricalNode(
                      new Vector3Int(n, p, localPlace.z), electricalCableTile);
                }

                var disposalPipeTile =
                    getTile as Objects.Disposals.DisposalPipe;
                ...
                if (pipeTile != null) {
                  var matrixStruct =
                      layer.matrix.UnderFloorLayer.Tilemap.GetTransformMatrix(
                          localPlace);
                  var connection =
                      PipeTile.GetRotatedConnection(pipeTile, matrixStruct);
          ...
          if (remove) {
            ...
            ToInsertDictionary[AlocalPlacezzero].RemoveRange(
                LastIndex + 1,
  ...
  public IEnumerable<T> GetAllTilesByType<T>(Vector3Int position,
                                             LayerType LayerType)
      where T : LayerTile {
  ...
  public Matrix4x4? GetMatrix4x4(Vector3Int cellPosition, LayerType layerType,
                                 bool UseExactForMultilayer = false) {
    if (layerType == LayerType.Objects)
      return null;
    if (Layers.TryGetValue(layerType, out var layer)) {
  ...
  public void AddOverlay(Vector3Int cellPosition, OverlayTile overlayTile,
    ...
    if (HasOverlay(cellPosition, overlayTile.LayerType, overlayTile))
      return;
  ...
  public void AddOverlay(Vector3Int cellPosition, TileType tileType,
                         string tileName, Matrix4x4? transformMatrix = null,
                         Color? color = null) {
  ...
  public Color? GetColourOfFirstTile(Vector3Int cellPosition,
                                     OverlayType overlayType,
                                     LayerType layerType) {
  ...
  public void RemoveFloorWallOverlaysOfType(Vector3Int cellPosition,
                                            OverlayType overlayType,
                                            bool onlyIfCleanable = false) {
    RemoveOverlaysOfType(cellPosition, LayerType.Floors, overlayType,
                         onlyIfCleanable);
    RemoveOverlaysOfType(cellPosition, LayerType.Walls, overlayType,
                         onlyIfCleanable);
  }

  public void RemoveAllOverlays(Vector3Int cellPosition, LayerType layerType,
                                bool onlyIfCleanable = false) {
    cellPosition.z = 0;
  ...
  public bool HasOverlay(Vector3Int cellPosition, TileType tileType,
                         string overlayName) {
...
public enum OverlayType {
  // none is used to say there is no overlay, add new category if you need a new
  // type
  None,
  Gas,
  Damage,
