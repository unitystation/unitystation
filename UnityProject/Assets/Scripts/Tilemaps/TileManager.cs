...
public static class TilePaths {
  private static Dictionary<TileType, string> paths =
...
public class TileManager : SingletonManager<TileManager>, IInitialise {
  ...
  private Dictionary<TileType, Dictionary<string, LayerTile>> tiles =
      new Dictionary<TileType, Dictionary<string, LayerTile>>();
  private bool initialized;

  [SerializeField]
  private List<TilePathEntry> layerTileCollections = new List<TilePathEntry>();
  ...
  public static LayerTile GetTile(TileType tileType, string key) {
    if (!Instance.initialized)
      Instance.StartCoroutine(Instance.LoadAllTiles());

    if (Instance.tiles.TryGetValue(tileType, out var tiles) &&
        tiles.TryGetValue(key, out var layerTile)) {
      return layerTile;
    }
