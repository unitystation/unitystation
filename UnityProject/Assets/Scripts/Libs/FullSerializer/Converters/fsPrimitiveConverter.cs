...
namespace FullSerializer.Internal
{
	public class fsPrimitiveConverter : fsConverter
	{
		...
		{
			return type == typeof(sbyte) || type == typeof(byte) || type == typeof(short) || type == typeof(ushort) || type == typeof(int) ||
		...
		public override fsResult TrySerialize(object instance, out fsData serialized, Type storageType)
		{
			...
			if (UseDouble(instanceType))
			{
				...
				if (instance.GetType() == typeof(float) &&
				    ...
				    (float) instance != float.MinValue && (float) instance != float.MaxValue && !float.IsInfinity((float) instance) &&
