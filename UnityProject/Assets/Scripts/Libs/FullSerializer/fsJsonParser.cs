...
namespace FullSerializer
{
	...
	public class fsJsonParser
	{
		...
		private fsResult MakeFailure(string message)
		{
			...
			string error = "Error while parsing: " + message + "; context = <" + _input.Substring(start, length) + ">";
		...
		private fsResult TryParseNumber(out fsData data)
		{
			...
			// double -- includes a .
			if (numberString.Contains(".") || numberString.Contains("e") || numberString.Contains("E") || numberString == "Infinity" ||
		...
		private fsResult TryParseObject(out fsData obj)
		{
			...
			Dictionary<string, fsData> result =
				new Dictionary<string, fsData>(fsGlobalConfig.IsCaseSensitive ? StringComparer.Ordinal : StringComparer.OrdinalIgnoreCase);
