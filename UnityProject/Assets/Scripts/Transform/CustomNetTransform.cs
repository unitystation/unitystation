...
//		var worldPos = serverState.WorldPosition;//
		...
		if ( registerTile && registerTile.Matrix ) {
			...
			if ( initError ) {
				...
				Logger.LogWarning( $"{gameObject.name}: unable to detect MatrixId!" );
			} else {
...
//		Logger.Log( $"{gameObject.name} doing matrix switch check for {pos}" );
		int newMatrixId = MatrixManager.AtPoint( pos ).Id;
		if ( serverState.MatrixId != newMatrixId ) {
//			Logger.Log( $"{gameObject} matrix {serverState.MatrixId}->{newMatrixId}" );
	...
	{
//		Logger.Log( $"{gameObject.name} Notified" );
		SyncMatrix();
