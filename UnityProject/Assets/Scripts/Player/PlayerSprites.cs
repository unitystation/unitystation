...
/// <summary>
/// Handle displaying the sprites related to player, which includes underwear
/// and the body. Note that the clothing you put on (UniCloths) are handled in
/// Equipment Ghosts are handled in GhostSprites.
/// </summary>
...
#region Inspector fields
  ...
  /// <summary>
  /// Threshold value where we switch from partial burning to fully engulfed
  /// sprite.
  /// </summary>
  ...
  public readonly Dictionary<NamedSlot, ClothingItem> clothes =
      new Dictionary<NamedSlot, ClothingItem>();

  public readonly List<BodyPartSprites> Addedbodypart =
      new List<BodyPartSprites>();

  public readonly List<SpriteHandlerNorder> OpenSprites =
      new List<SpriteHandlerNorder>();

  public readonly List<BodyPartSprites> SurfaceSprite =
      new List<BodyPartSprites>();
  ...
  private void OnDestroy() {
    playerHealth.OnClientFireStacksChange.RemoveListener(
        OnClientFireStacksChange);
  }
  ...
  private void AddOverlayGameObjects() {
    if (engulfedBurningOverlay == null) {
      engulfedBurningOverlay =
          Instantiate(engulfedBurningPrefab, OverlaySprites.transform)
              .GetComponent<PlayerDirectionalOverlay>();
    ...
    if (partialBurningOverlay == null) {
      partialBurningOverlay =
          Instantiate(partialBurningPrefab, OverlaySprites.transform)
              .GetComponent<PlayerDirectionalOverlay>();
    ...
    if (electrocutedOverlay == null) {
      electrocutedOverlay =
          Instantiate(electrocutedPrefab, OverlaySprites.transform)
              .GetComponent<PlayerDirectionalOverlay>();
  ...
  public void SubSetBodyPart(BodyPart Body_Part, string path) {
    ...
    if (customisationStorage != null) {
      ...
      if (Body_Part.OptionalOrgans.Count > 0) {
        BodyPartDropDownOrgans.OnPlayerBodyDeserialise(Body_Part, data,
        ...
        if (Body_Part.LobbyCustomisation == null) {
          Logger.Log(
              $"[PlayerSprites] - Could not find {Body_Part.name}'s characterCustomization script. Returns -> {Body_Part.LobbyCustomisation.characterCustomization}",
              Category.Character);
          return;
        }
        Body_Part.LobbyCustomisation.OnPlayerBodyDeserialise(
            Body_Part, data, livingHealthMasterBase);
      }
    }
  ...
  public void SetupSprites() {
    ...
    if (ThisCharacter.SerialisedBodyPartCustom == null) {
      ...
      Logger.LogWarning(
          $"{gameObject} has spawned with null bodyPart customizations. This error should only appear for Dummy players only.");
      return;
    }
    ...
    if (customisationStorage != null) {
      BodyPartDropDownOrgans.OnPlayerBodyDeserialise(
          null, customisationStorage.Data, livingHealthMasterBase);
    }
    ...
    foreach (var Customisation in SetRace.Base.CustomisationSettings) {
      ...
      var SpriteHandlerNorder =
          Spawn
              .ServerPrefab(ToInstantiateSpriteCustomisation.gameObject, null,
      ...
      SpriteHandlerNorder.name =
          Customisation.CustomisationGroup.ThisType.ToString();

      var newone = new IntName();
      newone.Int = CustomNetworkManager.Instance.IndexLookupSpawnablePrefabs
                   [ToInstantiateSpriteCustomisation.gameObject];
      ...
      // SubSetBodyPart
      foreach (var Sprite_s in Customisation.CustomisationGroup
                   .PlayerCustomisations) {
        if (Sprite_s.Name ==
            externalCustomisation.SerialisedValue.SelectedName) {
          SpriteHandlerNorder.SpriteHandler.SetSpriteSO(
              Sprite_s.SpriteEquipped);
          SpriteHandlerNorder.SetSpriteOrder(
              new SpriteOrder(Customisation.CustomisationGroup.SpriteOrder));
          newone.Data = JsonConvert.SerializeObject(
              new SpriteOrder(SpriteHandlerNorder.SpriteOrder));
          Color setColor = Color.black;
          ColorUtility.TryParseHtmlString(
              externalCustomisation.SerialisedValue.Colour, out setColor);
          setColor.a = 1;
          SpriteHandlerNorder.SpriteHandler.SetColor(setColor);
        }
      }
    }
    GetComponent<RootBodyPartController>().PlayerSpritesData =
        JsonConvert.SerializeObject(ToClient);
  ...
  public void SetSurfaceColour() {
    ...
    if (RaceBodyparts.Base.SkinColours.Count > 0) {
      ColorUtility.TryParseHtmlString(ThisCharacter.SkinTone,
                                      out CurrentSurfaceColour);
  ...
  /// <summary>
  /// Toggle the electrocuted overlay for the player's mob. Assumes player mob
  /// is humanoid.
  /// </summary>
  ...
  public void OnCharacterSettingsChange(CharacterSettings characterSettings) {
    if (RootBodyPartsLoaded == false) {
      ...
      SetUpCharacter();
      SetupSprites();
      livingHealthMasterBase.CirculatorySystem.SetBloodType(
          RaceBodyparts.Base.BloodType);
      livingHealthMasterBase.InitialiseHunger(
          RaceBodyparts.Base.NumberOfMinutesBeforeStarving);
    }
  }

  public void NotifyPlayer(NetworkConnection recipient,
                           bool clothItems = false) {
    if (clothItems) {
      for (int i = 0; i < characterSprites.Length; i++) {
        var clothItem = characterSprites[i];
        PlayerAppearanceMessage.SendTo(gameObject, i, recipient,
                                       clothItem.GameObjectReference, true,
                                       true);
      }
    }
  }
  ...
  /// </summary>
  public void ShowMuzzleFlash() { StartCoroutine(AnimateMuzzleFlash()); }
  ...
  public void OnClothingEquipped(ClothingV2 clothing, bool isEquipped) {
    // Logger.Log($"Clothing {clothing} was equipped {isEquiped}!",
    // Category.Inventory);
    ...
    if (isEquipped) {
      for (int n = 0; n < 11; n++) {
        ...
        if (IsBitSet((ulong)clothing.HideClothingFlags, n) &&
            (IsBitSet((ulong)hideClothingFlags, n))) {
          overflow |= 1UL << n;
        } else if (IsBitSet((ulong)clothing.HideClothingFlags,
                            n)) // check if n'th bit is set to 1
        {
  ...
  private bool IsBitSet(ulong b, int pos) { return ((b >> pos) & 1) != 0; }
  ...
  /*
      private void ValidateHideFlag(ClothingHideFlags hideFlag, string name)
      {
              ...
              {
                      Logger.LogError($"Can't find {name} clothingItem linked to
     {hideFlag}", Category.PlayerInventory); return;
              }
    ...
    foreach (var ID in NewInternalNetIDs) {
      ...
      if (Contains == false) {
        if (CustomNetworkManager.Instance.allSpawnablePrefabs.Count > ID.Int) {
          var OB =
              Instantiate(
                  CustomNetworkManager.Instance.allSpawnablePrefabs[ID.Int],
                  this.gameObject.transform)
                  .transform;
          var Net = SpriteHandlerManager.GetRecursivelyANetworkBehaviour(
              OB.gameObject);
