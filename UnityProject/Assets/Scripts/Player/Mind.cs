...
public class Mind : NetworkBehaviour, IActionGUI
{
	...
	{
		if (AdminCommandsManager.IsAdmin(this.connectionToClient, out var _, "ADMIN_POSSESS_BODY"))
		{
	...
	{
		if (AdminCommandsManager.IsAdmin(connectionToClient, out _, "ADMIN_AGHOST"))
		{
			if (IsGhosting)
			{
				StopGhosting();
