...
namespace Objects.Construction {
  ...
  /// </summary>
  public class APCFrame : NetworkBehaviour,
                          ICheckedInteractable<HandApply>,
                          IExaminable {
    [SerializeField]
    private StatefulState initialState = null;
    [SerializeField]
    private StatefulState cablesAddedState = null;
    [SerializeField]
    private StatefulState powerControlAddedState = null;
    [SerializeField]
    private StatefulState powerCellAddedState = null;
    [SerializeField]
    private StatefulState wrenchedState = null;
    ...
    [SerializeField]
    private ItemTrait powerCellTrait = null;
    [Tooltip("APC gameObject for this frame to create")]
    [SerializeField]
    private GameObject APCObject = null;
    ...
    private void OnEnable() {
      try {
        ...
        Logger.LogError(
            $"Catched a NRE in APCFrame OnEnable() {exception.Message} \n {exception.StackTrace}",
            Category.Electrical);
      }
    }
    ...
    public bool WillInteract(HandApply interaction, NetworkSide side) {
      ...
      if (CurrentState == initialState) {
        ...
        return Validations.HasUsedItemTrait(interaction,
               ...
               Validations.HasUsedItemTrait(
                   interaction, CommonTraits.Instance.PowerControlBoard);
        ...
        // wrench on cover or crowbar out power control module which removes the
        // power cell too
        return Validations.HasUsedItemTrait(interaction,
                                            CommonTraits.Instance.Wrench) ||
               Validations.HasUsedItemTrait(interaction,
                                            CommonTraits.Instance.Crowbar);
      } else if (CurrentState == wrenchedState) {
        // screw in parts or crowbar off the cover
        return Validations.HasUsedItemTrait(
                   interaction, CommonTraits.Instance.Screwdriver) ||
               Validations.HasUsedItemTrait(interaction,
                                            CommonTraits.Instance.Crowbar);
      }
    ...
    private void InitialStateInteraction(HandApply interaction) {
      if (Validations.HasUsedItemTrait(interaction,
                                       CommonTraits.Instance.Cable) &&
          Validations.HasUsedAtLeast(interaction, 5)) {
        // add 5 cables
        ToolUtils.ServerUseToolWithActionMessages(
            interaction, 2f, "You start adding cables to the frame...",
        ...
        // deconsruct
        ToolUtils.ServerUseToolWithActionMessages(
            interaction, 2f, "You start deconstructing the frame...",
            ...
            () => {
              Spawn.ServerPrefab(CommonPrefabs.Instance.Metal,
                                 SpawnDestination.At(gameObject), 2);
    ...
    /// <summary>
    /// Stage 2, Add the power control module to continue contruction, or
    /// wirecutters to move to stage 1
    /// </summary>
    /// <param name="interaction"></param>
    private void CablesAddedStateInteraction(HandApply interaction) {
      if (Validations.HasUsedItemTrait(interaction,
        ...
        Chat.AddActionMsgToChat(
            interaction, $"You remove the cables.",
            $"{interaction.Performer.ExpensiveName()} removes the cables.");
        ToolUtils.ServerPlayToolSound(interaction);
        Spawn.ServerPrefab(CommonPrefabs.Instance.SingleCableCoil,
                           SpawnDestination.At(gameObject), 5);
        stateful.ServerChangeState(initialState);

        spriteHandler.ChangeSprite((int)SpriteStates.Frame);
      } else if (Validations.HasUsedItemTrait(
                     interaction, CommonTraits.Instance.PowerControlBoard)) {
        // stick in the circuit board
        Chat.AddActionMsgToChat(
            interaction,
            $"You place the {interaction.UsedObject.ExpensiveName()} inside the frame.",
    ...
    /// <summary>
    /// Stage 3, Add in power cell, or remove power control module to go back to
    /// stage 2.
    /// </summary>
    /// <param name="interaction"></param>
    private void PowerControlAddedStateInteraction(HandApply interaction) {
      if (Validations.HasUsedItemTrait(interaction,
                                       CommonTraits.Instance.Crowbar)) {
        Chat.AddActionMsgToChat(
            interaction, $"You remove the power control module from the frame.",
        ...
        Chat.AddActionMsgToChat(
            interaction,
            $"You place the {usedObject.ExpensiveName()} inside the frame.",
    ...
    private void PowerCellAddedStateInteraction(HandApply interaction) {
      ...
      if (Validations.HasUsedItemTrait(interaction,
        ...
        ToolUtils.ServerPlayToolSound(interaction);
        ToolUtils.ServerUseToolWithActionMessages(
            interaction, 2f, "You begin securing the cover to the frame...",
      ...
      } else if (Validations.HasUsedItemTrait(interaction,
                                              CommonTraits.Instance.Crowbar)) {
    ...
    private void WrenchedStateInteraction(HandApply interaction) {
      // Complete construction, spawn new machine and send data over to it.
      if (Validations.HasUsedItemTrait(interaction,
                                       CommonTraits.Instance.Screwdriver)) {
        Chat.AddActionMsgToChat(
            interaction, $"You secure the electronics to the APC.",
            $"{interaction.Performer.ExpensiveName()} secures the electronics to the APC.");
        ToolUtils.ServerPlayToolSound(interaction);

        MatrixInfo matrix = MatrixManager.AtPoint(
            gameObject.GetComponent<CustomNetTransform>().ServerPosition, true);

        var localPosInt = MatrixManager.WorldToLocalInt(
            gameObject.GetComponent<CustomNetTransform>().ServerPosition,
            matrix);
        var econs = interaction.Performer.RegisterTile()
                        .Matrix.GetElectricalConnections(localPosInt);
        ...
        GameObject WallMount =
            Spawn
                .ServerPrefab(APCObject,
                              gameObject.GetComponent<CustomNetTransform>()
                                  .ServerPosition,
        ...
        if (Directional != null)
          Directional.FaceDirection(
              gameObject.GetComponent<Directional>().CurrentDirection);

        ItemSlot apcPowerControlSlot =
            WallMount.GetComponent<ItemStorage>().GetIndexedItemSlot(0);
        ItemSlot apcPowerCellSlot =
            WallMount.GetComponent<ItemStorage>().GetIndexedItemSlot(1);
        ...
        ToolUtils.ServerPlayToolSound(interaction);
        ToolUtils.ServerUseToolWithActionMessages(
            interaction, 2f, "You begin prying the cover from the frame...",
    ...
    public string Examine(Vector3 worldPos) {
      ...
      if (CurrentState == initialState) {
        msg =
            " Add five wires to continue construction. Or use a welder to deconstruct.\n";
      }
      if (CurrentState == cablesAddedState) {
        msg =
            " Add a power control module to the frame to continue construction. Or use a wirecutter to remove the cables.\n";
      }
      if (CurrentState == powerControlAddedState) {
        msg =
            "Add a power cell module to continue contruction. Or use a crowbar to remove power control module.\n";
      }
      if (CurrentState == powerCellAddedState) {
        msg =
            "Use a wrench the secure the cover or use crowbar to remove power control module.\n";
      }
      if (CurrentState == wrenchedState) {
        msg =
            "Use a screwdriver to finish construction or use crowbar to remove the cover.\n";
      }
    ...
    public void ServerInitFromComputer(APC apc) {
      ItemSlot apcPowerControlSlot =
          apc.GetComponent<ItemStorage>().GetIndexedItemSlot(0);
      ItemSlot apcPowerCellSlot =
          apc.GetComponent<ItemStorage>().GetIndexedItemSlot(1);
    ...
    public void WhenDestroyed(DestructionInfo info) {
      if (CurrentState == powerCellAddedState ||
          CurrentState == powerControlAddedState) {
        RemoveCircuitAndParts();
      }
      integrity.OnWillDestroyServer.RemoveListener(WhenDestroyed);
    }
