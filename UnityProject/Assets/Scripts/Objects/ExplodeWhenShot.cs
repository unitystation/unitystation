...
public class ExplodeWhenShot : NetworkBehaviour
{
	...
	private void SetFire()
	{
		...
		EffectsFactory.SpawnFireTileClient(Random.Range(0.4f, 1f), pos, transform.parent);
		pos.x--;
		pos.y++;
		...
		for (int i = 0; i < cLength; i++)
		{
			for (int j = 0; j < rHeight; j++)
			{
				...
				{
					EffectsFactory.SpawnFireTileClient(Random.Range(0.4f, 1f), checkPos, transform.parent);
					maxNumOfFire--;
				}
				if (maxNumOfFire <= 0)
				{
