	...
	public class FieldGenerator : MonoBehaviour, ICheckedInteractable<HandApply>, IOnHitDetect, IExaminable
	{
		...
		private AnimatedTile GetTileFromDirection(Direction direction)
		{
			switch (direction)
			{
				...
				case Direction.Right:
					return horizontal;
				default:
					Logger.LogError($"Somehow got a wrong direction for {gameObject.ExpensiveName()} tile setting", Category.Unknown);
					return vertical;
			}
		}
		...
		private Vector3Int GetCoordFromDirection(Direction direction)
		{
			switch (direction)
			{
				...
				default:
					Logger.LogError($"Somehow got a wrong direction for {gameObject.ExpensiveName()}", Category.Unknown);
		...
		private Direction GetOppositeDirection(Direction direction)
		{
			switch (direction)
			{
				...
				default:
					Logger.LogError($"Somehow got wrong opposite direction for {gameObject.ExpensiveName()}", Category.Unknown);
					return Direction.Up;
			}
		}
