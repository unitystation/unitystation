...
namespace Objects.Medical
{
	public class CloningPod : NetworkBehaviour, IRefreshParts
	{
		...
		private IEnumerator ServerProcessCloning(CloningRecord record)
		{
			...
			{
				var playerBody = PlayerSpawn.RespawnPlayerAt(record.mind,  record.mind.occupation,  record.mind.CurrentCharacterSettings, transform.position.CutToInt()).GetComponent<LivingHealthMasterBase>();
