...
namespace Objects.Other
{
	public class JanitorCart : ReagentContainer, ICheckedInteractable<HandApply>, IHoverTooltip
	{
		...
		private bool GrabTool(HandApply interaction, ItemTrait thingToGrab)
		{
			foreach (var slot in jaintorToolsHolding.GetIndexedSlots())
			{
				...
				Chat.AddActionMsgToChat(interaction.Performer, $"{interaction.PerformerPlayerScript.visibleName} grabs " +
																$"something from the {gameObject.DisplayName()}.");
				return true;
			}
		...
		private void AddGarbageBag(HandApply interaction)
		{
			...
			{
				Chat.AddActionMsgToChat(interaction.Performer, $"{interaction.PerformerPlayerScript.visibleName} adds a {interaction.HandObject.DisplayName()} to the {gameObject.DisplayName()}.");
				return;
			}
			Chat.AddExamineMsg(interaction.Performer, $"The {gameObject.DisplayName()} has no space for the {interaction.HandObject.DisplayName()}");
		}
		...
		private void AddItemToGarbageBag(HandApply interaction)
		{
			foreach (var slot in jaintorToolsHolding.GetItemSlots())
			{
				...
				{
					Chat.AddActionMsgToChat(interaction.Performer, $"{interaction.PerformerPlayerScript.visibleName} throws a {interaction.HandObject.DisplayName()} into the {gameObject.DisplayName()}.");
					return;
				}
