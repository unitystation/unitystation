...
namespace Objects.Other {
  ...
  public class Turret : NetworkBehaviour,
    ...
    [SerializeField]
    [Tooltip(
        "Used to get the lethal bullet and spawn the gun when deconstructed")]
    ...
    [SerializeField]
    [Tooltip(
        "Used as back up if cant find bullet from mapped gun or gun put in")]
    ...
    [Range(0.1f, 10f)]
    [Tooltip(
        "Multiplies on the default fire delay of the gun inside, lower is quicker")]
    private float shootSpeedMultiplier = 1f;

    [SerializeField]
    [Range(0.1f, 10f)]
    [Tooltip(
        "Used for taser or default laser when theres no gun inside, multiplied by shootSpeedMultiplier")]
    ...
    [SerializeField]
    [Tooltip(
        "If turret not normal, then will always target mobs, if turret is Ai then will ignore all other checks and always fire")]
    // If turret not normal, then will always target mobs, if turret is Ai then
    // will ignore all other checks and always fire
    private TurretType turretType = TurretType.Normal;

    [SyncVar(hook = nameof(SyncOpen))]
    private bool open;

    private enum TurretType { Normal, Ai, Syndicate }

    // Check for Weapon Authorization:
    // No/Yes - neutralizes people who have a weapon out but are not Heads or
    // Security staff.
    [Tooltip(
        "Neutralize people who have a weapon out but are not Heads or Security staff")]
...
#region Lifecycle
    ...
    public void OnSpawnServer(SpawnInfo info) {
      ...
      var newGun = Spawn.ServerPrefab(spawnGun, registerTile.WorldPosition,
                                      transform.parent);

      if (newGun.Successful == false)
        return;
      ...
      // For some reason couldnt use item storage, would just stay above the
      // turret
    ...
    private void SyncRotation(Vector2 oldValue, Vector2 newValue) {
      ...
      // Ballistic turret sprite is initially pointing upwards, whereas we
      // assume it points down for laser so add 180
      if (ballisticTurret) {
        angle += 180;
      }
    ...
    private void UpdateLoop() {
      ...
      if (shootingTarget || coverOpen == false ||
          turretState == TurretState.Off || target == null)
        return;
      if (shootingTimer < shootSpeed)
        return;
      shootingTarget = true;
      shootingTimer = 0;
...
#region Shooting
    ...
    private void SearchForMobs() {
      ...
      var mobsFound = Physics2D.OverlapCircleAll(
          turretPos.To2Int(), range, LayerMask.GetMask("Players", "NPC"));
      ...
      var orderedMobs =
          mobsFound
              .OrderBy(x => (turretPos - x.transform.position).sqrMagnitude)
              .ToList();
      ...
      foreach (var mob in orderedMobs) {
        ...
        if (mob.TryGetComponent<PlayerScript>(out var script)) {
          ...
          if (script.PlayerState != PlayerScript.PlayerStates.Normal ||
              script.IsDeadOrGhost)
            continue;

          // Check if player is allowed, but only if not an Ai turret as those
          // will shoot all targets
          if (turretType != TurretType.Ai && ValidatePlayer(script))
            continue;

          worldPos = script.WorldPos;
        }
        // Test for mob, syndicate and AI will always target mobs, otherwise
        // only on unidentified
        else if ((turretType != TurretType.Normal ||
                  CheckUnidentifiedLifeSigns) &&
                 mob.TryGetComponent<MobAI>(out var mobAi)) {
        ...
        // TODO maybe add bool to linecast to only do raycast if linecast
        // reaches WorldTo
        var linecast = MatrixManager.Linecast(
            turretPos, LayerTypeSelection.Walls,
            LayerMask.GetMask("Door Closed", "Walls"), worldPos);

        // TODO theres something buggy with linecast, seems like the raycast
        // check is going through doors?? lineRenderer.positionCount = 2;
        // lineRenderer.SetPositions(new []{turretPos, linecast.ItHit ?
        // linecast.HitWorld : worldPos}); lineRenderer.enabled = true;

        // Check to see if we hit a wall or closed door, allow for tolerance
        if (linecast.ItHit)
          continue; // && Vector3.Distance(worldPos, linecast.HitWorld) > 0.2f
    ...
    /// <summary>
    /// Validate player based on settings of turret, true if player is allowed,
    /// false if player failed validation
    /// </summary>
    private bool ValidatePlayer(PlayerScript script) {
      ...
      if (CheckUnauthorisedPersonnel) {
        ...
        foreach (var access in authorisedAccess) {
          if (AccessRestrictions.CheckAccess(script.gameObject, access) ==
              false)
            continue;
      ...
      // Record Checking
      if (CheckUnidentifiedLifeSigns || CheckForArrest ||
          CheckSecurityRecords) {
        var hasRecord = false;
        ...
        // Neutralize All Unidentified Life Signs
        // Unknown name check here or else it would be possible for someone to
        // add record with the name Unknown and then wouldn't be targeted by
        // turrets
        if (CheckUnidentifiedLifeSigns &&
            (hasRecord == false || script.visibleName.Equals("Unknown"))) {
          return false;
        }
      }
      ...
      if (CheckWeaponAuthorisation && script.Equipment != null) {
        if (CheckSlot(NamedSlot.rightHand) == false ||
            CheckSlot(NamedSlot.leftHand) == false) {
          return false;
        }
        ...
        bool CheckSlot(NamedSlot slot) {
          ...
          if (Validations.HasItemTrait(handItem.GameObjectReference,
                                       CommonTraits.Instance.Gun)) {
    ...
    private void ShootAtDirection(Vector2 rotationToShoot) {
      ...
      SoundManager.PlayNetworkedAtPos(bulletSound, registerTile.WorldPosition,
                                      sourceObj: gameObject);

      CastProjectileMessage.SendToAll(gameObject, bulletName, rotationToShoot,
                                      default);
    }
    ...
    private void SetUpBullet() {
      ...
      if (gun != null) {
        if (gun is GunElectrical electrical &&
            electrical.firemodeProjectiles.Count > 0 &&
            electrical.firemodeFiringSound.Count > 0) {
          bulletName =
              electrical.firemodeProjectiles[0].GetComponent<Bullet>().name;
          ...
          bulletName =
              gun.CurrentMagazine.initalProjectile.GetComponent<Bullet>().name;
    ...
    private void ChangeCoverState() {
      ...
      if (coverStateChanging)
        return;

      // Open if we need to
      if (coverOpen == false && turretState != TurretState.Off &&
          target != null) {
        StartCoroutine(WaitForAnimation(true));
        return;
      }

      // Dont bother closing if we are already closed and Only close if no
      // targets and we are open and have power
      if (coverOpen == false ||
          (coverOpen && turretState != TurretState.Off && target != null))
        return;
    ...
    private IEnumerator WaitForAnimation(bool stateAfter) {
      ...
      // Wait for animation to complete before allowing to fire or change cover
      // state
    ...
    public enum TurretState { Off, Stun, Lethal }
...
#region Hand Interaction
    ...
    public bool WillInteract(HandApply interaction, NetworkSide side) {
      ...
      return Validations.HasUsedItemTrait(interaction,
                                          CommonTraits.Instance.Crowbar);
    }
    ...
    public void ServerPerformInteraction(HandApply interaction) {
      ...
      if (Validations.HasUsedItemTrait(interaction, CommonTraits.Instance.Id)) {
        if (accessRestrictions.CheckAccessCard(interaction.HandObject) ==
          ...
          Chat.AddExamineMsgFromServer(
              ...
              $"You need higher authorisation to unlock this {gameObject.ExpensiveName()}");
          return;
        }

        // If unlocked then quick to lock, if locked then if unconnected to
        // switch quick to unlock Else locked and connected so take long to stop
        // rush unlocking to switch turrets off
        var time = unlocked ? 1f : connectedSwitch != null ? 10f : 1f;

        var bar = StandardProgressAction.Create(
            new StandardProgressActionConfig(
                StandardProgressActionType.Construction, false, false, true),
            Perform);
        bar.ServerStartProgress(interaction.Performer.RegisterTile(), time,
                                interaction.Performer);
      ...
      ToolUtils.ServerUseToolWithActionMessages(
          ...
          () => {
            var frame = Spawn.ServerPrefab(
                framePrefab, registerTile.WorldPosition, transform.parent);
            ...
            frame.GameObject.GetComponent<TurretFrame>().SetUp(
                gun != null ? gun.GetComponent<Pickupable>() : null);
    ...
    private void OnTurretDestroy(DestructionInfo info) {
      Spawn.ServerPrefab(framePrefab, registerTile.WorldPosition,
                         transform.parent);
    ...
    public void UpdateGui() {
      if (NetworkTabManager.Instance == null)
        return;

      var peppers =
          NetworkTabManager.Instance.GetPeepers(gameObject, NetTabType.Turret);
      if (peppers.Count == 0)
        return;
      ...
      valuesToSend.Add(new ElementValue() {
        ...
        Value = Encoding.UTF8.GetBytes(
            hasPower ? turretState == TurretState.Off ? "Off" : "On"
                     : "No Power")
      });

      valuesToSend.Add(new ElementValue() {
        Id = "WeaponsLabel",
        Value = Encoding.UTF8.GetBytes(CheckWeaponAuthorisation ? "Yes" : "No")
      });
      valuesToSend.Add(new ElementValue() {
        Id = "RecordLabel",
        Value = Encoding.UTF8.GetBytes(CheckSecurityRecords ? "Yes" : "No")
      });
      valuesToSend.Add(new ElementValue() {
        Id = "ArrestLabel",
        Value = Encoding.UTF8.GetBytes(CheckForArrest ? "Yes" : "No")
      });
      valuesToSend.Add(new ElementValue() {
        Id = "PersonnelLabel", Value = Encoding.UTF8.GetBytes(
                                   CheckUnauthorisedPersonnel ? "Yes" : "No")
      });
      valuesToSend.Add(new ElementValue() {
        Id = "LifeSignsLabel", Value = Encoding.UTF8.GetBytes(
                                   CheckUnidentifiedLifeSigns ? "Yes" : "No")
      });

      // Update all UI currently opened.
      TabUpdateMessage.SendToPeepers(gameObject, NetTabType.Turret,
                                     TabAction.Update, valuesToSend.ToArray());
    }
    ...
    public string Examine(Vector3 worldPos = default(Vector3)) {
      ...
      message.AppendLine(
          $"The turret is {(unlocked ? "unlocked" : "locked")}, use an authorised id to {(unlocked ? "lock" : "unlock")} it");

      message.AppendLine(
          "Turret must be unlocked to change connected APC or turret switch.");

      if (hasPower == false || turretState == TurretState.Off) {
        message.AppendLine("It is turned off");
      } else {
        message.AppendLine(
            $"It is set to {(turretState == TurretState.Stun ? "stun" : "<color=red>lethal</color>")}");
      }
    ...
    public bool CanOpenNetTab(GameObject playerObject, NetTabType netTabType) {
      if (turretType != TurretType.Ai && unlocked == false &&
          playerObject.GetComponent<PlayerScript>().PlayerState !=
              PlayerScript.PlayerStates.Ai) {
        Chat.AddExamineMsgFromServer(playerObject, "Turret is locked");
        return false;
      }

      // Only allow changing settings on non Ai turrets, as the settings only
      // work on those
...
#region Multitool Interaction
    ...
    MultitoolConnectionType IMultitoolLinkable.ConType =>
        MultitoolConnectionType.Turret;
    IMultitoolMasterable IMultitoolSlaveable.Master => connectedSwitch;
    bool IMultitoolSlaveable.RequireLink =>
        false; // TODO: set to false to ignore false positive; currently links
               // are serialized on the switch
    bool IMultitoolSlaveable.TrySetMaster(PositionalHandApply interaction,
                                          IMultitoolMasterable master) {
      if (unlocked == false) {
        Chat.AddExamineMsgFromServer(
            interaction.Performer,
            "You try to link the controller but the turret interface is locked!");
        return false;
      }
