...
public class Turret : NetworkBehaviour, ICheckedInteractable<HandApply>, IMultitoolSlaveable, IExaminable, IServerSpawn, ICanOpenNetTab
{
    ...
    public void UpdateGui()
    {
        ...
        valuesToSend.Add(new ElementValue() {
            Id = "PowerLabel", Value = Encoding.UTF8.GetBytes(
                                           hasPower ? turretState == TurretState.Off ? "Off" : "On" : "No Power")
        });

        valuesToSend.Add(new ElementValue() {
            Id = "WeaponsLabel", Value = Encoding.UTF8.GetBytes(CheckWeaponAuthorisation ? "Yes" : "No")
        });
        valuesToSend.Add(new ElementValue() {
            Id = "RecordLabel", Value = Encoding.UTF8.GetBytes(CheckSecurityRecords ? "Yes" : "No")
        });
        valuesToSend.Add(new ElementValue() {
            Id = "ArrestLabel", Value = Encoding.UTF8.GetBytes(CheckForArrest ? "Yes" : "No")
        });
        valuesToSend.Add(new ElementValue() {
            Id = "PersonnelLabel", Value = Encoding.UTF8.GetBytes(CheckUnauthorisedPersonnel ? "Yes" : "No")
        });
        valuesToSend.Add(new ElementValue() {
            Id = "LifeSignsLabel", Value = Encoding.UTF8.GetBytes(CheckUnidentifiedLifeSigns ? "Yes" : "No")
        });
