...
public class MatrixMove : ManagedNetworkBehaviour {
	...
	private void InitServerState()
	{
		if ( flyingDirection == Vector2.zero ) {
			Logger.LogWarning($"{gameObject.name} move direction unclear",Categories.Movement);
...
//		Logger.Log("Stopped movement");
	...
	public void SetSpeed( float absoluteValue ) {
		...
		if ( absoluteValue > maxSpeed ) {
			Logger.LogWarning($"MaxSpeed {maxSpeed} reached, not going further",Categories.Movement);
	...
	private void CheckMovementServer()
	{
		...
		if ( isMovingServer ) {
			...
			if ( !SafetyProtocolsOn || CanMoveTo( goal ) ) {
				...
				Logger.Log( "Stopping due to safety protocols!",Categories.Movement );
				StopMovement();
			}
		}
	}
