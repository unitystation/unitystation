...
public class MatrixMove : ManagedNetworkBehaviour {
	...
	private void InitServerState()
	{
		if ( flyingDirection == Vector2.zero ) {
			TADB_Debug.LogWarning($"{gameObject.name} move direction unclear",TADB_Debug.Category.Movement.ToString());
...
//		TADB_Debug.Log( $"Calculated pivot {pivot} for {gameObject.name}" );
...
//		TADB_Debug.Log($"Started moving with speed {serverTargetState.Speed}");
...
//		TADB_Debug.Log("Stopped movement");
	...
	public void SetSpeed( float absoluteValue ) {
		...
		if ( absoluteValue > maxSpeed ) {
			TADB_Debug.LogWarning($"MaxSpeed {maxSpeed} reached, not going further",TADB_Debug.Category.Movement.ToString());
	...
	private void CheckMovementServer()
	{
		...
		if ( isMovingServer ) {
			...
			if ( !SafetyProtocolsOn || CanMoveTo( goal ) ) {
				...
				TADB_Debug.Log( "Stopping due to safety protocols!",TADB_Debug.Category.Movement.ToString() );
				StopMovement();
			}
		}
	}
...
//		TADB_Debug.Log($"Orientation is now {serverTargetState.Orientation}, Corrected direction from {serverTargetState.Direction} to {newDirection}");
