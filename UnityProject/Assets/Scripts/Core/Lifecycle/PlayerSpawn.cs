...
public static class PlayerSpawn {
  ...
  /// <summary>
  /// Server-side only. For use when a player has only joined (as a
  /// JoinedViewer) and is not in control of any mobs. Spawns the joined viewer
  /// as the indicated occupation and transfers control to it. Note that this
  /// doesn't take into account game mode or antags, it just spawns whatever is
  /// requested.
  /// </summary>
  ...
  ServerSpawnPlayer(PlayerSpawnRequest request, JoinedViewer joinedViewer,
                    Occupation occupation, CharacterSettings characterSettings,
                    bool showBanner = true, Vector3Int? spawnPos = null,
                    Mind existingMind = null, NetworkConnection conn = null) {
    ...
    var newPlayer =
        ServerSpawnInternal(conn, occupation, characterSettings, existingMind,
                            showBanner: showBanner, spawnPos: spawnPos);
    if (newPlayer != null && occupation.IsCrewmember) {
      CrewManifestManager.Instance.AddMember(
          newPlayer.GetComponent<PlayerScript>(), occupation.JobType);
    }
  ...
  private static bool ValidateCharacter(PlayerSpawnRequest request) {
    ...
    */
    if (ServerValidations.HasIllegalCharacterAge(
            request.CharacterSettings.Age)) {
  ...
  private static void ValidateFail(JoinedViewer joinedViewer, string userId,
                                   string message) {
  ...
  public static GameObject ServerSpawnPlayer(PlayerSpawnRequest spawnRequest) {
    return ServerSpawnPlayer(spawnRequest, spawnRequest.JoinedViewer,
                             spawnRequest.RequestedOccupation,
  ...
  public static void ServerRespawnPlayer(Mind forMind,
    ...
    ServerSpawnInternal(connection, occupation, settings, forMind,
                        willDestroyOldBody: oldGhost != null,
                        spawnPos: Position);
  ...
  /// <summary>
  /// For use when a player is alive or dead and you want to clone their body
  /// and transfer their control to it.
  ///
  /// Clones a given mind's current body to a new body and transfers control of
  /// the mind's connection to that new body.
  /// </summary>
  /// <param name="forMind"></param>
  /// <param name="worldPosition"></param>
  public static GameObject ServerClonePlayer(Mind forMind,
    ...
    return ServerSpawnInternal(connection, occupation, settings, forMind,
                               worldPosition, false, showBanner: false);
  }

  // Time to start spawning players at arrivals
  private static readonly DateTime ARRIVALS_SPAWN_TIME =
      new DateTime().AddHours(12).AddMinutes(2);

  /// <summary>
  /// Spawns a new player character and transfers the connection's control into
  /// the new body. If existingMind is null, creates the new mind and assigns it
  /// to the new body.
  ///
  /// Fires server and client side player spawn hooks.
  /// </summary>
  /// <param name="connection">connection to give control to the new player
  /// character</param> <param name="occupation">occupation of the new player
  /// character</param> <param name="characterSettings">settings of the new
  /// player character</param> <param name="existingMind">existing mind to
  /// transfer to the new player, if null new mind will be created and assigned
  /// to the new player character</param> <param name="spawnPos">world position
  /// to spawn at</param> <param name="spawnItems">If spawning a player, should
  /// the player spawn without the defined initial equipment for their
  /// occupation?</param> <param name="willDestroyOldBody">if true, indicates
  /// the old body is going to be destroyed rather than pooled, thus we
  /// shouldn't send any network message which reference's the old body's ID
  /// since it won't exist.</param>
  ///
  /// <returns>the spawned object</returns>
  private static GameObject
  ServerSpawnInternal(NetworkConnection connection, Occupation occupation,
                      CharacterSettings characterSettings, Mind existingMind,
                      Vector3Int? spawnPos = null, bool spawnItems = true,
                      bool willDestroyOldBody = false, bool showBanner = true) {
    ...
    if (spawnPos == null) {
      ...
      if (GameManager.Instance.stationTime < ARRIVALS_SPAWN_TIME ||
          occupation.LateSpawnIsArrivals == false) {
      ...
      if (spawnTransform == null) {
        Logger.LogErrorFormat(
            ...
            Category.EntitySpawn, connection.address, occupation.DisplayName);
        return null;
      }
    ...
    var newPlayer = ServerCreatePlayer(spawnPos.GetValueOrDefault(),
                                       occupation.SpecialPlayerPrefab);
    ...
    ServerTransferPlayer(connection, newPlayer, oldBody, Event.PlayerSpawned,
                         characterSettings, willDestroyOldBody);
    ...
    // fire all hooks
    var info =
        SpawnInfo.Player(occupation, characterSettings,
                         CustomNetworkManager.Instance.humanPlayerPrefab,
                         SpawnDestination.At(spawnPos), spawnItems: spawnItems);
    Spawn._ServerFireClientServerSpawnHooks(
        SpawnResult.Single(info, newPlayer));

    if (occupation != null && showBanner) {
      SpawnBannerMessage.Send(newPlayer, occupation.DisplayName,
                              occupation.SpawnSound.AssetAddress,
                              occupation.TextColor, occupation.BackgroundColor,
  ...
  /// <summary>
  /// Use this when a player is currently a ghost and wants to reenter their
  /// body.
  /// </summary>
  /// <param name="forConnection">connection to transfer control to</param>
  /// TODO: Remove need for this parameter
  /// <param name="forConnection">object forConnection is currently in control
  /// of</param> <param name="forMind">mind to transfer control back into their
  /// body</param>
  public static void ServerGhostReenterBody(NetworkConnection forConnection,
    ...
    ServerTransferPlayer(forConnection, body, fromObject, Event.PlayerSpawned,
                         settings, oldGhost != null);
  ...
  /// <summary>
  /// Use this when a player rejoins the game and already has a logged-out body
  /// in the game. Transfers their control back to the body.
  /// </summary>
  ...
  public static void ServerRejoinPlayer(JoinedViewer viewer, GameObject body) {
    ...
    var occupation = mind.occupation;
    var settings = ps.characterSettings;
    ServerTransferPlayer(viewer.connectionToClient, body, viewer.gameObject,
                         Event.PlayerRejoined, settings);
  ...
  /// <summary>
  /// Spawns a ghost for the indicated mind's body and transfers the
  /// connection's control to it.
  /// </summary>
  ...
  public static void ServerSpawnGhost(Mind forMind) {
    ...
    if (registerTile == null) {
      Logger.LogErrorFormat(
          "Cannot spawn ghost for body {0} because it has no registerTile",
          Category.Ghosts, body.name);
      return;
    }
    ...
    if (spawnPosition == TransformState.HiddenPos) {
      // spawn ghost at occupation location if we can't determine where their
      // body is
      Transform spawnTransform =
          SpawnPoint.GetRandomPointForJob(forMind.occupation.JobType, true);
      if (spawnTransform == null) {
        Logger.LogErrorFormat(
            "Unable to determine spawn position for occupation {1}. Cannot spawn ghost.",
            Category.Ghosts, forMind.occupation.DisplayName);
        return;
      }
    ...
    // using parentTransform.rotation rather than Quaternion.identity because
    // objects should always be upright w.r.t.  localRotation, NOT world
    // rotation
    var ghost = UnityEngine.Object.Instantiate(
        CustomNetworkManager.Instance.ghostPrefab, spawnPosition,
    ...
    // fire all hooks
    var info = SpawnInfo.Ghost(
        forMind.occupation, settings, CustomNetworkManager.Instance.ghostPrefab,
    ...
    if (PlayerList.Instance.IsAdmin(forMind.ghost.connectedPlayer)) {
      var adminItemStorage = AdminManager.Instance.GetItemSlotStorage(
          forMind.ghost.connectedPlayer);
  ...
  public static void ServerSpawnGhost(JoinedViewer joinedViewer,
    ...
    Vector3Int spawnPosition =
        SpawnPoint.GetRandomPointForJob(JobType.ASSISTANT)
            .transform.position.CutToInt();

    // Get spawn location
    var matrixInfo = MatrixManager.AtPoint(spawnPosition, true);
    var parentTransform = matrixInfo.Objects;
    var newPlayer = UnityEngine.Object.Instantiate(
        CustomNetworkManager.Instance.ghostPrefab, spawnPosition,
        parentTransform.rotation, parentTransform);

    // Create the mind without a job refactor this to make it as a ghost mind
    Mind.Create(newPlayer);
    ServerTransferPlayer(joinedViewer.connectionToClient, newPlayer, null,
                         Event.GhostSpawned, characterSettings);

    if (PlayerList.Instance.IsAdmin(
            PlayerList.Instance.Get(joinedViewer.connectionToClient))) {
  ...
  public static void ServerSpawnDummy(Transform spawnTransform = null) {
    ...
    if (spawnTransform != null) {
      ...
      CharacterSettings randomSettings =
          CharacterSettings.RandomizeCharacterSettings();

      ServerTransferPlayer(null, dummy, null, Event.PlayerSpawned,
                           randomSettings);

      // fire all hooks
      var info = SpawnInfo.Player(
          OccupationList.Instance.Get(JobType.ASSISTANT), randomSettings,
          CustomNetworkManager.Instance.humanPlayerPrefab,
  ...
  /// <summary>
  /// Server-side only. Creates the player object on the server side and fires
  /// server-side spawn hooks. Doesn't transfer control to the client yet.
  /// Client side hooks should be fired after client has been informed of the
  /// spawn
  /// </summary>
  ...
  private static GameObject ServerCreatePlayer(Vector3Int spawnWorldPosition,
    ...
    // objects should always be upright w.r.t.  localRotation, NOT world
    // rotation
    var player = UnityEngine.Object.Instantiate(
        playerPrefab, spawnPosition, parentTransform.rotation, parentTransform);

    return player;
  }

  public static void
  ServerTransferPlayerToNewBody(NetworkConnection conn, GameObject newBody,
                                GameObject oldBody, Event eventType,
                                CharacterSettings characterSettings,
                                bool willDestroyOldBody = false) {
    ServerTransferPlayer(conn, newBody, oldBody, eventType, characterSettings,
                         willDestroyOldBody);
  }

  /// <summary>
  /// Server-side only. Transfers control of a player object to the indicated
  /// connection.
  /// </summary>
  /// <param name="conn">The client's NetworkConnection. If logged out the
  /// playerlist will return an invalid connectedplayer</param> <param
  /// name="newBody">The character gameobject to be transfered into.</param>
  /// <param name="oldBody">The old body of the character.</param>
  /// <param name="eventType">Event type for the player sync.</param>
  /// <param name="characterSettings">settings, ignored if transferring to an
  /// existing player body</param> <param name="willDestroyOldBody">if true,
  /// indicates the old body is going to be destroyed rather than pooled, thus
  /// we shouldn't send any network message which reference's the old body's ID
  /// since it won't exist.</param>
  private static void ServerTransferPlayer(NetworkConnection conn,
                                           GameObject newBody,
                                           GameObject oldBody, Event eventType,
                                           CharacterSettings characterSettings,
                                           bool willDestroyOldBody = false) {
    if (oldBody) {
      var oldPlayerNetworkActions =
          oldBody.GetComponent<PlayerNetworkActions>();
      if (oldPlayerNetworkActions) {
        oldPlayerNetworkActions.RpcBeforeBodyTransfer();
      }

      // no longer can observe their inventory
      oldBody.GetComponent<DynamicItemStorage>()?.ServerRemoveObserverPlayer(
          oldBody);
    }

    var netIdentity = newBody.GetComponent<NetworkIdentity>();
    if (netIdentity.connectionToClient != null) {
      CustomNetworkManager.Instance.OnServerDisconnect(
          netIdentity.connectionToClient);
    }

    var connectedPlayer = PlayerList.Instance.Get(conn);
    if (connectedPlayer ==
        ConnectedPlayer.Invalid) // this isn't an online player
    {
      ...
      // NOTE: With mirror upgrade 04 Feb 2020, it appears we no longer need to
      // do what has been commented out below. Below appears to have been an
      // attempt to give authority back to server But it's implicitly given such
      // authority by the ReplacePlayerForConnection call - that call now
      // removes authority for the player's old object
      //  if (oldBody)
      //  {
      //  	NetworkServer.ReplacePlayerForConnection(new
      //  NetworkConnectionToClient(0), oldBody);
      //  }
    ...
    // The ClosetHandlerMessage will attach the container to the transfered
    // player.
    ...
    if (playerObjectBehavior && playerObjectBehavior.parentContainer) {
      FollowCameraMessage.Send(newBody,
                               playerObjectBehavior.parentContainer.gameObject);
    }
    ...
    if (characterSettings != null) {
      ...
      playerScript.playerName =
          playerScript.PlayerState != PlayerScript.PlayerStates.Ai
