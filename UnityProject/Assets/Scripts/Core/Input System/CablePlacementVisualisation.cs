...
public class CablePlacementVisualisation : MonoBehaviour
{
	...
	public void OnHover()
	{
		if (!UIManager.IsMouseInteractionDisabled && PlayerManager.LocalPlayerScript?.ItemStorage?.GetActiveHandSlot() != null)
		{
			// get mouse position
			...
			if (mousePosition != lastMouseWordlPositionInt
			    && Validations.HasItemTrait(PlayerManager.LocalPlayerScript.OrNull()?.ItemStorage.OrNull()?.GetActiveHandSlot()?.ItemObject, CommonTraits.Instance.Cable))
			{
