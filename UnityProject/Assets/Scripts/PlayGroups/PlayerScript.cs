...
namespace PlayGroup
{
    public class PlayerScript : ManagedNetworkBehaviour
    {
        ...
        public override void UpdateMe()
        {
            ...
            {
                pingUpdate = 0f;
                var ping = CustomNetworkManager.Instance.client.GetRTT();
        ...
        public ChatChannel GetAvailableChannels(bool transmitOnly = true)
        {
            ...
            var headset = UIManager.InventorySlots.EarSlot.Item;
            if (headset)
            {
                var key = headset.GetComponent<Headset>().EncryptionKey;
                transmitChannels = transmitChannels | EncryptionKey.Permissions[key];
            }
            var receiveChannels = ChatChannel.Examine | ChatChannel.System;
