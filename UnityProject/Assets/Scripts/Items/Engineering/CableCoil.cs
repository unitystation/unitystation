...
namespace Objects.Electrical {
  ...
  public class CableCoil : NetworkBehaviour,
                           ICheckedInteractable<ConnectionApply> {
    ...
    /// </summary>
    private const string MSG_BUILD_CONFLICT =
        "There is already a cable at that position.";
    ...
    public bool WillInteract(ConnectionApply interaction, NetworkSide side) {
      ...
      if (!Validations.HasComponent<InteractableTiles>(
              interaction.TargetObject))
        return false;
      // If there's a table, we should drop there
      if (MatrixManager.IsTableAt(
              Vector3Int.RoundToInt(interaction.WorldPositionTarget),
    ...
    public void ServerPerformInteraction(ConnectionApply interaction) {
      ...
      if (cableCoil != null) {
        Vector3Int worldPosInt =
            Vector3Int.RoundToInt(interaction.WorldPositionTarget);
        MatrixInfo matrixInfo = MatrixManager.AtPoint(worldPosInt, true);
        Vector3Int localPosInt =
            MatrixManager.WorldToLocalInt(worldPosInt, matrixInfo);
        Matrix matrix = matrixInfo?.Matrix;

        // if there is no matrix or IsClearUnderfloor == false - return
        if (matrix == null ||
            matrix.IsClearUnderfloorConstruction(localPosInt, true) == false) {
          return;
        }
        ...
        if (IsLegalConnection(wireEndA, wireEndB)) {
          ...
          foreach (IntrinsicElectronicData eConnI in eConnList) {
            if (eConnI.WireEndA == Connection.Overlap ||
                eConnI.WireEndB == Connection.Overlap) {
              ...
              foreach (IntrinsicElectronicData eConnJ in eConnList) {
                if (eConnJ.WireEndA == wireEndB ||
                    eConnJ.WireEndB == wireEndB) {
                  if (eConnI.WireEndA == eConnJ.WireEndA ||
                      eConnI.WireEndB == eConnJ.WireEndA) {
                    MsgAndAddToPool(eConnList, MSG_BUILD_CONFLICT);
                    return;
                  } else if (eConnI.WireEndA == eConnJ.WireEndB ||
                             eConnI.WireEndB == eConnJ.WireEndB) {
                    MsgAndAddToPool(eConnList, MSG_BUILD_CONFLICT);
                    return;
                  }
                }
    ...
    /// </summary>
    /// <param name="eConnList">List of electrical connections. Will not add to
    /// electrical pool if null.</param> <param name="addMsg">Message to show as
    /// an examine message. Will not show anything if null.</param>
    private void MsgAndAddToPool(List<IntrinsicElectronicData> eConnList,
                                 string msg) {
      if (msg != null) {
    ...
    private void BuildCable(Vector3 position, Connection wireEndA,
    ...
    /// connection.
    /// </summary>
    /// <param name="position">Position of the cell we're building the cable
    /// at.</param> <param name="wireEndA">The first connection we're building
    /// from.</param> <param name="wireEndB">The second connection we're
    /// building to.</param> <param name="interaction">Object to allow us to
    /// interact with the server.</param>
    public void FindOverlapsAndCombine(Vector3 position, Connection wireEndA,
                                       Connection wireEndB,
      ...
      if (wireEndA == Connection.Overlap || wireEndB == Connection.Overlap) {
        ...
        List<IntrinsicElectronicData> IEnumerableEconns =
            interaction.Performer.RegisterTile()
                .Matrix.GetElectricalConnections(position.RoundToInt());
        List<IntrinsicElectronicData> eConns =
            new List<IntrinsicElectronicData>(IEnumerableEconns);
          ...
          for (int i = 0; i < eConns.Count; i++) {
            ...
            if (powerTypeCategory == eConns[i].Categorytype) {
              ...
              if (eConns[i].WireEndA == Connection.Overlap) {
                ...
                ReplaceEConn(position, eConns[i], wireEndA, wireEndB,
                             interaction);
                return;
              }
    ...
    private void ReplaceEConn(Vector3 position, IntrinsicElectronicData eConn,
                              Connection wireEndA, Connection wireEndB,
                              ConnectionApply interaction) {
      ...
      if (eConn != null) {
        oldTileCost =
            eConn.MetaDataPresent.RelatedTile.SpawnAmountOnDeconstruct;
      ...
      interaction.Performer.GetComponentInParent<Matrix>().AddElectricalNode(
          position.RoundToInt(), tile, true);
