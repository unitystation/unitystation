...
public class PlaceableTile : MonoBehaviour,
  ...
  [NonSerialized]
  public LayerTypeSelection layerTypeSelection =
      LayerTypeSelection.Underfloor | LayerTypeSelection.Effects;
  ...
  public bool WillInteract(PositionalHandApply interaction, NetworkSide side) {
    ...
    // it's annoying to place something when you're trying to pick up instead to
    // add to your current stack, so if it's possible to stack what is being
    // targeted with what's in your hand, we will defer to that
    ...
    var interactableTiles =
        InteractableTiles.GetAt(interaction.WorldPositionTarget, side);
    var tileAtPosition = interactableTiles.LayerTileAt(
        interaction.WorldPositionTarget, layerTypeSelection);

    foreach (var entry in waysToPlace) {
      // open space
      if (tileAtPosition == null && entry.placeableOn == LayerType.None &&
          entry.placeableOnlyOnTile == null) {
        return true;
      }
      // placing on an existing tile
      else if (tileAtPosition.LayerType == entry.placeableOn &&
               (entry.placeableOnlyOnTile == null ||
                entry.placeableOnlyOnTile == tileAtPosition)) {
        return true;
      }
    }
    return false;
  ...
  public void ServerPerformInteraction(PositionalHandApply interaction) {
    ...
    var matrix = InteractableTiles.TryGetNonSpaceMatrix(
        interaction.WorldPositionTarget.RoundToInt(), true);
    ...
    Vector3Int cellPos =
        interactableTiles.WorldToCell(interaction.WorldPositionTarget);
    var tileAtPosition = interactableTiles.LayerTileAt(
        interaction.WorldPositionTarget, layerTypeSelection);
    ...
    if (placeableTileEntry != null) {
      ...
      void ProgressFinishAction() {
        interactableTiles.TileChangeManager.MetaTileMap.SetTile(
            cellPos, placeableTileEntry.layerTile);
        interactableTiles.TileChangeManager.SubsystemManager.UpdateAt(cellPos);
        Inventory.ServerConsume(interaction.HandSlot,
                                placeableTileEntry.itemCost);
      ...
      var bar =
          StandardProgressAction.Create(ProgressConfig, ProgressFinishAction)
              .ServerStartProgress(targetPosition,
                                   placeableTileEntry.timeToPlace, performer);
    }
  }

  /// <summary>
  /// Some tile items can be placed on multiple things, so this defines one way
  /// in which an item can be placed
  /// </summary>
  [Serializable]
  private class PlaceableTileEntry {
    ...
    [Tooltip(
        "What layer this can be placed on top of. Choose None to allow placing on empty space.")]
    [SerializeField]
    public LayerType placeableOn = LayerType.Base;

    [Tooltip(
        "Particular tile this is placeable on. Leave empty to allow placing on any tile.")]
    [SerializeField]
