	...
	public class PowerSink : SignalReceiver, ICheckedInteractable<HandApply>, IExaminable
	{
		...
		public void ServerPerformInteraction(HandApply interaction)
		{
			...
			if (interaction.UsedObject.TryGetComponent<RemoteSignaller>(out var signaller))
			{
				...
				Chat.AddExamineMsg(interaction.Performer, $"You pair the {interaction.UsedObject.DisplayName()} to this device.");
			}
		}
		...
		private void Anchor()
		{
			...
			Chat.AddActionMsgToChat(gameObject, $"The {gameObject.DisplayName()} makes a clicking sound as it <b>anchors</b> to the ground.");
		}
		private void UnAnchor()
		{
			...
			ElectricalManager.Instance.electricalSync.StructureChange = true;
			UpdateManager.Remove(CallbackType.PERIODIC_UPDATE, CheckForVoltage);
			Chat.AddActionMsgToChat(gameObject, $"The {gameObject.DisplayName()} makes a clicking sound as it <b>unanchors</b> from the ground.");
		}
