...
namespace Items.PDA
{
	...
	public class PDALogic : NetworkBehaviour, ICheckedInteractable<HandApply>, ICheckedInteractable<InventoryApply>, IServerInventoryMove
	{
		...
		[SerializeField, BoxGroup("Settings")]
		private AddressableAudioSource defaultRingtone = default;
		...
		private bool WillInsert(GameObject item, NetworkSide side)
		{
			...
			if (item.TryGetComponent<IDCard>(out _) && Validations.CanFit(IDSlot, item, side, true))
			{
				return true;
			}

			if (item.TryGetComponent<PDACartridge>(out _) && Validations.CanFit(CartridgeSlot, item, side, true))
			{
				return true;
			}
		...
		private void ServerInsertItem(GameObject item, ItemSlot fromSlot, GameObject player)
		{
			...
			else if (Validations.HasItemTrait(item, telecrystalTrait))
			{
				if (IsUplinkLocked == false)
				{
					...
					var uplinkMessage =
						$"You press the Telecrystal{(quantity == 1 ? "" : "s")} into the screen of your {gameObject.ExpensiveName()}\n" +
