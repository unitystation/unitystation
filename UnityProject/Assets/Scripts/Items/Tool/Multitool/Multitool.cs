...
namespace Items.Engineering {
  public class Multitool : MonoBehaviour,
    ...
    private MultitoolConnectionType configurationBuffer =
        MultitoolConnectionType.Empty;

    private readonly List<IMultitoolMasterable> buffers =
        new List<IMultitoolMasterable>();
    private IMultitoolMasterable Buffer => buffers.Count > 0? buffers[0] : null;

    public bool WillInteract(PositionalHandApply interaction,
                             NetworkSide side) {
    ...
    public void ServerPerformInteraction(PositionalHandApply interaction) {
      if (interaction.TargetObject != null) {
        var multitoolBases =
            interaction.TargetObject.GetComponents<IMultitoolLinkable>();
        foreach (var multitoolBase in multitoolBases) {
          ...
          if (Vector3.Distance(slaveComponent.transform.position,
    ...
    public void PrintElectricalThings(PositionalHandApply interaction) {
      Vector3Int worldPosInt =
          interaction.WorldPositionTarget.To2Int().To3Int();
      ...
      bool deviceFound = interaction.TargetObject != null &&
                         interaction.TargetObject.TryGetComponent(out device);

      StringBuilder sb = new StringBuilder(
          "The multitool couldn't find anything electrical here.");
    ...
    public void ServerPerformInteraction(HandActivate interaction) {
      Chat.AddExamineMsgFromServer(
          interaction.Performer, "You clear the multitool's internal buffer.");
      buffers.Clear();
