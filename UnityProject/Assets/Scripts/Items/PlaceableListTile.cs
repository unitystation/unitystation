...
/// Same as PlaceableTile but with PlaceableTileListEntry instead of PlaceableTileEntry
/// (one way to place can contain multiple tiles to place on)
/// </summary>
public class PlaceableListTile : MonoBehaviour, ICheckedInteractable<PositionalHandApply>
{
	[NonSerialized]
	...
	[SerializeField]
	private List<PlaceableTileListEntry> waysToPlace = null;
	...
	public bool WillInteract(PositionalHandApply interaction, NetworkSide side)
	{
		...
		foreach (var entry in waysToPlace)
		{
			...
			// placing on an existing tile
					 ...
					 (entry.placeableOnlyOnTile == null || entry.placeableOnlyOnTile.Contains(tileAtPosition)))
			{
				return true;
			}
		}
	...
	public void ServerPerformInteraction(PositionalHandApply interaction)
	{
		...
		PlaceableTileListEntry placeableTileEntry = null;
		...
		foreach (var entry in waysToPlace)
		{
			...
			// placing on an existing tile
			else if (tileAtPosition.LayerType == entry.placeableOn && (entry.placeableOnlyOnTile == null || entry.placeableOnlyOnTile.Contains(tileAtPosition)))
			{
				placeableTileEntry = entry;
				break;
			}
	...
	/// </summary>
	[Serializable]
	private class PlaceableTileListEntry
	{
		...
		[SerializeField]
		public List<LayerTile> placeableOnlyOnTile = null;
