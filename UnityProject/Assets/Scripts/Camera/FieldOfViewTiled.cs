...
public class FieldOfViewTiled : ThreadedBehaviour
{
    ...
    public void UpdateSightSourceFov()
    {
        ...
        for (var i = nearbyShroudsInWorkerThread.Count; i-- > 0;)
        {
        ...
        for (var i = inFieldOFVision.Count; i-- > 0;)
        {
            ...
            var offsetPos = ShroudCornerOffset(Angle(((Vector2) sourcePosCache - inFieldOFVision[i]).normalized));
    ...
    {
        var hit = Physics2D.Linecast(Camera2DFollow.followControl.target.position, endPos, _layerMask);
        // If it hits a wall we should enable the shroud
        //		Debug.DrawLine(GetPlayerSource().transform.position, endPos,Color.red);
        if (hit)
        {
            var dist = Vector2.Distance(hit.point, endPos);
            if (dist > 0.5f)
            {
    ...
    {
        var shroudObject = EffectsFactory.Instance.SpawnShroudTile(new Vector3(vector2.x, vector2.y, 0));
    ...
    public ReadOnlyCollection<Vector2> GetNearbyShroudTiles()
    {
        ...
        for (var offsetx = -MonitorRadius; offsetx <= MonitorRadius; offsetx++)
        {
            for (var offsety = -MonitorRadius; offsety <= MonitorRadius; offsety++)
            {
                var x = (int) sourcePosCache.x + offsetx;
                var y = (int) sourcePosCache.y + offsety;
