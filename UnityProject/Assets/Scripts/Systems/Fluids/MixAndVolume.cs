...
namespace Pipes
{
	...
	public class MixAndVolume
	{
		...
		public Tuple<ReagentMix, GasMix> Take(MixAndVolume inMixAndVolume, bool removeVolume = true)
		{
			...
			{
				Logger.LogError("Tried to take from pipe but its volume was 0!", Category.Unknown);
			}
		...
		public void Divide(float divideAmount, bool changeVolume = true)
		{
			...
			{
				Logger.LogError("Tried to divide pipe contents, but the amount to divide by was 0!", Category.Unknown);
			}
			...
			var newOne = new float[gasMix.Gases.Length];
			for (var i = 0; i < gasMix.Gases.Length; i++)
			{
		...
		public void EqualiseWith(PipeData another, bool equaliseGas, bool equaliseLiquid)
		{
			...
			if (equaliseLiquid)
			{
				...
				{
					Logger.LogError("Tried to equalise two pipes, but their total volume was 0!", Category.Unknown);
				}
		...
		public void EqualiseWithMultiple(List<PipeData> others, bool equaliseGas, bool equaliseLiquid)
		{
			...
			if (equaliseLiquid)
			{
				...
				if (CodeUtilities.IsEqual(totalVolume, 0))
				{
					Logger.LogError("Tried to equalise multiple pipes, but their total volume was 0!", Category.Unknown);
				}
