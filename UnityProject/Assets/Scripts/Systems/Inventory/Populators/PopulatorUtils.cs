...
public static class PopulatorUtils
{
	...
	public static Occupation TryGetOccupation(PopulationContext context)
	{
		...
		{
			Loggy.Error("PopulationContext does not have any SpawnInfo, ID cannot be auto populated. Please" +
		...
		{
			Loggy.Error().Format("PopulationContext SpawnInfo does not have a SpawnType of Player. Auto ID population" +
		...
		if (occupation == null)
		{
			Loggy.Error().Format("Unable to get occupation from spawn info, this is likely a bug because" +
