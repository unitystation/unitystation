...
public static class SweetExtensions
{
	...
	{
		return go == null
			? global::NetId.Invalid
			: go.GetComponent<Matrix>()
				? go.GetComponentInChildren<NetworkIdentity>().netId 
				: go.GetComponent<NetworkIdentity>().netId; //maxValue is invalid (see NetId.cs)
	}
