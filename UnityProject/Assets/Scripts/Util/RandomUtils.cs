...
public static class RandomUtils {
  public static Quaternion RandomRotation2D() {
    ...
    var randomRotation =
        Quaternion.AngleAxis(UnityEngine.Random.Range(-180f, 180f), axis);

    return randomRotation;
  }

  // Courtesy of
  // https://answers.unity.com/questions/856819/generate-a-random-point-with-an-anullus.html
  /// <summary>
  /// Gets a random point in an annulus within the given minimum and maximum
  /// radius.
  /// </summary>
  public static Vector3 RandomAnnulusPoint(float minRadius, float maxRadius) {
    ...
    return randomVector.normalized * minRadius +
           randomVector * (maxRadius - minRadius);
  }
  ...
  GetRandomPointOnStation(bool avoidSpace = false,
                          bool avoidImpassable = false) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      if (avoidSpace &&
          MatrixManager.IsSpaceAt(point, CustomNetworkManager.IsServer,
                                  stationMatrix)) {
        continue;
      }

      if (avoidImpassable &&
          MatrixManager.IsPassableAtAllMatricesOneTile(
              point, CustomNetworkManager.IsServer) == false) {
        continue;
      }
