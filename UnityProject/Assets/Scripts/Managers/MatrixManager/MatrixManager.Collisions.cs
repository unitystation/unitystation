...
public partial class MatrixManager {
  ...
  /// <summary>
  /// Currently moving matrices. Should be monitored for new intersections with
  /// other matrix bounds
  /// </summary>
  private List<MatrixInfo> movingMatrices = new List<MatrixInfo>();

  /// <summary>
  /// bounds intersections that should be actively checked for actual tile
  /// collisions
  /// </summary>
  private List<MatrixIntersection> trackedIntersections =
      new List<MatrixIntersection>();
  ...
  private static LayerType[] effectsToRemove = { LayerType.Effects,
                                                 LayerType.Grills,
                                                 LayerType.Floors };
  ...
  private void InitCollisions(MatrixInfo matrixInfo) {
    ...
    if (matrixInfo?.MatrixMove != null) {
      ...
      matrixInfo.MatrixMove.MatrixMoveEvents.OnStopMovementServer.AddListener(
          () => {
            if (movingMatrices.Contains(matrixInfo)) {
              var participatingIntersections = trackedIntersections.FindAll(
                  intersection => intersection.Matrix1 == matrixInfo);
              matrixInfo.MatrixMove.MatrixMoveEvents.OnFullStopClient
                  .AddListener(CollideBeforeStop(matrixInfo,
                                                 participatingIntersections));
                  movingMatrices.Remove(matrixInfo);
                  trackedIntersections.RemoveAll(
                      intersection => intersection.Matrix1 == matrixInfo);
            }
          });
    }

    UnityAction CollideBeforeStop(MatrixInfo matrix,
                                  List<MatrixIntersection> intersections) {
      return () => {
  ...
  private void RefreshIntersections() {
    ...
    void UpdateAndPruneIntersections() {
      ...
      foreach (var trackedIntersection in trackedIntersections) {
        if (trackedIntersection.Matrix1.BoundsIntersect(
                trackedIntersection.Matrix2, out var hotZone)) { // refresh rect
          if (toUpdate == null) {
            toUpdate = new List<MatrixIntersection>();
          }

          toUpdate.Add(trackedIntersection.Clone(ref hotZone));
        } else { // stop tracking non-intersecting ones
          if (toRemove == null) {
  ...
  private static readonly MatrixIntersection[] noIntersections =
      new MatrixIntersection[0];
  ...
  private void CheckTileCollisions(MatrixIntersection i) {
    ...
    foreach (var worldPos in i.Rect.allPositionsWithin()) {
      ...
      if (resistance1 + (10 * i.Matrix1.Speed) >=
          resistance2 + (10 * i.Matrix2.Speed)) {
        // attacker tile is stronger:

        // destroy victim tile
        TryPushing(i.Matrix2, cellPos2, i.Matrix1.MovementVector,
                   i.Matrix1.Speed);
        ApplyCritDamage(i.Matrix2, cellPos2, worldPos);

        // slightly damage adjacent victim tiles
        ApplyTilemapDamage(i.Matrix2, cellPos2 + Vector3Int.up,
                           resistance1 * 0.4f, worldPos);
        ApplyTilemapDamage(i.Matrix2, cellPos2 + Vector3Int.down,
                           resistance1 * 0.4f, worldPos);
        ApplyTilemapDamage(i.Matrix2, cellPos2 + Vector3Int.left,
                           resistance1 * 0.4f, worldPos);
        ApplyTilemapDamage(i.Matrix2, cellPos2 + Vector3Int.right,
                           resistance1 * 0.4f, worldPos);

        // damage back attacker
        TryPushing(i.Matrix1, cellPos1, i.Matrix2.MovementVector,
                   i.Matrix2.Speed);
        ApplyDamage(i.Matrix1, cellPos1, resistance2, worldPos);
        if (resistance2 > EXTRA_COLLISION_THRESHOLD) {
          collisions += (byte)((resistance2 - EXTRA_COLLISION_THRESHOLD) /
                               EXTRA_COLLISION_THRESHOLD);
        }
        ...
        ApplyTilemapDamage(i.Matrix1, cellPos1 + Vector3Int.up,
                           resistance2 * 0.4f, worldPos);
        ApplyTilemapDamage(i.Matrix1, cellPos1 + Vector3Int.down,
                           resistance2 * 0.4f, worldPos);
        ApplyTilemapDamage(i.Matrix1, cellPos1 + Vector3Int.left,
                           resistance2 * 0.4f, worldPos);
        ApplyTilemapDamage(i.Matrix1, cellPos1 + Vector3Int.right,
                           resistance2 * 0.4f, worldPos);
        ...
        if (resistance1 > EXTRA_COLLISION_THRESHOLD) {
          collisions += (byte)((resistance1 - EXTRA_COLLISION_THRESHOLD) /
                               EXTRA_COLLISION_THRESHOLD);
        }
      }
      ...
      foreach (var layer in layersToRemove) {
        i.Matrix1.TileChangeManager.MetaTileMap.RemoveTileWithlayer(cellPos1,
                                                                    layer);
        i.Matrix2.TileChangeManager.MetaTileMap.RemoveTileWithlayer(cellPos2,
                                                                    layer);
      }
      foreach (var layer in effectsToRemove) {
        i.Matrix1.TileChangeManager.MetaTileMap.RemoveAllOverlays(cellPos1,
                                                                  layer);
        i.Matrix2.TileChangeManager.MetaTileMap.RemoveAllOverlays(cellPos2,
                                                                  layer);
      }
    }
    ...
    if (collisions > 0) {
      ...
      ExplosionUtils.PlaySoundAndShake(
          epicenter, (byte)Mathf.Clamp(collisions * 12, 16, byte.MaxValue),
    ...
    // Damage methods
    void ApplyDamage(MatrixInfo victimMatrix, Vector3Int cellPos,
                     float hitEnergy, Vector3Int worldPos) {
    ...
    void ApplyCritDamage(MatrixInfo victimMatrix, Vector3Int cellPos,
                         Vector3Int worldPos) {
      // LivingHealthBehaviour
      ...
      if (damage > 9000) {
        foreach (var damageableLayer in matrix.MetaTileMap.LayersValues) {
          if (damageableLayer.LayerType == LayerType.Objects)
            continue;
          matrix.TileChangeManager.MetaTileMap.RemoveTileWithlayer(
              cellPos, damageableLayer.LayerType);
        }
      }
    }
    ...
    void ApplyWireDamage(MatrixInfo matrix, Vector3Int cellPos) {
      ...
      foreach (var wire in matrix.Matrix.Get<CableInheritance>(cellPos, true)) {
        if (Random.value >= 0.5) {
          ...
          StartCoroutine(
              DestroyWireWithDelay(wire, (byte)(Random.value * 20f)));
        } else {
          // Destruction
          wire.ToDestroy();
        }
    ...
    float ApplyIntegrityDamage(MatrixInfo matrix, Vector3Int cellPos,
                               float damage) {
      if (matrix == null)
        return 0;
    ...
    float ApplyLivingDamage(MatrixInfo matrix, Vector3Int cellPos,
                            float damage) {
      if (matrix == null)
        return 0;

      byte count = 0;
      foreach (var healthBehaviour in matrix.Matrix.Get<LivingHealthMasterBase>(
                   cellPos, true)) {
        healthBehaviour.ApplyDamageToBodyPart(
            matrix.GameObject, damage, AttackType.Melee, DamageType.Brute);
        count++;
      }
    ...
    void TryPushing(MatrixInfo matrix, Vector3Int cellPos,
                    Vector2Int pushVector, float speed) {
  ...
  private void SlowDown(MatrixIntersection i, int collisions) {
    ...
    void InternalSlowDown(MatrixInfo info) {
      float slowdownFactor =
          Mathf.Clamp(1f - (Mathf.Clamp(collisions, 1, 50) / 100f) + info.Mass,
                      0.1f, 0.95f);
      float speed =
          (info.MatrixMove.ServerState.Speed * slowdownFactor) - 0.07f;
      if (speed <= 1f) {
  ...
  private void OnDrawGizmos() {
    ...
    foreach (var intersection in Instance.TrackedIntersections) {
      ...
      DebugGizmoUtils.DrawRect(intersection.Matrix1.WorldBounds.Minimum,
                               intersection.Matrix1.WorldBounds.Maximum);
      Gizmos.color = Color.blue;
      DebugGizmoUtils.DrawRect(intersection.Matrix2.WorldBounds.Minimum,
                               intersection.Matrix2.WorldBounds.Maximum);
      Gizmos.color = Color.yellow;
      DebugGizmoUtils.DrawRect(intersection.Rect.Minimum,
                               intersection.Rect.Maximum);
    }
  }
}

/// <summary>
/// First and second matrix are swappable – intersections (m1,m2) and (m2,m1)
/// will be considered equal. Rect isn't checked for equality
/// </summary>
public readonly struct MatrixIntersection {
  ...
  public MatrixIntersection(MatrixInfo matrix1, MatrixInfo matrix2,
                            BetterBounds rect) {
    Matrix1 = matrix1;
    Matrix2 = matrix2;
    Rect = rect;
  ...
  public MatrixIntersection Clone() => new MatrixIntersection(Matrix1, Matrix2,
                                                              Rect);

  public MatrixIntersection Clone(ref BetterBounds rect) =>
      new MatrixIntersection(Matrix1, Matrix2, rect);
  ...
  public static bool operator ==(MatrixIntersection left,
                                 MatrixIntersection right) {
