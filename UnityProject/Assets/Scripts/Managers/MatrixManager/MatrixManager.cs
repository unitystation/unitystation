...
/// Matrix manager keeps a list of matrices that you can access from both client
/// and server. Contains world/local position conversion methods, as well as
/// several cross-matrix adaptations of Matrix methods. Also very common use
/// scenario is Get()'ting matrix info using matrixId from PlayerState
[ExecuteInEditMode]
public partial class MatrixManager : SingletonManager<MatrixManager> {
  public Dictionary<int, MatrixInfo> ActiveMatrices {
    get; private set;
  } = new Dictionary<int, MatrixInfo>();

  public List<MatrixInfo> ActiveMatricesList {
    get; private set;
  } = new List<MatrixInfo>();

  public Dictionary<Scene, List<Matrix>> InitializingMatrixes =
      new Dictionary<Scene, List<Matrix>>();

  public List<MatrixInfo> MovableMatrices {
    get; private set;
  } = new List<MatrixInfo>();
  ...
  /// </summary>
  public Dictionary<Collider2D, Tilemap> wallsTileMaps =
      new Dictionary<Collider2D, Tilemap>();
  ...
  private bool AreAllMatrixReady() {
    if (SubSceneManager.Instance.loadedScenesList.Count !=
        InitializingMatrixes.Count) {
      return false;
    }
  ...
  [Server]
  private void OnScenesLoaded() { StartCoroutine(WaitForAllMatrices()); }
  ...
  private void ClientAllMatrixReady() {
    for (int i = 0; i < ActiveMatricesList.Count; i++) {
      // TODO fixme: expensive and overly complicated way to make tiles
      // interactable by the client (wrenching pipe tiles, etc)
      ActiveMatricesList[i].Matrix.MetaTileMap.InitialiseUnderFloorUtilities(
          CustomNetworkManager.IsServer);
      TileChangeNewPlayer.Send(ActiveMatricesList[i].NetID);
    }

    SpriteRequestCurrentStateMessage.Send(
        SpriteHandlerManager.Instance.GetComponent<NetworkIdentity>().netId);
  }
  ...
  private void RegisterMatrix(Matrix matrix) {
    var matrixInfo = CreateMatrixInfoFromMatrix(
        matrix, matrix.NetworkedMatrix.MatrixSync.matrixID);
  ...
  private static MatrixInfo CreateMatrixInfoFromMatrix(Matrix matrix, int id) {
    ...
    return new MatrixInfo(id) {
      ...
      GameObject = gameObj,
      Objects =
          gameObj.transform.GetComponentInChildren<ObjectLayer>().transform,
  ...
  public static MatrixInfo AtPoint(Vector3Int worldPos, bool isServer,
                                   MatrixInfo possibleMatrix = null) {
    // for performance, this is just a suggestion on which matrix we believe
    // this point could be in, not always correct
    if (possibleMatrix != null &&
        IsInMatrix(worldPos, isServer, possibleMatrix)) {
  ...
  private static bool IsInMatrix(Vector3Int worldPos, bool isServer,
  ...
  public static CustomPhysicsHit Linecast(Vector3 Worldorigin,
                                          LayerTypeSelection layerMask,
                                          LayerMask? Layermask2D,
                                          Vector3 WorldTo) {
    return RayCast(Worldorigin, Vector2.zero, 0, layerMask, Layermask2D,
                   WorldTo);
  }

  public static CustomPhysicsHit
  RayCast(Vector3 Worldorigin, Vector2 direction, float distance,
          LayerTypeSelection layerMask, LayerMask? Layermask2D = null,
          Vector3? WorldTo = null, LayerTile[] tileNamesToIgnore = null) {
    Worldorigin.z = 0;
    ...
    if (Layermask2D != null) {
      var Hit2D = Physics2D.Raycast(Worldorigin, direction.normalized, distance,
                                    Layermask2D.Value);
      if (ClosestHit != null) {
  ...
  public struct CustomPhysicsHit {
    ...
    public CustomPhysicsHit(Vector3 _TileHitWorld, Vector3 _HitWorld,
                            Vector2 _Normal, float _RelativeHit,
  ...
  public static bool LineIntersectsRect(Vector2 p1, Vector2 p2,
                                        BetterBoundsInt r) {
    var xMin = r.xMin;
    var yMin = r.yMin;
    var xMax = r.xMax;
    ...
    return LineIntersectsLine(p1, p2, new Vector2(xMin, yMin),
                              new Vector2(xMin, yMax)) ||
           LineIntersectsLine(p1, p2, new Vector2(xMin, yMin),
                              new Vector2(xMax, yMin)) ||
           LineIntersectsLine(p1, p2, new Vector2(xMax, yMax),
                              new Vector2(xMin, yMax)) ||
           LineIntersectsLine(p1, p2, new Vector2(xMax, yMax),
                              new Vector2(xMax, yMin)) ||
  ...
  private static bool LineIntersectsLine(Vector2 l1p1, Vector2 l1p2,
                                         Vector2 l2p1, Vector2 l2p2) {
  ...
  public static bool IsConstructable(Vector3Int worldPos,
                                     MatrixInfo matrixInfo) {
    if (matrixInfo.Matrix.MetaTileMap.IsConstructable(
            WorldToLocalInt(worldPos, matrixInfo))) {
      return true;
    }
  ...
  /// <summary>
  /// Server only.
  /// Finds all TilemapDamage at given world pos, so you could apply damage to
  /// given tiles via DoMeleeDamage etc.
  /// </summary>
  /// <param name="worldTarget"></param>
  /// <returns></returns>
  public static IReadOnlyList<TilemapDamage>
  GetDamageableTilemapsAt(Vector3Int worldTarget) {
    var Matrix = MatrixManager.AtPoint(worldTarget,
  ...
  /// <param name="bumper">PlayerMove trying to bump / move, used to check if we
  /// can swap with another player with help intent.</param> <returns>the bump
  /// type which occurs at the specified point (BumpInteraction.None if it's
  /// open space)</returns>
  public static BumpType GetBumpTypeAt(Vector3Int worldOrigin, Vector2Int dir,
                                       PlayerMove bumper, bool isServer) {
    ...
    if (hasHelpIntent) {
      ...
      if (other != null) {
        ...
        if (bumper.PlayerScript.pushPull.IsPullingSomething &&
            bumper.PlayerScript.pushPull.PulledObject ==
                other.PlayerScript.pushPull) {
    ...
    bool isPassable = IsPassableAtAllMatrices(worldOrigin, targetPos, isServer,
                                              includingPlayers: true,
                                              context: bumper.gameObject);
    // Only push if not passable, e.g. for directional windows being pushed from
    // parallel
    if (isPassable == false &&
        GetPushableAt(worldOrigin, dir, bumper.gameObject, isServer, true)
                .Count > 0) {
  ...
  /// Checks what type of bump occurs at the specified destination.
  /// </summary>
  /// <param name="playerState">player state, used to get current world
  /// position</param> <param name="playerAction">action indicating the
  /// direction player is trying to move</param> <param name="bumper">PlayerMove
  /// trying to bump / move, used to check if we can swap with another player
  /// with help intent.</param> <returns>the bump type which occurs at the
  /// specified point (BumpInteraction.None if it's open space)</returns>
  public static BumpType GetBumpTypeAt(PlayerState playerState,
                                       PlayerAction playerAction,
                                       PlayerMove bumper, bool isServer) {
    return GetBumpTypeAt(playerState.WorldPosition.RoundToInt(),
                         playerAction.Direction(), bumper, isServer);
  }

  /// <summary>
  /// Gets the closest door to worldOrigin on the path from worldOrigin to
  /// targetPos (if there is one) Assumes that
  /// </summary>
  /// <param name="worldOrigin">Origin World position to check from. This is
  /// required because e.g. Windoors may not appear closed from certain
  /// positions.</param> <param name="targetPos">target world position to
  /// check</param> <returns>The DoorTrigger of the closed door object specified
  /// in the summary, null if no such object exists at that location</returns>
  public static InteractableDoor
  GetClosedDoorAt(Vector3Int worldOrigin, Vector3Int targetPos, bool isServer) {
    // Check door on the local tile first
    Vector3Int localTarget =
        WorldToLocalInt(targetPos, AtPoint(targetPos, isServer).Matrix);
    var originDoorList = GetAt<InteractableDoor>(worldOrigin, isServer);
    foreach (InteractableDoor originDoor in originDoorList) {
      if (originDoor &&
          originDoor.GetComponent<RegisterDoor>().IsPassableFromInside(
              localTarget, isServer) == false)
        return originDoor;
    }

    // No closed door on local tile, check target tile
    Vector3Int localOrigin =
        WorldToLocalInt(worldOrigin, AtPoint(worldOrigin, isServer).Matrix);
    var targetDoorList = GetAt<InteractableDoor>(targetPos, isServer);
    foreach (InteractableDoor targetDoor in targetDoorList) {
      if (targetDoor &&
          targetDoor.GetComponent<RegisterDoor>().IsPassableFromOutside(
              localOrigin, isServer) == false)
        return targetDoor;
    }
  ...
  public static DoorMasterController GetNewClosedDoorAt(Vector3Int worldOrigin,
  ...
  /// </summary>
  /// <param name="worldPosition">Position at which the node is looked
  /// for.</param> <returns>MetaDataNode at the position. If no Node that isn't
  /// space is found, MetaDataNode.Node will be returned.</returns>
  public static MetaDataNode GetMetaDataAt(Vector3Int worldPosition) {
    var mat = AtPoint(worldPosition, CustomNetworkManager.Instance._isServer,
                      MainStationMatrix);
  ...
  /// <summary>
  /// Server only:
  /// Picks best matching matrix at provided coords and releases reagents to
  /// that tile. <inheritdoc cref="MetaDataLayer.ReagentReact"/>
  /// </summary>
  ...
  /// <summary>
  /// Triggers an Subsystem Update at the given position. Only triggers, when a
  /// MetaDataNode already exists.
  /// </summary>
  /// <param name="worldPosition">Position where the update is
  /// triggered.</param>
  ...
  /// <see cref="MatrixManager.GetPushableAt(Vector3Int, Vector2Int, GameObject,
  /// bool)"/>
  private static void GetPushablesOneTile(ref List<PushPull> pushableList,
                                          Vector3Int pushableLocation,
                                          Vector2Int dir, GameObject pusher,
                                          bool isServer,
                                          bool ignoreNonBlockable,
                                          bool isLeaving) {
    Vector3Int localPushableLocation = WorldToLocalInt(
        pushableLocation, AtPoint(pushableLocation, isServer).Matrix);
    foreach (PushPull pushPull in GetAt<PushPull>(pushableLocation, isServer)) {
      if (pushPull == null || pushPull.gameObject == pusher)
        continue;

      if (ignoreNonBlockable) {
        if (isLeaving) {
          // ignore nonblocking pushables on tile we're leaving
          if (pushPull.registerTile.IsPassableFromInside(localPushableLocation +
                                                             (Vector3Int)dir,
                                                         isServer: isServer)) {
            continue;
          }
          ...
          if (pushPull.registerTile.IsPassableFromOutside(
                  pushableLocation - (Vector3Int)dir, isServer: isServer)) {
            continue;
          }
        }
      }

      // If the object being Push/Pulled is a player, and that player is
      // buckled, we should use the pushPull object that the player is buckled
      // to. By design, chairs are not "solid" so, the condition above will
      // filter chairs but won't filter players
      ...
      if (playerMove && playerMove.IsBuckled) {
        PushPull buckledPushPull =
            playerMove.BuckledObject.GetComponent<PushPull>();
  ...
  /// <summary>
  /// Checks if there are any pushables int the specified direction which can be
  /// pushed from the current position.
  /// </summary>
  /// <param name="worldOrigin">position pushing from</param>
  /// <param name="dir">direction to push</param>
  /// <param name="pusher">gameobject of the thing attempting the push, only
  /// used to prevent itself from being able to push itself</param> <param
  /// name="ignoreNonBlockable">true if only objects that block the indicated
  /// movement should be included</param> <returns>each pushable other than
  /// pusher at worldTarget for which it is possible to actually move it when
  /// pushing from worldOrigin (i.e. if it's against a wall and you try to push
  /// against the wall, that pushable would be excluded). Empty list if no
  /// pushables.</returns>
  public static List<PushPull> GetPushableAt(Vector3Int worldOrigin,
                                             Vector2Int dir, GameObject pusher,
                                             bool isServer,
                                             bool ignoreNonBlockable) {
    ...
    GetPushablesOneTile(ref result, worldTarget, dir, pusher, isServer,
                        ignoreNonBlockable, false);

    return result;
  }

  /// <summary>
  /// Gets the player move at the position which is currently able to be swapped
  /// with (if one exists).
  /// </summary>
  /// <param name="targetWorldPos">Position to check</param>
  /// <param name="mover">gameobject of the thing attempting the move, only used
  /// to prevent itself from being checked</param> <returns>the player move that
  /// is swappable, otherwise null</returns>
  public static PlayerMove GetSwappableAt(Vector3Int targetWorldPos,
                                          GameObject mover, bool isServer) {
    var playerMoves = GetAt<PlayerMove>(targetWorldPos, isServer);
    foreach (PlayerMove playerMove in playerMoves) {
      if (playerMove && playerMove.IsSwappable &&
  ...
  public static bool IsTotallyImpassable(Vector3Int worldTarget,
                                         bool isServer) {
  ...
  public static List<T> GetAs<T>(Vector3Int worldPos, bool isServer)
  ...
  public static List<T> GetReachableAt<T>(Vector3Int fromPos, Vector3Int toPos,
                                          bool isServer)
      where T : MonoBehaviour {
  ...
  /// <summary>
  /// checks all tiles adjacent to the indicated world position for objects with
  /// the indicated component. Probably pretty expensive.
  /// </summary>
  /// <param name="worldPos"></param>
  /// <param name="isServer"></param>
  /// <typeparam name="T"></typeparam>
  /// <returns></returns>
  public static List<T> GetAdjacent<T>(Vector3Int worldPos, bool isServer)
      where T : MonoBehaviour {
    List<T> result = new List<T>();

    result.AddRange(GetAt<T>(worldPos + Vector3Int.right, isServer));
    result.AddRange(
        GetAt<T>(worldPos + Vector3Int.right + Vector3Int.up, isServer));
    result.AddRange(GetAt<T>(worldPos + Vector3Int.up, isServer));
    result.AddRange(
        GetAt<T>(worldPos + Vector3Int.up + Vector3Int.left, isServer));
    result.AddRange(GetAt<T>(worldPos + Vector3Int.left, isServer));
    result.AddRange(
        GetAt<T>(worldPos + Vector3Int.left + Vector3Int.down, isServer));
    result.AddRange(GetAt<T>(worldPos + Vector3Int.down, isServer));
    result.AddRange(
        GetAt<T>(worldPos + Vector3Int.down + Vector3Int.right, isServer));

    return result;
  }

  /// <summary>
  /// checks all reachable tiles adjacent to the indicated world position for
  /// objects with the indicated component. Probably pretty expensive.
  /// </summary>
  /// <param name="worldPos"></param>
  /// <param name="isServer"></param>
  /// <typeparam name="T"></typeparam>
  /// <returns></returns>
  public static List<T> GetReachableAdjacent<T>(Vector3Int worldPos,
                                                bool isServer)
      where T : MonoBehaviour {
    List<T> result = new List<T>();

    result.AddRange(
        GetReachableAt<T>(worldPos, worldPos + Vector3Int.right, isServer));
    result.AddRange(GetReachableAt<T>(
        worldPos, worldPos + Vector3Int.right + Vector3Int.up, isServer));
    result.AddRange(
        GetReachableAt<T>(worldPos, worldPos + Vector3Int.up, isServer));
    result.AddRange(GetReachableAt<T>(
        worldPos, worldPos + Vector3Int.up + Vector3Int.left, isServer));
    result.AddRange(
        GetReachableAt<T>(worldPos, worldPos + Vector3Int.left, isServer));
    result.AddRange(GetReachableAt<T>(
        worldPos, worldPos + Vector3Int.left + Vector3Int.down, isServer));
    result.AddRange(
        GetReachableAt<T>(worldPos, worldPos + Vector3Int.down, isServer));
    result.AddRange(GetReachableAt<T>(
        worldPos, worldPos + Vector3Int.down + Vector3Int.right, isServer));

    return result;
  }

  // shorthand for calling GetAt at the targeted object's position
  public static List<T> GetAt<T>(GameObject targetObject, NetworkSide side)
      where T : MonoBehaviour {
    return GetAt<T>((Vector3Int)targetObject.TileWorldPosition(),
                    side == NetworkSide.Server);
  }

  /// Cross-matrix edition of <see
  /// cref="Matrix.IsPassableAt(UnityEngine.Vector3Int,bool)"/>
  ///< inheritdoc cref="Vector3Int"/>
  public static bool IsPassableAtAllMatricesOneTile(
      Vector3Int worldPos, bool isServer, bool includingPlayers = true,
      List<LayerType> excludeLayers = null, List<TileType> excludeTiles = null,
      GameObject context = null, bool ignoreObjects = false,
      bool onlyExcludeLayerOnDestination = false) {
    var matrixInfo = AtPoint(worldPos, isServer);
    var localPos = WorldToLocalInt(worldPos, matrixInfo);
    var value = matrixInfo.Matrix.IsPassableAtOneMatrixOneTile(
        localPos, isServer, includingPlayers, excludeLayers, excludeTiles,
        context, ignoreObjects, onlyExcludeLayerOnDestination);
    return value;
  }

  /// Cross-matrix edition of <see
  /// cref="Matrix.IsPassableAt(UnityEngine.Vector3Int,UnityEngine.Vector3Int,bool,GameObject)"/>
  ///< inheritdoc
  ///< cref="Matrix.IsPassableAt(UnityEngine.Vector3Int,UnityEngine.Vector3Int,bool,GameObject)"/>
  public static bool IsPassableAtAllMatrices(
      Vector3Int worldOrigin, Vector3Int worldTarget, bool isServer,
      CollisionType collisionType = CollisionType.Player,
      bool includingPlayers = true, GameObject context = null,
      MatrixInfo excludeMatrix = null, List<LayerType> excludeLayers = null,
      bool isReach = false, bool onlyExcludeLayerOnDestination = false) {
    MatrixInfo possibleMatrix =
        context ? context.GetComponent<RegisterTile>()?.Matrix.MatrixInfo
                : null;
    ...
    var value = matrixOrigin.Matrix.IsPassableAtOneMatrix(
        localPosOrigin, localPosTarget, isServer, collisionType,
        includingPlayers, context, excludeLayers, isReach: isReach,
    ...
    if (matrixOrigin != matrixTarget) {
      ...
      value = matrixTarget.Matrix.IsPassableAtOneMatrix(
          localPosOrigin, localPosTarget, isServer, collisionType,
          includingPlayers, context, excludeLayers, isReach: isReach,
          onlyExcludeLayerOnDestination: onlyExcludeLayerOnDestination);
      if (value == false) {
  ...
  /// <inheritdoc
  /// cref="ObjectLayer.HasAnyDepartureBlockedByRegisterTile(Vector3Int, bool,
  /// RegisterTile)"/>
  public static bool HasAnyDepartureBlockedAt(Vector3Int worldPos,
                                              bool isServer,
                                              RegisterTile context) {
    var matrixInfo = AtPoint(worldPos, isServer);
    var localPos = WorldToLocalInt(worldPos, matrixInfo);
    var value = matrixInfo.Matrix.HasAnyDepartureBlockedOneMatrix(
        localPos, isServer, context);
    return value;
  }
  ...
  public static bool IsSlipperyOrNoGravityAt(Vector3Int worldPos,
                                             MatrixInfo possibleMatrix) {
    return IsSlipperyAt(worldPos, possibleMatrix) ||
           IsNoGravityAt(worldPos, true, possibleMatrix);
  }

  public static bool IsFloatingAt(GameObject context, Vector3Int worldPos,
                                  bool isServer, MatrixInfo possibleMatrix) {
  ...
  public static bool IsFloatingAt(GameObject[] context, Vector3Int worldPos,
    ...
    foreach (Vector3Int pos in worldPos.BoundsAround().allPositionsWithin) {
      var matrixInfo = AtPoint(pos, isServer, possibleMatrix);
      var localPos = WorldToLocalInt(pos, matrixInfo);
      if (matrixInfo.MetaTileMap.IsEmptyAt(context, localPos, isServer) ==
          false) {
        return false;
      }
    }
  ...
  /// Cross-matrix edition of <see
  /// cref="Matrix.IsAtmosPassableAt(UnityEngine.Vector3Int,bool)"/>
  ...
  /// </Summary>
  public T GetFirst<T>(Vector3Int position, bool isServer)
      where T : MonoBehaviour {
    foreach (var matrixInfo in ActiveMatricesList) {
      T first = matrixInfo.Matrix.GetFirst<T>(
          WorldToLocalInt(position, matrixInfo), isServer);
      if (first) {
        return first;
      }
    }
  ...
  public static MatrixInfo Get(int id) {
    // Sometimes Get is still being called on the old matrixmanager instance on
    // a round restart
  ...
  LocalToWorldInt(Vector3 localPos, MatrixInfo matrix,
                  MatrixState state = default(MatrixState)) {
    return Vector3Int.RoundToInt(LocalToWorld(localPos, matrix, state));
  }

  /// <summary>
  /// Convert local matrix coordinates to world position. Keeps offsets in mind
  /// (+ rotation and pivot if MatrixMove is present)
  /// </summary>
  ...
  public static Vector3 LocalToWorld(Vector3 localPos, MatrixInfo matrix,
    ...
    if (matrix == null || matrix.Equals(MatrixInfo.Invalid) ||
        localPos == TransformState.HiddenPos) {
    ...
    Vector3 unpivotedPos =
        localPos - matrix.MatrixMove.Pivot; // localPos - localPivot
    Vector3 rotatedPos =
    ...
    Vector3 rotatedPivoted =
        ...
        matrix.GetOffset(
            state); // adding back localPivot and applying localToWorldOffset
  ...
  public static Vector3 WorldToLocal(Vector3 worldPos, MatrixInfo matrix) {
    // Invalid matrix info provided
    if (matrix is null || matrix.Equals(MatrixInfo.Invalid) ||
        worldPos == TransformState.HiddenPos) {
      return TransformState.HiddenPos;
    }
    ...
    return (state.FacingOffsetFromInitial(matrix.MatrixMove)
                .QuaternionInverted *
            (worldPos - matrix.MatrixMove.Pivot - matrix.GetOffset(state))) +
  ...
  /// <inheritdoc cref="WorldToLocalInt(Vector3, int)"/>
  public static Vector3Int WorldToLocalInt(Vector3 worldPos,
                                           MatrixInfo matrix) {
    return Vector3Int.RoundToInt(WorldToLocal(worldPos, matrix));
  }

  /// <summary>
  /// Convert world position to local matrix coordinates. Keeps offsets in mind
  /// (+ rotation and pivot if MatrixMove is present)
  /// </summary>
  ...
  /// </summary>
  public static List<Vector3Int> GetTiles(Vector3 startPos, Vector2 targetPos,
                                          int travelDistance) {
  ...
  /// <summary>
  /// Do something to matrix using its local position if you only know world
  /// position
  ...
  public static void ForMatrixAt(Vector3Int worldPos, bool isServer,
                                 Action<MatrixInfo, Vector3Int> matrixAction) {
    var matrixAtPoint = AtPoint(worldPos, isServer);
    matrixAction.Invoke(matrixAtPoint,
                        WorldToLocalInt(worldPos, matrixAtPoint));
  }
}
