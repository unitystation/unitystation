...
public class MatrixInfo : IEquatable<MatrixInfo> {
  ...
  // todo: placeholder, should depend on solid tiles count instead (and use
  // caching)
  ...
  public Vector2Int MovementVector =>
      (IsMovable && MatrixMove.IsMovingServer)
          ? MatrixMove.ServerState.FlyingDirection.VectorInt
          : Vector2Int.zero;
  ...
  public Vector3Int GetOffset(MatrixState state = default(MatrixState)) {
    ...
    if (cachedPosition != state.Position) {
      ...
      CachedOffset = initialOffset +
                     (state.Position.RoundToInt() - MatrixMove.InitialPosition);
    }
  ...
  public MatrixInfo(int id) { Id = id; }
  ...
  public bool
  Equals(MatrixInfo other) => other is null == false && Id == other.Id;

  public override bool Equals(object obj) => obj is MatrixInfo other &&
                                             Equals(other);

  public override int GetHashCode() => Id;

  /// Figuring out netId. NetworkIdentity is located on the pivot (parent)
  /// gameObject for MatrixMove-equipped matrices
  private static uint getNetId(Matrix matrix) {
    ...
    NetworkedMatrix networkedMatrix =
        matrix.gameObject.GetComponentInParent<NetworkedMatrix>();
    ...
    NetworkIdentity component = networkedMatrix.OrNull()
                                    ?.MatrixSync.OrNull()
                                    ?.GetComponent<NetworkIdentity>();

    if (component && component.netId != NetId.Invalid &&
        component.netId != NetId.Empty) {
      netId = component.netId;
    }

    if (netId == NetId.Invalid) {
      Logger.LogWarning($"Invalid NetID for matrix {matrix.gameObject.name}!",
                        Category.Matrix);
    }
  ...
  public static IEqualityComparer<MatrixInfo> IdComparer {
    get;
  ...
  public static bool operator !=(MatrixInfo left,
                                 MatrixInfo right) => left == right == false;
}