...
public class PlayerList : NetworkBehaviour
{
	...
	private void TryAdd(ConnectedPlayer player)
	{
		...
		{
			Logger.Log("Refused to add invalid connected player",Categories.PlayerList);
			return;
		}
...
//			Logger.Log($"Updating {Get(player.Connection)} with {player}");
		...
		{
			values.Add(player);
			Logger.Log($"Added {player}. Total:{values.Count}; {string.Join("; ",values)}",Categories.PlayerList);
...
//			Logger.Log( "Ignoring kick timer for invalid connection" );
	...
	{
		Logger.Log($"Removed {player}",Categories.PlayerList);
	...
	private ConnectedPlayer getInternal(Func<ConnectedPlayer,bool> condition, bool lookupOld = false)
	{
		...
		if ( lookupOld )
		{
			for ( var i = 0; i < oldValues.Count; i++ )
			{
				...
				{
					Logger.Log( $"Returning old player {oldValues[i]}", Categories.PlayerList);
					return oldValues[i];
				}
			}
		}
	...
	public void Remove(NetworkConnection connection)
	{
		...
		{
			Logger.LogError($"Cannot remove by {connection}, not found", Categories.PlayerList);
		}
		else
		{
