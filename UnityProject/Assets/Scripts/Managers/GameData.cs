...
public class GameData : MonoBehaviour {
  ...
  /// <summary>
  /// Whether --offlinemode command line argument is passed. Enforces offline
  /// mode.
  /// </summary>
  private bool forceOfflineMode;

  /// <summary>
  /// Is offline mode enabled, allowing login skip / working without connection
  /// to server.? Disabled always for release builds.
  /// </summary>
  public bool OfflineMode =>
      (!BuildPreferences.isForRelease && offlineMode) || forceOfflineMode;
  ...
  private
    set { GameInfo.IsHeadlessServer = value; }
  }
...
#region Lifecycle
  ...
  private void Init() {
    ...
    DevBuild = true;
#endif
    var buildInfo = JsonUtility.FromJson<BuildInfo>(File.ReadAllText(
        Path.Combine(Application.streamingAssetsPath, "buildinfo.json")));
    ...
    Logger.Log($"Build Version is: {BuildNumber}. " +
               (OfflineMode ? "Offline mode" : string.Empty));
    CheckHeadlessState();
  ...
  private void OnLevelFinishedLoading(Scene oldScene, Scene newScene) {
    ...
    if (CheckHeadlessState()) {
      ...
      //calcFrameRate; 			Logger.Log($"Starting server in HEADLESS mode. Target
      //framerate is {Application.targetFrameRate}", 				Category.Server);
      ...
      if (rconManager == null) {
        GameObject rcon =
            Instantiate(Resources.Load("Rcon/RconManager") as GameObject, null)
                as GameObject;
  ...
  private async void AttemptAutoJoin(Action<String> OnLoginSuccess) {
    ...
    await ServerData.ValidateUser(
        FirebaseAuth.DefaultInstance.CurrentUser, OnLoginSuccess,
        LobbyManager.Instance.lobbyDialogue.LoginError);
  }

  private async void HubToServerConnect(string ip, string port, string uid,
                                        string token) {
    await Task.Delay(TimeSpan.FromSeconds(0.1));
    ...
    if (!String.IsNullOrEmpty(token)) {
      ...
      if (!string.IsNullOrEmpty(response.errorMsg)) {
        Logger.LogError(
            $"Something went wrong with hub token validation {response.errorMsg}",
            Category.DatabaseAPI);
        LobbyManager.Instance.lobbyDialogue.LoginError(
            $"Could not verify your details {response.errorMsg}");
        return;
      }

      await FirebaseAuth.DefaultInstance
          .SignInWithCustomTokenAsync(response.message)
          .ContinueWithOnMainThread(async task => {
            if (task.IsCanceled) {
            ...
            if (task.IsFaulted) {
              Logger.LogError("Task Faulted: " + task.Exception,
                              Category.DatabaseAPI);
              LobbyManager.Instance.lobbyDialogue.LoginError($"Task Faulted: " +
                                                             task.Exception);
              return;
            }

            var success =
                await ServerData.ValidateUser(task.Result, null, null);

            if (success) {
              Logger.Log("Signed in successfully with valid token",
                         Category.DatabaseAPI);
              LobbyManager.Instance.lobbyDialogue.ShowCharacterEditor(
