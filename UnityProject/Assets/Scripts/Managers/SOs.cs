...
/// <summary>
/// In order for the SingletonScriptableObject to work, the singleton instance
/// must be mapped into this component. Otherwise Unity won't include the asset
/// in the build (singleton will work only in editor).
/// </summary>
public class SOs : SingletonManager<SOs> {
  // Put all singletons here (and in the dictionary below) and assign them in
  // editor.
  ...
  public T GetEntry<T>()
    ...
    if (typeSOMap.TryGetValue(typeof(T), out ScriptableObject value)) {
      if (value == null) {
        Logger.LogError(
            $"{typeof(T).FullName} is not assigned to {gameObject.name} prefab.");
        return null;
      }
    ...
    Logger.LogWarning(
        $"{nameof(SOs)} is missing entry for {typeof(T).FullName}.");
    return default;
  }
}
}
