...
public class MainStationListSO : ScriptableObject
{
	...
	public List<AssetReference> MainStations = new();
	...
	public string GetRandomMainStation()
	{
		...
		var mapSoList = MainStations.Where(scene => SceneUtility.GetBuildIndexByScenePath(scene.ToString()) > -1).ToList();
		...
		return mapSoList.PickRandom().ToString();
	}
}
