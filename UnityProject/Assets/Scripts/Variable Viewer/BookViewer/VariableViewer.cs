...
public static class VariableViewer {
  public static void ProcessTile(Vector3 Location, GameObject WhoBy) {
    ...
    List<GameObject> _Objects =
        registerTiles.Select(x => x.gameObject).ToList();
    ...
    var interactableTiles = matrix.GetComponentInParent<InteractableTiles>();
        ...
        foreach (var Object in _Objects) { transforms.Add(Object.transform); }
  ...
  public static void ProcessTransform(Transform transform, GameObject WhoBy,
    ...
    if (Librarian.TransformToBookShelf.ContainsKey(transform)) {
      ...
      BookShelf =
          Librarian.Library.LibraryBookShelf.PartialGenerateLibraryBookShelf(
              transform);
    }
  ...
  public static void ProcessOpenBook(
      ulong BookID,
      GameObject ByWho) // yes yes if you Have high Ping then rip, -Creator
  {
    Librarian.Book Book;
  ...
  public static void
  SendBookShelfToClient(Librarian.Library.LibraryBookShelf BookShelf,
                        GameObject ToWho) {
    SubBookshelfNetMessage.Send(BookShelf, ToWho);
  }

  // Receive from Client side
  public static void RequestOpenPageValue(ulong PageID, uint SentenceID,
                                          bool IsSentence, bool iskey,
                                          GameObject ToWho) {
      ...
      if (!IsSentence) {
        ...
        if (_MonoBehaviour == null) {
          if ((Page.Variable as string) == "null") {
            Logger.LogWarning("Trying to process page value as book PageID > " +
                                  PageID,
                              Category.VariableViewer);
            return;
          }
        ...
        if (iskey) {
          book = Librarian.Book.GenerateNonMonoBook(
              Page.IDtoSentence[SentenceID].KeyVariable);
          SendBookToClient(book, ToWho);
        } else {
          book = Librarian.Book.GenerateNonMonoBook(
              Page.IDtoSentence[SentenceID].ValueVariable);
          SendBookToClient(book, ToWho);
        }
      }
    } else {
  ...
  public static void RequestSendBookshelf(ulong BookshelfID,
                                          bool IsNewbookBookshelf,
                                          GameObject WhoBy) {
    if (Librarian.IDToBookShelf.ContainsKey(BookshelfID)) {
      ...
      Logger.LogError("Bookshelf ID has not been generated BookshelfID > " +
                          BookshelfID,
                      Category.VariableViewer);
    }
  }
  ...
  public static void RequestChangeVariable(ulong PageID, string ChangeTo,
                                           bool SendToClient, GameObject WhoBy,
                                           string AdminId) {
    if (Librarian.IDToPage.ContainsKey(PageID)) {
      UIManager.Instance.adminChatWindows.adminToAdminChat.ServerAddChatRecord(
          WhoBy.name + " Modified " + Librarian.IDToPage[PageID].VariableName +
              " on " + Librarian.IDToPage[PageID].BindedTo.Title + " From " +
              VVUIElementHandler.Serialise(
                  Librarian.IDToPage[PageID].Variable,
                  Librarian.IDToPage[PageID].VariableType) +
              " to " + ChangeTo + " with Send to clients? " + SendToClient,
          AdminId);
      Librarian.PageSetValue(Librarian.IDToPage[PageID], ChangeTo);
      if (SendToClient) {
        var monoBehaviour =
            (Librarian.IDToPage[PageID].BindedTo.BookClass as Component);
        UpdateClientValue.Send(ChangeTo,
                               Librarian.IDToPage[PageID].VariableName,
                               TypeDescriptor.GetClassName(monoBehaviour),
                               monoBehaviour.gameObject);
      }
    } else {
      Logger.LogError("Page ID has not been generated Page ID > " + PageID,
                      Category.VariableViewer);
    }
  }

  public static void RequestInvokeFunction(ulong PageID, GameObject WhoBy,
                                           string AdminId) {
    if (Librarian.IDToPage.ContainsKey(PageID)) {
      UIManager.Instance.adminChatWindows.adminToAdminChat.ServerAddChatRecord(
          WhoBy.name + " Invoked " + Librarian.IDToPage[PageID].VariableName +
              " on " + Librarian.IDToPage[PageID].BindedTo.Title,
          AdminId);
  ...
  // public static void RequestChangeSentenceVariable(ulong PageID, uint
  // SentenceID, string ChangeTo)
  //{
  //	if (Librarian.IDToPage.ContainsKey(PageID))
  //	{
  ...
  //		else
  //		{
  //			Logger.LogError("Sentence ID has not been generated in Page Page
  //ID > " + PageID + " Sentence ID >  " + SentenceID, Category.VariableViewer);
  //		}

  //	}
  //	else
  //	{
  //		Logger.LogError("Page ID has not been generated Page ID > " +
  //PageID, Category.VariableViewer);
  //	}
  //}
}
...
public static class Librarian {
  ...
  public static Dictionary<ulong, Book> IDToBook =
      new Dictionary<ulong, Book>();
  public static Dictionary<ulong, Page> IDToPage =
      new Dictionary<ulong, Page>();

  public static ulong BookShelfAID = 1;
  public static ulong BookAID = 1;
  public static ulong PageAID = 1;

  public static Dictionary<Transform, Library.LibraryBookShelf>
      TransformToBookShelf =
          new Dictionary<Transform, Library.LibraryBookShelf>();
  public static Dictionary<MonoBehaviour, Book> MonoBehaviourToBook =
      new Dictionary<MonoBehaviour, Book>();
  public static Dictionary<object, Book> ObjectToBook =
      new Dictionary<object, Book>();

  public static Type TupleTypeReference =
  ...
  public static Book GetAttributes(Book Book, object Script) {
    ...
    foreach (FieldInfo Field in coolFields) {
      if (Field.GetCustomAttributes(typeof(ObsoleteAttribute), true).Length ==
          0) {
        Page Page = new Page();
        ...
        GenerateSentenceValuesforSentence(Page.Sentences, Field.FieldType, Page,
                                          Script, FInfo: Field);
    ...
    if (TupleTypeReference !=
        monoType) // Causes an error if this is not here and Tuples can not get
                  // Custom properties so it is I needed to get the properties
    {
      ...
      foreach (PropertyInfo Properties in coolProperties) {
        if (Properties.GetCustomAttributes(typeof(ObsoleteAttribute), true)
                .Length == 0) {
          Page Page = new Page();
          ...
          GenerateSentenceValuesforSentence(Page.Sentences,
                                            Properties.PropertyType, Page,
                                            Script, PInfo: Properties);
          Book.BindedPagesAdd(Page);
        }
      }
    }
    ...
    foreach (MethodInfo Method in coolMethods) {
      ...
      // GenerateSentenceValuesforSentence(Page.Sentences, Field.FieldType,
      // Page, Script, FInfo: Field);
      Book.BindedPagesAdd(Page);
    }
  ...
  public static void GenerateSentenceValuesforSentence(
      Sentence sentence, Type VariableType, Page Page, object Script,
    ...
    if (FInfo == null && PInfo == null) {
              ...
              foreach (var c in list) {
                ...
                if (valueType.IsGenericType) {
                  ...
                  if (baseType == typeof(KeyValuePair<, >)) {
                    _sentence.KeyVariable =
                        valueType.GetProperty("Key").GetValue(c, null);
                    _sentence.ValueVariable =
                        valueType.GetProperty("Value").GetValue(c, null);

                    _sentence.ValueVariableType =
                        valueType.GetGenericArguments()[1];
                    _sentence.KeyVariableType =
                        valueType.GetGenericArguments()[0];
                  }
                }

                if (!valueType.IsClass) {
                  GenerateSentenceValuesforSentence(_sentence, c.GetType(),
                                                    Page, c);
                }
      ...
      if (VariableType.IsGenericType) {
        IEnumerable list;
        if (FInfo == null) {
          list = PInfo.GetValue(Script)
                     as IEnumerable; // icollection<keyvaluepair>
        } else {
        ...
        if (list != null) {
          foreach (object c in list) {
            ...
            if (!valueType.IsClass) {
              GenerateSentenceValuesforSentence(_sentence, c.GetType(), Page,
                                                c);
            }
  ...
  public class Library {
    ...
    public void RecursivePopulate(Transform Object, Transform Parent) {
      ...
      if (TransformToBookShelves.ContainsKey(Object)) {
        ...
        var libraryBookShelf = LibraryBookShelf.PartialGenerateLibraryBookShelf(
    ...
    public class LibraryBookShelf {
      ...
      public List<LibraryBookShelf> InternalContain =
          new List<LibraryBookShelf>();
      ...
      public static LibraryBookShelf
      PartialGenerateLibraryBookShelf(Transform _Transform) {
        List<Transform> Children = new List<Transform>();
        ...
        return PartialGenerateLibraryBookShelf(_Transform, _Transform.parent,
                                               Children);
      }

      public static LibraryBookShelf
      PartialGenerateLibraryBookShelf(Transform _Transform, Transform _Parent,
        ...
        if (_Parent != null) {
          library.TransformToBookShelves[_Parent].InternalContain.Add(
              libraryBookShelf);
        }

        libraryBookShelf.Contains.AddRange(Children);
        IDToBookShelf[libraryBookShelf.ID] = libraryBookShelf;
        TransformToBookShelf[libraryBookShelf.Shelf.transform] =
            libraryBookShelf;
      ...
      public bool ParentChange() {
        ...
        if (Shelf.transform.parent != null) {
          library.TransformToBookShelves[Shelf.transform.parent]
              .InternalContain.Add(this);
        }
  ...
  public class Book {
    ...
    public List<Page> BindedPages {
      get {
        if (UnGenerated) {
          Logger.LogWarning(
              "USE GetBindedPages()!,since these books are ungenerated ",
              Category.VariableViewer);
        }
    ...
    public void BindedPagesAdd(Page page) { _BindedPages.Add(page); }
  ...
  public class Page {
    ...
    public Dictionary<uint, Sentence> IDtoSentence =
        new Dictionary<uint, Sentence>();
    ...
    public void SetValue(string Value) {
      ...
      try {
        if (PInfo != null) {
          PInfo.SetValue(BindedTo.BookClass,
                         DeSerialiseValue(Variable, Value, Variable.GetType()));
        } else if (Info != null) {
          Info.SetValue(BindedTo.BookClass,
                        DeSerialiseValue(Variable, Value, Variable.GetType()));
        }
    ...
    public static object DeSerialiseValue(object InObject,
                                          string StringVariable, Type InType) {
      if (VVUIElementHandler.Type2Element.ContainsKey(InType)) {
        return (VVUIElementHandler.Type2Element[InType].DeSerialise(
            StringVariable));
      } else {
        if (InType.IsEnum) {
          // if ()
          ...
          if (InType == null || InObject == null ||
              InObject as IConvertible == null) {
            Logger.Log(
                $"Can't convert {StringVariable} to {InObject.GetType()}  " +
    ...
    public void UpdatePage() {
      ...
      if (Sentences.Sentences != null) {
        ...
        ASentenceID++;
        if (PInfo != null) {
          GenerateSentenceValuesforSentence(Sentences, PInfo.PropertyType, this,
                                            BindedTo.BookClass, PInfo: PInfo);
        } else {
          GenerateSentenceValuesforSentence(Sentences, Info.FieldType, this,
                                            BindedTo.BookClass, FInfo: Info);
        }
      }
    }
  }
  ...
  public static Type UEGetType(string TypeName) {
    ...
    // If the TypeName is a full name, then we can try loading the defining
    // assembly directly
  ...
  public static void MemberInfoSetValue(this MemberInfo memberInfo,
                                        object ClassObject,
                                        object NewVariableObject) {
    switch (memberInfo.MemberType) {
    case MemberTypes.Field:
