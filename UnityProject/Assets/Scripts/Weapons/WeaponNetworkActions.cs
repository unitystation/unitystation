...
public class WeaponNetworkActions : ManagedNetworkBehaviour
{
		...
		BodyPartType damageZone, LayerType layerType)
	{
		...
		else
		{
			...
			if (integrity != null)
			{
				...
				{//damaging an object
					...
					{
						// JESTER SoundManager.PlayNetworkedAtPos(integrity.soundOnHit, gameObject.WorldPosServer(), Random.Range(0.9f, 1.1f), sourceObj: gameObject);
					}
			...
			else
			{
				...
				else
				{
					...
					// JESTER SoundManager.PlayNetworkedAtPos("PunchMiss", transform.position, sourceObj: gameObject);
		...
		if (didHit)
		{
			...
			{
				// JESTER SoundManager.PlayNetworkedAtPos(attackSoundName, transform.position, sourceObj: gameObject);
			}
