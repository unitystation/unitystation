...
namespace Lucene.Net.Store
{
	...
	public abstract class FSDirectory : Directory
	{
		...
		protected internal void  InitOutput(System.String name)
		{
			EnsureOpen();
			CreateDir();
            var file = new System.IO.FileInfo(System.IO.Path.Combine(internalDirectory.FullName, name));
		...
		protected internal FSDirectory(System.IO.DirectoryInfo path, LockFactory lockFactory)
		{
            ...
            {
                var lf = (FSLockFactory)lockFactory;
                var dir = lf.LockDir;
        ...
        public static System.String[] ListAll(System.IO.DirectoryInfo dir)
        {
            ...
            var result = new System.String[files.Length];
		...
		{
            var file = new System.IO.FileInfo(System.IO.Path.Combine(directory.FullName, name));
            return (long)file.LastWriteTime.ToUniversalTime().Subtract(new DateTime(1970, 1, 1, 0, 0, 0)).TotalMilliseconds; //{{LUCENENET-353}}
		}
		...
		{
			EnsureOpen();
            var fullFile = new System.IO.FileInfo(System.IO.Path.Combine(internalDirectory.FullName, name));
			bool success = false;
			int retryCount = 0;
