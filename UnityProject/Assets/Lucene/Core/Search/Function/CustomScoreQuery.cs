...
namespace Lucene.Net.Search.Function
{
	...
	public class CustomScoreQuery:Query, System.ICloneable
	{
		...
		public CustomScoreQuery(Query subQuery, params ValueSourceQuery[] valSrcQueries)
		{
			...
			this.valSrcQueries = valSrcQueries ?? (new ValueSourceQuery[0]);
			if (subQuery == null) {
        ...
        public override Query Rewrite(IndexReader reader)
        {
            ...
            var sq = subQuery.Rewrite(reader);
            if (sq != subQuery)
            {
            ...
            {
                var v = (ValueSourceQuery)valSrcQueries[i].Rewrite(reader);
		...
		public virtual Explanation CustomExplain(int doc, Explanation subQueryExpl, Explanation[] valSrcExpls)
		{
			...
			var exp = new Explanation(valSrcScore * subQueryExpl.Value, "custom score: product of:");
		...
		private class CustomWeight:Weight
		{
			...
			public override Scorer Scorer(IndexReader reader, bool scoreDocsInOrder, bool topScorer)
			{
				...
				var subQueryScorer = subQueryWeight.Scorer(reader, true, false);
			...
			private Explanation DoExplain(IndexReader reader, int doc)
			{
                ...
                var customExp = Enclosing_Instance.GetCustomScoreProvider(reader).CustomExplain(doc, subQueryExpl, valSrcExpls);
