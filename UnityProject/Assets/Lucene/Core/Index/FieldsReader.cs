	...
	public sealed class FieldsReader : ICloneable, IDisposable
	{
		...
		public /*internal*/ Document Doc(int n, FieldSelector fieldSelector)
		{
			...
			for (int i = 0; i < numFields; i++)
			{
				...
				var fi = fieldInfos.FieldInfo(fieldNumber);
				var acceptField = fieldSelector == null?FieldSelectorResult.LOAD:fieldSelector.Accept(fi.name);
		...
		private void  AddFieldLazy(Document doc, FieldInfo fi, bool binary, bool compressed, bool tokenize)
		{
			...
			else
			{
				...
				var index = FieldExtensions.ToIndex(fi.isIndexed, tokenize);
				var termVector = FieldExtensions.ToTermVector(fi.storeTermVector, fi.storeOffsetWithTermVector, fi.storePositionWithTermVector);
		...
		private sealed class LazyField : AbstractField
		{
			...
			{
				var localFieldsStream = Enclosing_Instance.fieldsStreamTL.Get();
		    ...
		    public override string StringValue
		    {
		        get
		        {
                    ...
                    if (fieldsData == null)
		        	{
		        		var localFieldsStream = GetFieldStream();
		        		try
		        		{
