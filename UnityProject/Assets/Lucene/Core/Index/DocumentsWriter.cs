	...
	public sealed class DocumentsWriter : IDisposable
	{
		internal class AnonymousClassIndexingChain:IndexingChain
		{
			...
			internal override DocConsumer GetChain(DocumentsWriter documentsWriter)
			{
				...
				*/
                ...
                var normsWriter = new NormsWriter();
				var docInverter = new DocInverter(termsHash, normsWriter);
		...
		* including any flushed segments. */
		...
		internal int Flush(bool closeDocStore)
		{
			lock (this)
			{
				...
				try
				{
					...
					if (infoStream != null)
					{
                        var si = new SegmentInfo(flushState.segmentName, flushState.numDocs, directory);
                        long newSegmentSize = si.SizeInBytes();
                        var message = System.String.Format(nf, "  oldRAMSize={0:d} newFlushedSize={1:d} docs/MB={2:f} new/old={3:%}",
		...
		internal void  CreateCompoundFile(System.String segment)
		{
			...
			var cfsWriter = new CompoundFileWriter(directory, segment + "." + IndexFileNames.COMPOUND_FILE_EXTENSION);
		...
		internal DocumentsWriterThreadState GetThreadState(Document doc, Term delTerm)
		{
			lock (this)
			{
				...
				// again.
				var state = threadBindings[ThreadClass.Current()];
				if (state == null)
				{
					...
					else
					{
						...
						var newArray = new DocumentsWriterThreadState[1 + threadStates.Length];
		...
		internal bool UpdateDocument(Document doc, Analyzer analyzer, Term delTerm)
		{
			...
			var state = GetThreadState(doc, delTerm);
			
			var docState = state.docState;
			docState.doc = doc;
		...
		internal void  RemapDeletes(SegmentInfos infos, int[][] docMaps, int[] delCounts, MergePolicy.OneMerge merge, int mergeDocCount)
		{
			lock (this)
			{
                ...
                var mapper = new MergeDocIDRemapper(infos, docMaps, delCounts, merge, mergeDocCount);
		...
		internal bool ApplyDeletes(SegmentInfos infos)
		{
			lock (this)
			{
				...
				for (int i = 0; i < infosEnd; i++)
				{
					...
					var reader = writer.readerPool.Get(infos.Info(i), false);
					try
					{
		...
		private bool ApplyDeletes(IndexReader reader, int docIDStart)
		{
			lock (this)
			{
				...
				foreach(var entry in deletesFlushed.queries)
				{
					var query = (Query) entry.Key;
					int limit = (int)entry.Value;
					var weight = query.Weight(searcher);
					var scorer = weight.Scorer(reader, true, false);
					if (scorer != null)
					{
						while (true)
						{
		...
		private void  AddDeleteTerm(Term term, int docCount)
		{
			lock (this)
			{
				var num = deletesInRAM.terms[term];
			...
			public bool Add(DocWriter doc)
			{
				lock (this)
				{
					...
					else
					{
						...
						{
							// Grow queue
							var newArray = new DocWriter[ArrayUtil.GetNextSize(gap)];
