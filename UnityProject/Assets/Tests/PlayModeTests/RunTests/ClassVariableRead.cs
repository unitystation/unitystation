...
public class ClassVariableRead {
  ...
  public enum VariableCheckType { String, Number, Bool }
  ...
  private BindingFlags SetBindingFlags =
      BindingFlags.Public | BindingFlags.Instance | BindingFlags.NonPublic |
      BindingFlags.Static | BindingFlags.FlattenHierarchy;

  public bool SatisfiesConditions(Type InType, object ClassObject,
                                  out string FailReport) {
    ...
    case VariableCheckType.Bool:
      ...
      if (Potentialbool == null) {
        FailReport =
            $"at {VariablePath} Variable did not match expected Type ( is in you specified a Number when it actually was a bool or string) The type of variable is {MI.GetUnderlyingType()} Was expecting bool ";
        return false;
      }

      if (Potentialbool.Value != TargetBoolValue) {
        FailReport =
            $"at {VariablePath} Variable bool did not match Expected {TargetBoolValue} was {Potentialbool.Value} ";
        return false;
      } else {
        FailReport = "";
        return true;
    ...
    case VariableCheckType.String:
      ...
      if (Value != TargetStringValue) {
        FailReport =
            $"at {VariablePath} Variable String did not match Expected {TargetStringValue} was {Value} ";
        return false;
      } else {
        FailReport = "";
        return true;
    ...
    case VariableCheckType.Number:
      ...
      if (PotentialNumberValue == null) {
        FailReport =
            $"at {VariablePath} Variable did not match expected Type ( is in you specified a Number when it actually was a bool or string) The type of variable is {MI.GetUnderlyingType()} Was expecting Float/any type of number Type ";
        return false;
      }
      ...
      if (HasMin || HasMax) {
        if (HasMin) {
          if (Min > PotentialNumberValue.Value) {
            FailReport =
                $"at {VariablePath} Value was below minimum of {Min} at {PotentialNumberValue.Value} ";
            return false;
          }
        }
        ...
        if (HasMax) {
          if (Max < PotentialNumberValue.Value) {
            FailReport =
                $"at {VariablePath} Value was Over maximum of {Max} at {PotentialNumberValue.Value} ";
            return false;
          }
        }
        ...
        if (PotentialNumberValue.Value != TargetValue) {
          FailReport =
              $"at {VariablePath} Value did not match The TargetValue of {TargetValue} was {PotentialNumberValue.Value}";
          return false;
        } else {
          FailReport = "";
          return true;
    ...
    FailReport =
        "Test was not within switch statement, impossible HELP!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!";
    return false;
  }
}
