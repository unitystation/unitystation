...
public partial class TestAction {
  ...
  public class ShowPrefab {
    ...
    [Range(1, 100)]
    public int NumberPresent = 1;
    [Range(0, 100)]
    public int TheStackAmount = 0;
    ...
    public bool Initiate(TestRunSO TestRunSO) {
          ...
          if (PrefabTracker.ForeverID == OriginalID) {
            ...
            if (InverseMustNotPresent) {
              ...
              TestRunSO.Report.AppendLine(
                  "Prefab is present, it should not be " +
                  PrefabTracker.gameObject);
              return false;
            }
            ...
            if (TheStackAmount != 0) {
              ...
              if (Stackable == null) {
                TestRunSO.Report.AppendLine(CustomFailedText);
                TestRunSO.Report.AppendLine("Stackable Was not present on " +
                                            PrefabTracker.gameObject);
                return false;
              } else {
      ...
      if (NumberPresent == Present) {
        ...
        TestRunSO.Report.AppendLine(CustomFailedText);
        TestRunSO.Report.AppendLine(
            "There was not the expected number of gameobjects Expected " +
