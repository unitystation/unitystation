...
public partial class TestAction {
  public bool ShowIsInPlayerInventory =>
      SpecifiedAction == ActionType.IsInPlayerInventory;

  [AllowNesting]
  [ShowIf("ShowIsInPlayerInventory")]
  public IsInPlayerInventory DataIsInPlayerInventory;
  ...
  public class IsInPlayerInventory {
    ...
    [AllowNesting]
    [ShowIf("NotLocalPlayer")]
    public Vector3 WorldPositionOfPlayer;

    public GameObject ObjectToSearchFor;

    public bool TargetSpecifiedSlot;
    [AllowNesting]
    [ShowIf("TargetSpecifiedSlot")]
    ...
    public bool Initiate(TestRunSO TestRunSO) {
      ...
      if (NotLocalPlayer) {
        var Magix =
            MatrixManager.AtPoint(WorldPositionOfPlayer.RoundToInt(), true);
        var List = Magix.Matrix.ServerObjects.Get(
            WorldPositionOfPlayer.ToLocal(Magix).RoundToInt());
        foreach (var registerTile in List) {
          if (registerTile.TryGetComponent<DynamicItemStorage>(
                  out DynamicItemStorage)) {
            break;
          }
        }
      } else {
        DynamicItemStorage =
            PlayerManager.LocalPlayer.GetComponent<DynamicItemStorage>();
      }

      if (DynamicItemStorage == null) {
        TestRunSO.Report.AppendLine(
            "Unable to find players inventory"); // IDK Maybe this should be
                                                 // here maybe not
        return false;
      }
      ...
      var OriginalID =
          ObjectToSearchFor.GetComponent<PrefabTracker>().ForeverID;
        ...
        if (slot.Item != null) {
          ...
          if (Found) {
            if (Inverse) {
              ...
              TestRunSO.Report.AppendLine(
                  $"{ObjectToSearchFor.name} Prefab was found in players inventory ");
              return false;
            } else {
              return true;
            }
      ...
      } else // Something wasn't found
      {
        TestRunSO.Report.AppendLine(CustomFailedText);
        TestRunSO.Report.AppendLine(
            $"{ObjectToSearchFor.name} Prefab was not found in players inventory ");
        return false;
      }
    }
  }

  public static bool RecursiveSearch(GameObject ObjectToCheck,
                                     string OriginalID) {
