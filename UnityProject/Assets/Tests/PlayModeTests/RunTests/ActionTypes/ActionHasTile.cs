...
public partial class TestAction {
  ...
  public class HasTile {
    ...
    public bool Initiate(TestRunSO TestRunSO) {
      ...
      if (NoTileAt == false) {
        var TileAt = Magix.Matrix.MetaTileMap.GetTile(
            WorldPosition.ToLocal(Magix).RoundToInt(), LayerTile.LayerType);

        if (TileAt != LayerTile) {
          TestRunSO.Report.AppendLine(CustomFailedText);
          TestRunSO.Report.AppendLine(
              $"Tile does not match specified one at {WorldPosition} is {TileAt.OrNull()?.name} When it should be {LayerTile.name} ");
          return false;
        } else {
          return true;
        }
        ...
        var TileAt = Magix.Matrix.MetaTileMap.GetTile(
            WorldPosition.ToLocal(Magix).RoundToInt(), LayerType);

        if (TileAt != null) {
          TestRunSO.Report.AppendLine(CustomFailedText);
          TestRunSO.Report.AppendLine(
              $"A tile is present at {WorldPosition} it should not be, it is {TileAt.name} ");
          return false;
        } else {
          return true;
        }
