...
public partial class TestAction {
  public bool ShowCheckValueGameObjectAt =>
      SpecifiedAction == ActionType.CheckValueGameObjectAt;

  [AllowNesting]
  [ShowIf("ShowCheckValueGameObjectAt")]
  public CheckValueGameObjectAt CheckValueGameObjectAtData;
  ...
  public class CheckValueGameObjectAt {
    ...
    public bool Initiate(TestRunSO TestRunSO) {
      ...
      var List = Magix.Matrix.ServerObjects.Get(
          PositionToCheck.ToLocal(Magix).RoundToInt());
      ...
      foreach (var Object in List) {
        ...
        if (PrefabTracker != null) {
          if (PrefabTracker.ForeverID == OriginalID) {
            ...
            if (Parameters.SatisfiesConditions(mono.GetType(), mono,
                                               out var ReportString)) {
              return true;
            } else {
              TestRunSO.Report.AppendLine(CustomFailedText);
              TestRunSO.Report.AppendLine(
                  $" on the Game object {PrefabTracker.name} the Component Variable check on {ComponentName} Failed because of ");
