...
#pragma warning restore 618
			...
			if (hasSpawned)
			{
				...
				Debug.LogError(
					$"{name} in {(transform.parent == null ? "(no parent)" : transform.parent.name)} at position {transform.position} has already spawned. " +
		...
		void AssignSceneID()
		{
			...
			if (sceneId == 0 || duplicate)
			{
				...
				if (BuildPipeline.isBuildingPlayer)
					...
					Debug.LogWarning("Scene " + gameObject.scene.path +
		...
		internal void OnStartClient()
		{
			...
			{
				Debug.LogError("hey NetworkBehaviours is null on " + gameObject + "at " +
				               GetGameObjectPath(gameObject) + " at " + gameObject.transform.localPosition);
				return;
			}
		...
		internal void AddObserver(NetworkConnection conn)
		{
			...
			{
				Debug.LogError(
					$"The server tried to add a disconnected player to the list of {gameObject.name} NetworkIdentity observers");
				return;
			}
			/// </CUSTOM UNITYSTATION CODE>
