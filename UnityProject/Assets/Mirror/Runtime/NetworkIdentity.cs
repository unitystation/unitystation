...
namespace Mirror
{
	...
	{
		//HoldStates if gameobject is inactive (like on start up)
		Queue<PayloadItem> payLoadQueue = new Queue<PayloadItem>();

		class PayloadItem
		{
			public NetworkReader payloadReader;
			public bool initialPayload;
		}
...
#pragma warning restore CS0649
		...
		private void OnEnable()
		{
			...
			{
				NetworkManager.singleton.lateUpdateAction += LateUpdateMe;
			}
		}
		...
		void AssignSceneID()
		{
			...
			bool duplicate = sceneIds.TryGetValue(m_SceneId, out NetworkIdentity existing) && existing != null &&
			                 existing != this;
		...
		public void SetSceneIdSceneHashPartInternal()
		{
			...
			if (LogFilter.Debug)
				Debug.Log(name + " in scene=" + gameObject.scene.name + " scene index hash(" + pathHash.ToString("X") +
				          ") copied into sceneId: " + m_SceneId.ToString("X"));
		}
		...
		internal void OnStartClient()
		{
			...
			if (LogFilter.Debug)
				Debug.Log("OnStartClient " + gameObject + " netId:" + netId + " localPlayerAuthority:" +
		...
		bool OnSerializeSafely(NetworkBehaviour comp, NetworkWriter writer, bool initialState)
		{
			...
			if (LogFilter.Debug)
				Debug.Log("OnSerializeSafely written for object=" + comp.name + " component=" + comp.GetType() +
				          " sceneId=" + m_SceneId.ToString("X") + "header@" + headerPosition + " content@" +
		...
		void OnDeserializeSafely(NetworkBehaviour comp, NetworkReader reader, bool initialState)
		{
			...
			try
			{
				if (LogFilter.Debug)
					Debug.Log("OnDeserializeSafely: " + comp.name + " component=" + comp.GetType() + " sceneId=" +
					          m_SceneId.ToString("X") + " length=" + contentSize);
			...
			catch (Exception e)
			{
				...
				Debug.LogError("OnDeserialize failed for: object=" + name + " component=" + comp.GetType() +
				               " sceneId=" + m_SceneId.ToString("X") + " length=" + contentSize +
				               ". Possible Reasons:\n  * Do " + comp.GetType() +
		...
		void ProcessPayloadQueue(PayloadItem payload)
		{
		...
		public bool AssignClientAuthority(NetworkConnection conn)
		{
			...
			{
				Debug.LogError(
					"AssignClientAuthority can only be used for NetworkIdentity components with LocalPlayerAuthority set.");
				return false;
			}
