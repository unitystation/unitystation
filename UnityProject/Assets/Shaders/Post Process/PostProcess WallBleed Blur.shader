...
Shader "PostProcess/WallBleed Blur"
{
	Properties
	{
		_MainTex ("", 2D) = "white" {}
	...
	fixed4 frag5Blur (output_5tap IN) : SV_Target
	{
		...
		fixed3 occlusionMask = tex2D(_FovMask, IN.texcoord).xyz;

		fixed3 bleedSample = mainSample.rgb + (blurredSum * occlusionMask.r * 0.25f);

		return bleedSample.rgbb;
	}
