...
Shader "Stencil/Zombified Object FOV Masked"
{
    Properties
    {
        ...
        [Toggle(MIX_WOUNDS)] _MixWoundColors ("Mix Wound Colours", Float) = 0

        // Deep fry effect, set per-instance via MaterialPropertyBlock
        [PerRendererData] _FryIntensity("Fry Intensity", Range(0,1)) = 0
        _WoundOverlayTex("Wound Overlay Texture", 2D) = "white" {}
        _WoundColor("Wound Colour", Color) = (0.6,0.15,0.1, 1)
        _FleshColor("Flesh Colour", Color) = (0.2, 0.5, 0.3, 1)
    }
   ...
   // IMPORTANT: goes here, not inside frag
            ...
            #pragma multi_compile _ MIX_WOUNDS
            ...
            uniform sampler2D _WoundOverlayTex;
            uniform fixed4 _WoundColor;
            uniform fixed4 _FleshColor;
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float2 hashInput = floor(i.texcoord / (_MainTex_TexelSize*32));
                float seed = frac(sin(dot(hashInput, float2(12.9898, 78.233))) * 1367.45);
                seed = floor(seed * 32.0 + 0.5) / 32.0;

                float2 offset = float2(seed, fmod(seed, 4.0));
                fixed4 woundSample = tex2D(_WoundOverlayTex, i.spritecoord + offset);
                ...
                float luminosity = dot(final, BT709);

                float mask = woundSample.a;
                float3 woundHue = _WoundColor * woundSample * luminosity;
                float3 fleshHue = _FleshColor.rgb * luminosity;
                fleshHue = lerp(fleshHue.rgb, final.rgb, 0.25f);
               
                #ifdef MIX_WOUNDS
                    woundHue = lerp(woundHue.rgb, final.rgb, 0.25f);
                #endif
                
                final.rgb = lerp(fleshHue.rgb, woundHue.rgb, mask);
                final.a = ((1-mask)* _FleshColor.a * final.a) + mask * _WoundColor.a * final.a;
                ...
                final.a = final.a * alphaFactor * i.color.a;
